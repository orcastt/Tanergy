...
**Updated**: 2026-05-20
**Status**: Canonical product overview and PRD slice index, aligned with the current parallel P0 alpha workstreams, the S3 commercial pivot, the S3/S4 Team-Group foundation baseline and the latest S1B infrastructure/ops cleanup: the Konva-only staging deploy is back online, strict Clerk session/admin smoke is green, staging database truth is now a fresh Supabase Pro Postgres project, the temporary Hetzner-local Postgres fallback has been removed, R2 staging objects were cleared for a clean asset lane, collaboration process updates no longer write every realtime change to Postgres, deployment readiness now has Web/API env templates plus TLS/header/CORS smoke and incident-response docs, `/account` and admin share a real hard-delete account path with Team/Group ownership guards, and the active image-generation product surface aligns to four live Jiekou-first image models before the remaining live image smoke and deeper Yjs/provider work.
...
| S1B Staging Infra | `PRD_slice_S1B_staging_infra.md` | Online staging Web/API, Postgres, R2, DNS, monitoring and email readiness | In progress; rebuilt staging Web/API/Supabase Pro/R2 and Konva-only redeploy are green, the active release no longer depends on the retired dirty worktree or temporary local Postgres fallback, repo-owned ops smoke/env templates/runbooks now exist, and R2 clean asset, Google/email, live AI and external monitoring/backup/error-tracking proof remain |
...
S1B Deploy/staging            90%  rebuilt Hetzner API host, public HTTPS API, Supabase Pro fresh schema, historical R2/board smoke and the Konva-only staging web deploy are green; release-style staging deploy, private shared env handling, Web/API env templates, ops readiness smoke and incident runbook are in place, while R2 clean asset, Google/email, live AI smoke and external monitoring/backup/error-tracking proof remain
...
S2 AI runtime/provider routes 68%  Jiekou-first local UX path exists; backend short-text `AiRun`, durable `text_output`, message-native chat backendization, four-model image refresh and longer image timeout now exist; remaining gate is live image smoke and broader provider coverage
...
S4 Collaboration              34%  local Yjs room/document foundation, reconnect/resync smoke harness, draft drawing presence preview and final-snapshot persistence exist; provider-grade multiplayer is still deferred from the release promise
```
...
- Full production provider coverage across every AI node type beyond the current Jiekou-first local fast path and the first server AiRun smoke route.
- Collections as a true asset-library product.
- Deep enterprise governance and finance tooling.

Near-term execution order:

1. Run the staging ops readiness smoke after the next deploy, then finish R2 clean asset smoke, staging spot check the now-wired `Manage board -> Copy board` Free-plan limit modal path, and finish Google/email verification plus production-like CORS/origin behavior.
...
S2 AI Productization
  - canvas-facing Jiekou-first fast path proven locally
...
S1B Deploy/staging            90%  重建后的 Hetzner API、public HTTPS API、Supabase Pro fresh schema、历史 R2/board smoke 与 Konva-only staging Web 已转绿；Web/API env 模板、ops readiness smoke、错误追踪接入和事故手册已存在；R2 clean asset、Google/email、live AI smoke 与外部监控/备份/错误追踪证明仍待完成
...
S4 Collaboration              34%  本地 Yjs room/document 基础、reconnect/resync smoke harness、draft drawing presence preview 和 final-snapshot persistence 已存在；provider 级多人协作仍后置
```
...
- 超出当前 Jiekou-first 本地 fast path 和第一条服务端 AiRun smoke route 的完整生产 provider 覆盖。
...
3. 把 Jiekou-first 画布 fast path 收口到 server-side AiRun provider-route/billing control plane。
