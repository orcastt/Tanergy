...
| S3 Admin/Billing/Analytics | `PRD_slice_S3_admin_billing_analytics.md` | Admin access, user management, credits, subscriptions, Team wallets, Group/Collaborate personal wallets, workspace dashboards, analytics, moderation | Active pivot: reusable admin/ledger/seat/AiRun scaffolds exist, but Team charging must move from member-personal seat credits to a workspace-owned Team wallet; real payment webhooks, finance reconciliation and production provider settlement remain pending |
...
S1A Schema/DB foundation      80%  core join points exist; Team-wallet schema delta and staging smoke remain
...
| S3 Admin/Billing/Analytics | `PRD_slice_S3_admin_billing_analytics.md` | Admin access、user management、credits、subscriptions、Team wallets、Group/Collaborate personal wallets、workspace dashboards、analytics、moderation | 活跃调整：admin/ledger/seat/AiRun 脚手架可复用，但 Team 扣费必须从成员个人 seat credits 改成 workspace-owned Team wallet；真实 payment webhooks、finance reconciliation 和生产 provider settlement 仍待完成 |
...
S1A Schema/DB foundation      80%  core join points 已存在；Team-wallet schema delta 和 staging smoke 仍待完成
...
S1D Board/share/invites       68%  CRUD/share/member 第一阶段存在；workspace invite accept 与 Team/Group permission split 待完成
