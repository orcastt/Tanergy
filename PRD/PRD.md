...
**Updated**: 2026-05-18
**Status**: Canonical product overview and PRD slice index, aligned with the current parallel P0 alpha workstreams, the S3 commercial pivot, the S3/S4 Team-Group foundation baseline and the latest S1B infrastructure cleanup: the Konva-only staging deploy is back online, strict Clerk session/admin smoke is green, staging database truth is now a fresh Supabase Pro Postgres project, the temporary Hetzner-local Postgres fallback has been removed, R2 staging objects were cleared for a clean asset lane, collaboration process updates no longer write every realtime change to Postgres, `/account` and admin share a real hard-delete account path with Team/Group ownership guards, and the active image-generation product surface aligns to four live Jiekou-first image models before the remaining live image smoke and deeper Yjs/provider work.
...
S1B Deploy/staging            88%  rebuilt Hetzner API host, public HTTPS API, Supabase Pro fresh schema, historical R2/board smoke and the Konva-only staging web deploy are green; release-style staging deploy plus private shared env handling are in place, while R2 clean asset, Google/email and live AI smoke remain
...
1. Finish R2 clean asset smoke, staging spot check the now-wired `Manage board -> Copy board` Free-plan limit modal path, and finish Google/email verification plus production-like CORS/origin behavior.
...
**更新日期**：2026-05-18
**状态**：规范产品总览和 PRD 切片索引，当前已对齐本轮 P0 alpha 的并行工作流、S3 商业口径调整，以及最新 S1B/S2 检查点：Konva-only staging 部署已恢复上线，严格 Clerk session/admin smoke 已转绿，staging 数据库事实源已切到 fresh Supabase Pro Postgres，临时 Hetzner 本机 Postgres fallback 已删除，R2 staging 对象已清理，协同过程更新不再逐条写入 Postgres，活跃生图产品面已收口到四个真实 Jiekou-first 图片模型；第二轮 signed-in Board 验收大部分已绿，`Manage board -> Copy board` 的 Free-plan limit 弹窗路径已完成本地 wiring，剩余闸门是 R2 clean asset smoke、该弹窗路径的 staging spot check、Google/email 和一条真实服务端生图 smoke，然后才继续更深的 Yjs/provider 工作。
...
| S1B Staging Infra | `PRD_slice_S1B_staging_infra.md` | 在线 staging Web/API、Postgres、R2、DNS 和 email readiness | 进行中；重建后的 staging Web/API/Supabase Pro/R2 与 Konva-only redeploy 已转绿；临时本机 Postgres fallback 已删除；R2 clean asset、Google/email 与 live AI 验收仍待完成 |
...
S1B Deploy/staging            88%  重建后的 Hetzner API、public HTTPS API、Supabase Pro fresh schema、历史 R2/board smoke 与 Konva-only staging Web 已转绿；R2 clean asset、Google/email 与 live AI smoke 仍待完成
