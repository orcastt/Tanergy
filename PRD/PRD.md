...
**Updated**: 2026-05-16
**Status**: Canonical product overview and PRD slice index, aligned with the current parallel P0 alpha workstreams, the S3 commercial pivot and the latest staging/S2 checkpoint: the Konva-only staging deploy is back online, strict Clerk session/admin smoke is green, `/account` and admin now share a real hard-delete account path with Team/Group ownership guards, and the active image-generation product surface aligns to four live Jiekou-first image models while the remaining gate is one live server-backed image smoke before deeper Yjs/provider work.
