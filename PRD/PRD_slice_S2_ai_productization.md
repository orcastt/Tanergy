...
- AI Chat automatic graph planning, durable long text-output persistence and broad refund/reconciliation depth are not current-alpha promises.
...
| Image Gen | Prompt Node -> Image Gen creates one image Asset. | Local GeekAI fast path exists; production gate is server AiRun route/settlement path |
| Image Gen 4 | Prompt Node -> Image Gen 4 creates four candidate image Assets. | Local repeated-call UX exists; production gate is server AiRun output-slot settlement |
| Image edit/reference | Image Node + Prompt Node -> Image Gen creates edited/fused image Asset. | Local GeekAI image/reference path exists; production gate is server Asset refs and provider adapter mapping |
...
- AI Chat 自动 graph planning、长文本 durable text-output persistence 以及更深的 refund/reconciliation depth，都不是当前 alpha 的承诺。
...
| Image Gen | Prompt Node -> Image Gen 创建一个 image Asset。 | 本地 GeekAI fast path 已存在；生产闸门是 server AiRun route/settlement path |
| Image Gen 4 | Prompt Node -> Image Gen 4 创建四个候选 image Assets。 | 本地 repeated-call UX 已存在；生产闸门是 server AiRun output-slot settlement |
| Image edit/reference | Image Node + Prompt Node -> Image Gen 创建 edited/fused image Asset。 | 本地 GeekAI image/reference path 已存在；生产闸门是 server Asset refs 和 provider adapter mapping |
