...
| Account deletion | Self-delete must remain available for privacy compliance, but paid Team/Group relationships cannot be orphaned. Solo/free-only users may delete immediately; users with active Team/Group ownership, active paid memberships, seat assignments or subscription obligations must first transfer, leave, cancel or clear those bindings. | First-pass hard-delete now exists for self-delete and admin delete with Team/Group ownership guards; blocker checklist for joined paid memberships/subscription cleanup is the next hardening cut |
