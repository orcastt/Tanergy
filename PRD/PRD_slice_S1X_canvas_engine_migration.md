...
- Konva Image Node first pass supports Canvas Image → Image Node, Image Node → Canvas Image and Capture selection → Image Node from the selection toolbar. Capture uses explicit selectedIds export bounds, uploads the generated PNG with `origin=merge_capture`, places the Image Node below the selection and keeps Board documents free of data/blob/Base64 payloads. Right-click Copy as PNG and Export as PNG use the same transparent selectedIds capture; Copy/Export as SVG has a limited first-pass vector serializer with placeholders for image/node internals.
