...
- Right-click menu Phase 3A exposes professional canvas submenus for Edit, Arrange, Reorder, Copy as and Export as. Cut, Group/Ungroup, Lock/Unlock, multi-selection Align/Distribute/Stretch/Flip, row/column tidy, Copy as PNG/SVG and Export as PNG/SVG are functional first passes; Move to page and transparent-background toggles remain disabled until their product contracts are implemented.
...
- Phase 5A adds a first-pass Konva persistence contract: Konva documents save as a v2 `{ renderer: 'konva', version: 2, activePageId, pages, canvasDocument }` envelope through the existing Board API, restore the active page's shapes/images/nodes/runtime edges/camera/settings, create board thumbnails from an offscreen Konva capture, and participate in Save now, autosave, Cmd/Ctrl+S, Snapshot/History/Clean and before-unload warning flows. Users can switch pages, create blank pages and double-click rename pages from the right-side collapsible Konva Pages drawer. Board History entries show the active Page title rather than repeating the Board title. Rendered page thumbnails, page delete/reorder and right-click Move to page remain follow-ups. The Board guard understands the v2 envelope, pages contract and runtime edge refs before persistence. The formal `/boards/[boardId]` route now has a dual-engine first pass for development/migration safety, but production defaults to Konva-only: tldraw v1 documents are blocked unless the explicit reference flag is enabled, unknown saved documents are blocked instead of auto-overwritten, and new Boards default to Konva.
