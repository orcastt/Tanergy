...
- Phase 5A adds a first-pass Konva persistence contract in the spike route: Konva documents save as a v2 `{ renderer: 'konva', version: 2, canvasDocument }` envelope through the existing Board API, restore shapes/images/nodes/runtime edges/camera/settings, create board thumbnails from an offscreen Konva capture, and participate in Save now, autosave, Cmd/Ctrl+S, Snapshot/History/Clean and before-unload warning flows. The Board guard understands the v2 envelope and blocks malformed camera/shape/runtime-edge/asset-ref structures before persistence. The production `/boards/[boardId]` route still remains on the current tldraw implementation until this path passes parity review.
