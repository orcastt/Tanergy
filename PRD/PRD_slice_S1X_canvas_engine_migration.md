...
**Updated**: 2026-05-18
**Status**: Konva v2 is the accepted formal Board runtime for new/saved Boards locally and on staging; Page polish, v1 copy tooling, runtimeGraph mock dataflow, public share viewing, plan-aware page-limit dialog, node minimum sizing and draft drawing collaboration preview have first passes. Remaining work is export/background polish, performance regression, real AiRun wiring and production-grade collaboration proof.
...
- Production-grade Yjs collaboration proof, rendered page-thumbnail assets, page duplicate and broader export polish remain deferred.
...
- Node cards are intentionally compact: port types are shown on hover as black text/image tooltips instead of permanent internal labels; Prompt and Analysis preset text boxes wrap and scroll long content inside the node without a global overlay, so normal canvas z-order still decides whether later lines/shapes cover the node; text is then edited in-place through a focused textarea overlay; node cards cannot be resized below their registry default size, keeping chat inputs, controls, text boxes and previews inside their card body; Image Gen/Image Gen 4 expose first-pass model, aspect ratio and resolution controls; Image Gen 4 exposes four image outputs. Image Gen/Image Gen 4 dynamically add image input ports as image references are connected and shrink them when disconnected.
...
- Phase 4A image operations add selected-image actions for `Remove BG` and `Object Cutout`. Current truthful product status is narrower than the original contract note: `Remove BG` is visible in the frontend but is not yet accepted as a live end-to-end feature until the backend/API/staging flow is reconnected and verified; `Object Cutout` remains a disabled first-pass affordance while the future point/box prompt with `facebookresearch/segment-anything` is designed.
...
- Phase 5A adds a first-pass Konva persistence contract: Konva documents save as a v2 `{ renderer: 'konva', version: 2, activePageId, pages, canvasDocument }` envelope through the existing Board API, restore the active page's shapes/images/nodes/runtime edges/camera/settings, create board thumbnails from an offscreen Konva capture, and participate in Save now, autosave, Cmd/Ctrl+S, Snapshot/History/Clean and before-unload warning flows. Users can switch pages, create blank pages, double-click rename pages, delete pages, reorder pages and identify pages by lightweight geometry thumbnails from the right-side collapsible Konva Pages drawer. The Pages drawer now shows the current plan page count and opens a centered upgrade dialog instead of creating/duplicating beyond the plan page limit. Board History entries show the active Page title rather than repeating the Board title. Right-click Move to page moves the current selection to another page, expands grouped members/frame children as one move scope, preserves runtime edges only when both endpoints move together and drops cross-page edges for now. True rendered page-thumbnail assets, Move selection to new page and page-scoped production collaboration remain follow-ups. The Board guard understands the v2 envelope, pages contract and runtime edge refs before persistence. The formal `/boards/[boardId]` route is now Konva-only in the active app: legacy v1 or unknown documents are blocked instead of opening any fallback runtime.
...
- Canvas header with Workspace back and inline Board rename. Board switcher/new/open affordances are intentionally outside the canvas top bar.
...
- draft drawing/process previews appear at reduced cadence before final save
