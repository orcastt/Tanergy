...
> 本文件记录旧 Tauri 桌面端 + FastAPI 商业化架构，当前不再作为主开发入口。新的 Web 协作画布架构草案见 `ARCH.web-collab.md`；当前执行计划见 `dev-plans/web-collaborative-canvas-pivot.md`。

**版本**: v0.8
**日期**: 2026-04-27
**状态**: Phase 2 商业化 — 核心能力完成，当前 P0 为 Admin Provider/Model 管理闭环
**上次更新**: Admin Provider/Model 默认值、fallback、health/test 计划设为当前主架构收口点
...
| FastAPI `/api/v1/proxy/image/chat` | Chat Completions 图片生成代理（Gemini/Nano Banana 路线） | 后端 Provider Registry |
...
/api/v1/proxy/image/chat     POST  Chat Image 图片代理（扣积分）
...
图片素材保存到 `$APPDATA/library/images`，工作流运行产生的图片仍保存到 `$APPDATA/assets/{workflow_id}`。`image_list` 生成图会同时写入工作流 assets 并登记为图片素材库条目，输出中携带 `asset_id`、`file_path`、`library_item_id`、`library_file_path`。Tauri `assetProtocol` 同时允许 `assets/**` 和 `library/**` 用于本地预览。
