...
import { getDefaultModelForCategory } from "./officialModelsStore"
...
export function splitOutlineImpl(
  ...
  if (imagePlans && imagePlans.length > 0) {
    ...
    newNodes.push({
      ...
      data: { nodeType: "image_list", count: imagePlans.length, model: getDefaultModelForCategory("image"), quality: "low", image_size: "0.5K", imageInputs: ["img_in_1"] },
      selected: false,
    } as Node)
    newEdges.push({
  ...
  if (!existingFormatter) {
    ...
    newNodes.push({
      ...
      data: {
        ...
        fontSize: 16,
        lineHeight: 1.75,
        model: getDefaultModelForCategory("text"),
