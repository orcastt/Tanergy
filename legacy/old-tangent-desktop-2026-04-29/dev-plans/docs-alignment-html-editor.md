...
- 端到端跑通官方 API 主路径；用户自带 Key / BYOK 暂不作为默认开发路径。
- Admin Dashboard 基础前端已实现，下一步是 Provider/Model 默认值、fallback、health/test 与 App 默认模型联动，详见 `admin-provider-model-management-p0.md`。
