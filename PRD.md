...
**版本**: v0.6
**日期**: 2026-05-01
**状态**: Web 重启方向正式 PRD；S1.5 复杂节点与 Asset LOD Slice A-D 已通过，当前主线是 Slice E Real Asset Pipeline。FastAPI local-dev、真实 `s3-compatible` Asset adapter、Postgres Board / Asset metadata persistence、Web-to-FastAPI switch、staging API package 和 `/boards` entry shell 已落地。
**当前优先级**: P0 最小图像链路已在 canvas spike 中证明可行；当前从“本地可保存/加载 + staging package”进入“真实 staging 基础设施 + Dashboard / Board CRUD 产品化 + Auth + 真实 AI 生图”阶段。多人协作继续后置到 P0.5。
...
- `Save audit` / `Save board` / `Load board` 仍是显式保存控件；正式产品需要 autosave 或更清晰的 save indicator、失败恢复和离开页面提示。
...
- P0 真实 AI 调用、Model Registry、AI Run log、登录/Auth、右侧 AI Chat 产品化仍未完成。
...
| 4. 页面清单 | `/boards` 与 `/boards/:boardId` 已部分落地；`/login`、右侧 AI Chat、Image Editor modal、Preview 弹层仍待开发。 |
...
| 编号 | 功能 | 说明 | 优先级 | 当前状态 |
|------|------|------|--------|----------|
| F01 | Web 登录 | 用户能登录进入 Dashboard；P0 可先用现有认证或本地开发模式 | P0 | 待开发；当前仅有 dev request context / headers fallback |
...
|----|------|
| 路径 | `/login` |
| 当前状态 | 待开发；当前只有 dev request context，不应视为真实登录 |
| 作用 | 用户登录或注册 |
...
|----|------|
| 路径 | `/boards/:boardId` |
| 当前状态 | 已有 entry shell；复用 canvas runtime，可按 board id auto-load/save/load，同一 persistence contract 可切 local/FastAPI |
| 作用 | 核心画布编辑 |
...
| 待补 | Autosave / save indicator、离开页面保护、正式 load failure UI、隐藏或产品化 dev-only controls |
...
- 部分通过：Dashboard / Boards、Canvas Editor 产品入口、Board save/load、状态反馈、图片模型选择器、左侧 Inspector。
- 待开发：正式登录/Auth、邮箱验证、完整 Board CRUD、autosave/save indicator、Model Registry、AI Proxy、AI Run logs、真实 Image Gen / Analysis、右侧 AI Chat planner、预算熔断、完整 i18n。
...
- [ ] `/boards/:boardId` 有 autosave 或清晰 save indicator，保存失败不会丢失当前画布。
