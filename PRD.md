...
**当前优先级**: P0 最小图像链路已在 canvas spike 中证明可行；当前从“本地可保存/加载 + staging package”进入“Board save regression / external-resource staging wiring”阶段。Dashboard metadata、Auth scaffold 和 AI contract scaffold 已有 first pass；真实 staging 基础设施、正式 Auth、邮箱验证和真实 AI 生图仍在外部资源就绪后推进；多人协作继续后置到 P0.5。
...
- `/boards/:boardId` 已有 Board 模式保存状态条、autosave/debounce、dirty warning、load/save error fallback 和刷新/关闭/Back warning；autosave debounce 现在是 1200ms，并保留 no-op dirty 事件去重，避免序列化 document 未变化时重复调用保存 API；Dashboard 标题单击打开、双击重命名，画布标题支持双击重命名并使用持久化 `board.title`；`/spikes/canvas` 仍保留 `Save audit` / `Save local` / `Load local` 开发控件。
...
| F02 | Dashboard / Board 列表 | 创建、打开、重命名 Board | P0 | 部分完成；`/boards` 支持 list / create / open / search / rename / delete / refresh / thumbnail placeholder / shape-asset count / list limit / loading-empty-error first pass；Board 标题单击打开、双击重命名，row action Open / Rename 保留；captured thumbnail / recent metadata 待补 |
...
- 待开发：正式登录/Auth、邮箱验证、captured Dashboard thumbnails/recent-open metadata、真实 AI Proxy、持久化 AI Run logs、真实 Image Gen / Analysis、右侧 AI Chat planner、预算熔断、完整 i18n。
...
- [x] `/boards` 和 `/boards/:boardId` 支持 Board 标题双击重命名，并避免 autosave 把标题覆盖回 URL-derived fallback。
