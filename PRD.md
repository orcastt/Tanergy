...
- Workspace Board gallery 现在支持 Board summary list、New board、Open、Search、Sort、Rename、Delete、Copy、Refresh、Panel metadata、缩略图占位/URL/上传、shape/asset 对象计数、`lastOpenedAt` 最近打开、Recently opened / Recently saved 排序、client-side Load more、star/pin/share/private/public 菜单动作、成员 nickname/role scaffold、loading/empty/error first pass；真实 captured thumbnail、真实 share permission、真实 member persistence 和 server-side pagination 仍待产品化。
...
- `/boards/:boardId` 已有统一 Board History first pass：autosave 成功后写入 `autosave` history entry，Snapshot 按钮写入 `manual` entry，Cmd/Ctrl+S 执行当前 Board save 并写入 `keyboard` entry，History summary list 按时间倒序展示并支持 Restore。免费层默认每个 Board 保留最近 100 条；后续订阅分层可在 `retentionTier` / `expiresAt` 上扩展。
...
| F02 | Workspace Board 列表 | 创建、打开、重命名 Board | P0 | 部分完成；`/workspaces` 支持 gallery/list / create / open / search / sort / rename / copy / delete / refresh / Panel metadata / thumbnail placeholder+URL+upload / shape-asset count / `lastOpenedAt` / Load more / loading-empty-error first pass；Board 标题单击打开、双击重命名，Panel 可改标题、描述、卡片颜色、缩略图、star/pin 和 visibility，并展示 owner/created/location/details 与成员管理 scaffold；card menu 支持 share link、star、pin、make private/public；captured thumbnail / real member permission / server-side pagination 待补 |
...
| F19 | Board History | Autosave、Snapshot 按钮、Cmd/Ctrl+S 都进入同一 History 时间线，并支持 History 列表和恢复 | P0 | first pass 已实现；Next local bridge、FastAPI local-dev 和 FastAPI Postgres 均有 history create/list/load；免费层默认保留最近 100 条，订阅分层和长期冷存储待后续 |
| F20 | Smart Drawing | 用户用画笔画出近似直线、曲线、圆、矩形、三角形等时，系统可本地拟合为标准线条/形状 | P0.5 polish | 计划中；先做本地几何识别和可撤销转换，不接 AI provider |
...
| Smart Drawing polish | 手绘线条/形状识别、拟合和一键撤销 | P0.5/P1 |
...
当前状态：`/workspaces` gallery 已可生成 `board-<timestamp>` 并跳转 `/boards/:boardId?new=1`，也可从保存列表打开已有 Board；Search、Sort、Rename、Copy、Delete、Panel description/card color/thumbnail/star/pin/visibility/share/details/member scaffold、`lastOpenedAt` 最近打开排序和 Load more 已接 local/FastAPI Board contract；真实 captured thumbnail、真实 share/member permission、server-side pagination 和 Alpha 级状态 polish 仍待产品化。
...
| 待补 | Captured thumbnail、server-side pagination、Alpha 级空/错/加载 polish |
...
- Board Management Panel 可编辑 title、description、card color、thumbnail URL/upload、star/pin、private/workspace/public visibility，并显示 owner、created、last modified、location、objects。
...
- 画布 Settings 面板可调整单个 Board 的 Background（Dots / Grid / Solid）、background color、pattern color、spacing、Snap Alignment、Snap Strength、Zoom Sensitivity、Edge Color 和 AI Chat Style；Dots 背景必须是很小、低对比度的细点，不喧宾夺主。
...
- 用户可通过画笔旁的 toggle 或工具选项开启/关闭；关闭时保持原始 tldraw draw 行为。
- 用户松开鼠标/触控笔后再进行识别，不在拖动画笔时频繁重算导致卡顿。
- 近似直线可拟合为 line shape；近似圆/椭圆可拟合为 ellipse；近似矩形可拟合为 rectangle；近似三角形/菱形可拟合为 triangle/diamond。
...
当前状态：first pass 已落地。`/boards/:boardId` 支持统一 History：autosave 成功保存当前 Board 后写入 `autosave` entry，Snapshot 按钮写入 `manual` entry，Cmd/Ctrl+S 执行当前 Board save 并写入 `keyboard` entry，History 面板按时间倒序展示并支持恢复；Next local bridge、FastAPI local-dev 和 FastAPI Postgres adapters 都支持 history create/list/load。当前免费层默认每个 Board 保留最近 100 条。
...
- Board History 已有 Next local bridge 和 FastAPI persistence contract；list 只返回 summary，load 才返回 document；免费 retention 默认最近 100 条。
...
| `thumbnail_url` | string | 否 | 后续 captured thumbnail 可复用 |
...
当前 P0 规则：免费层每个 Board 默认保留最近 100 条 history entry，超过后删除最旧记录。后续大规模生产可把 history body 放入 R2/S3，只在 Postgres 保留 metadata。
...
- 部分通过：Workspace Board gallery、Canvas Editor 产品入口、Board save/load/autosave/History、per-board Canvas Settings、Board Management Panel、状态反馈、图片模型选择器、左侧 Inspector。
- 待开发：正式登录/Auth、邮箱验证、captured Workspace Board thumbnails/server-side pagination、Smart Drawing 实现、真实 share/member permission、真实 AI Proxy、持久化 AI Run logs、真实 Image Gen / Analysis、右侧 AI Chat planner、预算熔断、完整 i18n。
...
- [ ] `/workspaces` Board gallery/list 支持 captured thumbnail / server-side pagination。
- [x] `/workspaces` Board Panel 可编辑 Board title、description、preset card color、thumbnail、star/pin 和 visibility，且这些字段只作为 Board metadata 保存。
