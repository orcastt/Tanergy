...
- Workspace Board gallery 现在支持 Board summary list、New board、Open、Search、Sort、Rename、Delete、Refresh、缩略图占位、shape/asset 对象计数、`lastOpenedAt` 最近打开、Recently opened / Recently saved 排序、client-side Load more、loading/empty/error first pass；真实 captured thumbnail 和 server-side pagination 仍待产品化。
...
| F02 | Workspace Board 列表 | 创建、打开、重命名 Board | P0 | 部分完成；`/workspaces` 支持 gallery/list / create / open / search / sort / rename / delete / refresh / thumbnail placeholder / shape-asset count / `lastOpenedAt` / Load more / loading-empty-error first pass；Board 标题单击打开、双击重命名，row action Open / Rename 保留；captured thumbnail / server-side pagination 待补 |
...
当前状态：`/workspaces` gallery 已可生成 `board-<timestamp>` 并跳转 `/boards/:boardId?new=1`，也可从保存列表打开已有 Board；Search、Sort、Rename、Delete、`lastOpenedAt` 最近打开排序和 Load more 已接 local/FastAPI Board contract；captured thumbnail、server-side pagination 和 Alpha 级状态 polish 仍待产品化。
...
|----|------|
| 路径 | `/workspaces` |
| 当前状态 | 已有 first-pass product surface；支持 Board gallery/list、New board、Open、Search、Sort、Rename、Delete、Refresh、thumbnail placeholder、shape/asset count、`lastOpenedAt`、Recently opened / Recently saved sort、Load more、loading/empty/error first pass；`/boards` 列表入口重定向到该页面 |
| 作用 | 管理 Board |
...
- 部分通过：Workspace Board gallery、Canvas Editor 产品入口、Board save/load/autosave/History、状态反馈、图片模型选择器、左侧 Inspector。
- 待开发：正式登录/Auth、邮箱验证、captured Workspace Board thumbnails/server-side pagination、真实 AI Proxy、持久化 AI Run logs、真实 Image Gen / Analysis、右侧 AI Chat planner、预算熔断、完整 i18n。
