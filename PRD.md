...
- Workspace Board gallery 现在支持 Board summary list、New board、Open、Search、Sort、Rename、Delete、Copy、Refresh、settings-like Panel metadata、缩略图占位/URL/上传/保存时自动捕获/手动 Refresh preview/一键 Remove 回默认、History preview thumbnails、shape/asset 对象计数、`lastOpenedAt` 最近打开、Recently opened / Recently saved 排序、client-side Load more、star/pin/share/private/public 菜单动作、右上角红色 pin 与 private/public 图标、share/copy toast、public/private 交替确认、成员 nickname/role scaffold、owner/admin editable guard、loading/empty/error first pass；真实 share permission、真实 member persistence 和 server-side pagination 仍待产品化。
...
| F02 | Workspace Board 列表 | 创建、打开、重命名 Board | P0 | 部分完成；`/workspaces` 支持 gallery/list / create / open / search / sort / rename / copy / delete / refresh / settings-like Panel metadata / thumbnail placeholder+URL+upload+captured preview+manual Refresh preview+History preview thumbnails+remove default / shape-asset count / `lastOpenedAt` / Load more / loading-empty-error first pass；Board 标题单击打开、双击重命名，Panel 可改标题、描述、卡片颜色、缩略图、star/pin 和 visibility，并展示 owner/created/location/details 与成员管理 scaffold；owner/admin 可编辑，editor/viewer 灰态；card menu 支持 share link、star、pin、make private/public；real member permission / server-side pagination 待补 |
...
| F19 | Board History | Autosave、Snapshot 按钮、Cmd/Ctrl+S 都进入同一 History 时间线，并支持 History 列表、过滤、作者显示、缩略图预览和恢复 | P0 | first pass 已实现；Next local bridge、FastAPI local-dev 和 FastAPI Postgres 均有 history create/list/load；History summary 可携带 `thumbnailUrl`，document 仍只在 restore/load 时返回；免费层默认 autosave 100 + user saves 100 分桶保留，订阅分层和长期冷存储待后续 |
...
当前状态：`/workspaces` gallery 已可生成 `board-<timestamp>` 并跳转 `/boards/:boardId?new=1`，也可从保存列表打开已有 Board；Search、Sort、Rename、Copy、Delete、settings-like Panel description/card color/thumbnail upload/remove/star/pin/visibility/share/details/member scaffold、保存时 captured thumbnail、manual Refresh preview、History preview thumbnails、`lastOpenedAt` 最近打开排序和 Load more 已接 local/FastAPI Board contract；owner/admin 可编辑的权限态已进入 UI contract，editor/viewer 灰态等待真实权限源；真实 share/member permission、server-side pagination 和 Alpha 级状态 polish 仍待产品化。
...
| 待补 | server-side pagination、Alpha 级空/错/加载 polish |
...
- 待开发：正式登录/Auth、邮箱验证、server-side pagination、Smart Drawing 浏览器 smoke / 阈值调参、真实 share/member permission、真实 AI Proxy、持久化 AI Run logs、真实 Image Gen / Analysis、右侧 AI Chat planner、预算熔断、完整 i18n。
...
- [ ] `/workspaces` Board gallery/list 支持 server-side pagination。
