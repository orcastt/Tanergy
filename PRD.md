...
**版本**: v0.6
**日期**: 2026-05-01
**状态**: Web 重启方向正式 PRD；S1.5 复杂节点与 Asset LOD Slice A-D 已通过，当前主线是 Slice E Real Asset Pipeline + P0 Product Shell。FastAPI local-dev、真实 `s3-compatible` Asset adapter、Postgres Board / Asset metadata persistence、Web-to-FastAPI switch、staging API package、`/boards` entry、Product Shell route skeleton、Board save UX 第一版、Dashboard metadata first pass 和 Auth scaffold first pass 已落地。
**当前优先级**: P0 最小图像链路已在 canvas spike 中证明可行；当前从“本地可保存/加载 + staging package”进入“AI contract scaffold + Board save regression”阶段。Dashboard metadata 和 Auth scaffold 已有 first pass；真实 staging 基础设施、正式 Auth、邮箱验证和真实 AI 生图仍在外部资源就绪后推进；多人协作继续后置到 P0.5。
...
- `/boards` Dashboard shell 和 `/boards/:boardId` 画布入口已是当前产品化入口的起点；`/spikes/canvas` 仍保留为技术验证入口。
- Product Shell 已按 `reference/Design.md` 新增 `/login`、`/signup`、`/forgot-password`、`/verify-email`、`/workspaces`、`/dashboard` redirect、`/settings` 和 `/account` route skeleton；Auth scaffold first pass 已有 typed session/user/workspace、Next/FastAPI session endpoint、dev request headers 和默认关闭的 Proxy route guard，但这些页面仍使用 mock user/workspace 和本地表单校验，不代表真实 Auth 已完成。
- Dashboard 现在支持 Board summary list、New board、按 board id open、Search、Rename、Delete、Refresh、local/FastAPI persistence label、缩略图占位、shape/asset 对象计数、20 条列表上限、loading skeleton、empty CTA 和 error retry；真实 captured thumbnail、recent/opened metadata 和更完整分页仍待产品化。
...
- `/boards/:boardId` 已有 Board 模式保存状态条、autosave/debounce、dirty warning、load/save error fallback 和刷新/关闭 warning；`/spikes/canvas` 仍保留 `Save audit` / `Save local` / `Load local` 开发控件。
- FastAPI 现在支持 `/health`、CORS allowlist、Asset upload/read、Board validate/list/save/load、`s3-compatible` object storage、Postgres Board persistence 和 Postgres Asset metadata adapter；真实 staging server、managed Postgres、R2 bucket、staging domain 和 secrets 尚未接入。
- P0 真实 AI 调用、Model Registry、AI Run log、真实登录/Auth、右侧 AI Chat 产品化仍未完成。
...
| 4. 页面清单 | `/boards` 与 `/boards/:boardId` 已部分落地；Auth/Workspace/Settings/Account Product Shell route skeleton 已落地；右侧 AI Chat、Image Editor modal、Preview 弹层仍待开发。 |
...
| 8. 不做什么 | 当前边界仍有效：不做桌面、公众号、复杂 Admin、完整模型市场、P0 多人协作。 |
...
| 编号 | 功能 | 说明 | 优先级 | 当前状态 |
|------|------|------|--------|----------|
| F01 | Web 登录 | 用户能登录进入 Dashboard；P0 可先用现有认证或本地开发模式 | P0 | Product Shell route skeleton + Auth scaffold first pass 已落地；`/login` / `/signup` / `/forgot-password` / `/verify-email` 只有 mock 表单校验，真实 Auth/session/email 待开发 |
| F02 | Dashboard / Board 列表 | 创建、打开、重命名 Board | P0 | 部分完成；`/boards` 支持 list / create / open / search / rename / delete / refresh / thumbnail placeholder / shape-asset count / list limit / loading-empty-error first pass，captured thumbnail / recent metadata 待补 |
...
| F05 | Image Gen Node | 接收 text 和 0-N 个 image 输入，生成 1 张 Image；支持模型、分辨率和比例参数 | P0 | mock 数据流已实现，真实 AI Run 待开发 |
| F06 | Image Gen 4 Node | 接收 text 和 0-N 个 image 输入，同一 prompt 调用 4 次，返回 4 张 Image 结果 | P0 | mock 四图链路已实现，真实四图生成待开发 |
| F07 | Analysis Node | 接收 image 和 prompt，内置默认分析 prompt，输出 text，用于反推提示词 | P0 | mock 数据流已实现，真实 Analysis provider path 待开发 |
...
| F13 | API 调用日志 | 记录用户、Board、模型、参数、状态、耗时、费用 | P0 | 待开发；AiRun / ApiCallLog schema 仍是目标模型 |
| F14 | 加载 / 空 / 错误状态 | 所有核心流程有明确状态反馈 | P0 | 部分完成；Dashboard / Board persistence / Asset guard 有基础反馈，Alpha 级状态待补 |
| F15 | 图片模型切换 | Image Gen / Image Gen 4 Node 和 AI Chat 输入区可选择 P0 图片模型 | P0 | mock/前端原型可见，后端 Model Registry 待开发 |
| F16 | i18n 基线 | P0 UI 默认英文，保留中文开发文案能力 | P0 | 待系统化；当前仍有开发态英文硬编码 |
| F17 | 左侧 Inspector | 选中复杂节点后显示模型、参数、运行摘要等详细配置；避免节点卡片变成巨型表单 | P0 | 原型已实现 |
| F18 | Asset / Board Persistence | 图片必须以 Asset 引用进入 Board document；Board 可保存、加载、列表，guard 阻止 `data:` / `blob:` / Base64 | P0 Blocker | 部分完成；local/FastAPI/S3-compatible/Postgres contract 已落地，真实 staging 资源和 save UX 待补 |
...
| Asset / Board persistence，禁止 Base64 入文档 | Canvas Markup + Merge Capture | 简单 Admin 成本视图 | 复杂 Admin Analytics |
| 后端 AI Proxy 和 API Logs | Model Registry 能力 schema | Stripe 测试购买 | 完整模型市场 |
| 安全限流和成本熔断 | 图片压缩/缩略图/懒加载 | P0.5 Presence | 多人实时协作 P0 |

当前 MoSCoW 核对：

- Must 中的 Canvas、Node Runtime、Image Node、Merge Capture、Asset / Board persistence contract 已进入可验证状态。
...
当前状态：`/boards` shell 已可生成 `board_<uuid>` 并跳转 `/boards/:boardId?new=1`，也可输入 board id 打开已有 Board；Search、Rename、Delete 已接 local/FastAPI Board contract；thumbnail、recent/opened time 和 Alpha 级空/错/加载 polish 仍待产品化。
...
当前状态：Prompt / Image Gen / Image Gen 4 / Analysis / Image 五类节点、端口连线、fan-out、input replacement 和 mock 数据流已在 canvas runtime 中通过；真实 AI Run、Model Registry、成本/日志和 provider result 入 Asset 尚未接入。
...
| 作用 | 用户登录或注册 |
| 主要内容 | App Shell、编辑式说明、邮箱输入、mock 密码/验证码校验、错误/成功提示 |
| 空状态 | 无 |
| 成功跳转 | `/boards` |
| 失败反馈 | 输入框错误或 toast |
...
|----|------|
| 路径 | `/boards` |
| 当前状态 | 已有 first-pass product surface；支持 Board list、New board、按 board id Open、Search、Rename、Delete、Refresh、persistence label、thumbnail placeholder、shape/asset count、20 条列表上限、loading skeleton、empty CTA、error retry |
| 作用 | 管理 Board |
| 主要内容 | Board 列表、New Board、搜索、最近打开时间 |
| 空状态 | `No boards yet` + `New Board` |
| 加载状态 | Skeleton board cards |
| 错误状态 | `Could not load boards. Retry` |
| 待补 | Captured thumbnail、recent/opened time、真正分页、Alpha 级空/错/加载 polish |
...
|----|------|
| 路径 | `/boards/:boardId` |
| 当前状态 | 已有 entry shell；复用 canvas runtime，可按 board id auto-load，Board 模式有 autosave/debounce 与保存状态条，同一 persistence contract 可切 local/FastAPI |
| 作用 | 核心画布编辑 |
...
| 当前状态 | 已有节点 Inspector 原型；后续参数项需改走 Model Registry schema |
...
本节是 Alpha-ready 的最终完成定义；当前实现状态以 2.1 表格为准。已经在 spike 中跑通的能力，只有在进入 `/boards` 产品入口、通过 persistence guard、具备错误/加载反馈，并完成对应质量门后，才算该功能 Alpha-ready。
...
- Board 已有 Next local bridge 和 FastAPI persistence contract，记录包含 `workspaceId` / `ownerId`；FastAPI 可用 local file-backed 或 Postgres adapter。
...
- AI Run / API Call Log、AI Chat Session、Model Option 仍是目标 schema，等待 Model Registry / AI Proxy 切片实现。
...
当前核对：本节是 Alpha 目标体验。当前已有 Dashboard loading/error、Board save/load guard 错误、Asset upload/read 错误和 FastAPI CORS / workspace guard 基础反馈；Auth、AI Run、右侧 AI Chat、预算熔断、完整离线/重试体验仍待开发。
...
当前核对：以下边界在 2026-05-01 仍有效。新增的 FastAPI、Postgres、R2/S3 和 deployment work 只服务 P0 Web AI image canvas，不表示恢复旧桌面端、公众号、复杂 Admin 或 P0 多人协作。
...
- 部分通过：Dashboard / Boards、Canvas Editor 产品入口、Board save/load/autosave、状态反馈、图片模型选择器、左侧 Inspector。
- 待开发：正式登录/Auth、邮箱验证、Dashboard metadata polish、Model Registry、AI Proxy、AI Run logs、真实 Image Gen / Analysis、右侧 AI Chat planner、预算熔断、完整 i18n。
...
- [x] `/boards` Dashboard 支持 search / rename / delete。
- [x] Product Shell route skeleton 支持 `/login` / `/signup` / `/forgot-password` / `/verify-email` / `/workspaces` / `/settings` / `/account`，并按 `reference/Design.md` 接入 mock user/workspace。
- [x] `/boards` Dashboard 支持 thumbnail placeholder、shape/asset count、list limit 和 loading/empty/error first pass。
- [ ] `/boards` Dashboard 支持 captured thumbnail / recent opened metadata / richer pagination。
