...
**版本**: v0.6
**日期**: 2026-05-01
**状态**: Web 重启方向正式 PRD；S1.5 复杂节点与 Asset LOD Slice A-D 已通过，当前主线是 Slice E Real Asset Pipeline + P0 Product Shell。FastAPI local-dev、真实 `s3-compatible` Asset adapter、Postgres Board / Asset metadata persistence、Web-to-FastAPI switch、staging API package、`/boards` entry、Product Shell route skeleton 和 Board save UX 第一版已落地。
**当前优先级**: P0 最小图像链路已在 canvas spike 中证明可行；当前从“本地可保存/加载 + staging package”进入“Dashboard metadata + Auth scaffold + AI contract scaffold”阶段。真实 staging 基础设施、正式 Auth、邮箱验证和真实 AI 生图仍在外部资源就绪后推进；多人协作继续后置到 P0.5。
...
- Product Shell 已按 `reference/Design.md` 新增 `/login`、`/signup`、`/forgot-password`、`/verify-email`、`/workspaces`、`/dashboard` redirect、`/settings` 和 `/account` route skeleton；这些页面使用 mock user/workspace 和本地表单校验，不代表真实 Auth 已完成。
- Dashboard 现在支持 Board summary list、New board、按 board id open、Search、Rename、Delete、Refresh 和 local/FastAPI persistence label；缩略图、recent/opened metadata 和完整空/错/加载状态仍待产品化。
...
| 编号 | 功能 | 说明 | 优先级 | 当前状态 |
|------|------|------|--------|----------|
| F01 | Web 登录 | 用户能登录进入 Dashboard；P0 可先用现有认证或本地开发模式 | P0 | Product Shell route skeleton 已落地；`/login` / `/signup` / `/forgot-password` / `/verify-email` 只有 mock 表单校验，真实 Auth/session/email 待开发 |
| F02 | Dashboard / Board 列表 | 创建、打开、重命名 Board | P0 | 部分完成；`/boards` 支持 list / create / open / search / rename / delete / refresh，thumbnail / recent metadata 待补 |
...
|----|------|
| 路径 | `/login` |
| 当前状态 | Product Shell route skeleton 已落地；使用 mock 表单校验，不应视为真实登录 |
| 作用 | 用户登录或注册 |
...
|----|------|
| 路径 | `/boards` |
| 当前状态 | 已有 shell；支持 Board list、New board、按 board id Open、Search、Rename、Delete、Refresh、persistence label |
| 作用 | 管理 Board |
| 主要内容 | Board 列表、New Board、搜索、最近打开时间 |
| 空状态 | `No boards yet` + `New Board` |
| 加载状态 | Skeleton board cards |
| 错误状态 | `Could not load boards. Retry` |
| 待补 | Thumbnail、recent/opened time、Alpha 级空/错/加载 polish |
...
- [ ] `/boards` Dashboard 支持 thumbnail / recent saved time。
