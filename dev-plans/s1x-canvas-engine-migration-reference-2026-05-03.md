...
- 2026-05-04 Phase 4A capture UX debt: Capture selection currently screenshots the live Konva stage by briefly entering capture mode, hiding non-selected shapes/overlays and restoring camera/scale afterward. This can cause a visible one-frame "click/flash" even though data is correct. Follow-up should move PNG capture to an offscreen stage/render tree so the visible canvas does not flicker.
...
| ◐ | 4A.3 | Selection → Image Node | 选择多个对象后 Capture to Image Node | Selection toolbar Capture 已启用：按显式 `selectedIds` bounds 渲染透明 PNG，上传 Asset API `origin=merge_capture`，再在选区下方创建 Image Node；不经 data URL 写入 Board/node state；当前通过 live stage capture mode 隐藏 overlay/非选中物体，会出现轻微闪烁，offscreen capture 是 polish follow-up | 多选图形/图片/节点后生成 Merged selection Image Node，Board JSON 无 raw image payload；后续验收无可见闪烁 |
...
| ◐ | 4A.26 | Image operation menu | 选中图片后，上方图片操作里出现算法操作 | single image selection toolbar 已有 Crop edit first pass：点击 Crop 进入四边裁切控件，浅紫色边框和角点与 resize 控件一致；拖动边缘会裁掉图片并收缩图片边界，不再自动拉伸裁切内容；Remove BG、Object Cutout 仍未接 | `KonvaSelectionToolbar.tsx`, `konvaImageCropCommands.ts`, `KonvaSelectionOverlay.tsx` | 选图片时看到 Crop；拖四边裁切，边框/角点为浅紫色；普通 shape 时隐藏；算法操作仍是后续 |
