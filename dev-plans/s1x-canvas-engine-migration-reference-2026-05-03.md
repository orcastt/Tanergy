...
- Alt/Option drag hardening: duplicate sessions now calculate every preview and final commit from immutable base shapes plus the last real drag point, ignore both source and clone ids for snap, and select clones only after pointerup to avoid intermittent source/clone drag event ordering issues.
- Alt/Option drag snap follow-up: duplicate sessions also ignore source/clone start bounds as snap targets so a slow copy near the original object cannot be pulled back to the source position.
