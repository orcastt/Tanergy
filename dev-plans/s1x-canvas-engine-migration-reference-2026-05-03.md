...
- 2026-05-05 Page UI first pass: Konva canvas now has a compact right-side collapsible Pages drawer with active-page switch, new blank page creation and double-click page rename. Save/autosave/Snapshot/History serialize the current active page back into `pages[]` instead of rebuilding a single default page, and restore/load/history restore the active page plus page list. True rendered page thumbnails, page delete/reorder and right-click Move to page remain follow-ups.
...
Next development focus: hand-test page create/switch/rename/save/load/history restore, then choose true page thumbnails/delete/reorder/Move to page, transparent-background/export polish, precise v1 style/binding migration or Phase 6 Yjs proof.
...
| ◐ | 5.16 | Page / multi-board contract | tldraw 有 pageId；未来需要多页和页面缩略图 | Konva v2 envelope 是兼容式 page contract：保留 `canvasDocument` 作为 active page mirror，同时新增 `activePageId` 和 `pages[]`；restore/load/history 使用 active page，guard/metrics 同时验证/统计 pages；右侧可收起 Pages drawer first pass 已支持切页、新建页、双击重命名，并在 Save/autosave/Snapshot 前把当前页写回 `pages[]`；真实渲染缩略图、page delete/reorder、右键 Move to page 仍未做 | `konvaBoardPageContract.ts`, `konvaBoardDocument.ts`, `useKonvaBoardPages.ts`, `KonvaCanvasPagesPanel.tsx`, `KonvaBoardSaveAudit.tsx`, `boardKonvaDocumentGuard.ts`, `board_konva_guard.py`, `boardTypes.ts`, `board_metadata.py` | 保存的新 Konva Board JSON 含多页 `pages[]`；旧无 pages 的 v2 doc 仍可 restore；切页后各页内容/camera 不串页；后端 tests 覆盖 pages metrics/guard |
