...
**Updated**: 2026-05-14
**Status**: Active historical reference plus regression checklist. Konva v2 formal Board route is now Konva-only in the active web app and the staging deploy; this file remains useful for migration baseline comparisons, while remaining work is export/background polish, performance regressions, real AiRun wiring and Phase 6 Yjs proof.
**Branch**: `feature/s1c-auth-admin-production-boundary`

## Principle

The historical pre-Konva runtime is the behavioral reference. The active web app is now Konva-only; this file exists to preserve migration baseline comparisons, not to describe a live fallback runtime.
...
- Phase 3A right-click menu first batch started: the menu now has real hover submenus for Edit / Arrange / Reorder / Copy as / Export as, edge clamping, Cut, platform-aware shortcuts and multi-selection Align commands. Group/lock/arrange/export commands are wired; Move to page is now live through the Page contract, while transparent-background toggles remain disabled.
...
- 2026-05-05 drag state-machine correction: ordinary drag and Alt/Option copy both use one `KonvaShapeDragSession` preview as the visual source of truth, so the object, selection handles, grouped/multi-selected companions and snap guides move together. Konva's native drag source is locked during the gesture; node/image shapes keep their normal render while moving. Frame drag also suppresses the stale top chrome while moving, preventing the old-position black border from lingering.
...
- 2026-05-05 historical migration checkpoint: the formal Board route briefly used a document-engine detector while Konva became the new route and old v1 data was being inspected.
- 2026-05-05 historical local cleanup checkpoint: local workspace Board storage was cleaned of old tldraw v1 Board records, leaving only Konva v2 Boards.
- 2026-05-13 Konva-only closeout checkpoint: the active web app no longer exposes a tldraw runtime/reference path; `/boards/[boardId]` is Konva-only, `/spikes/konva-canvas` is the remaining regression surface, and legacy v1 Board docs/history are blocked by guard instead of opening a fallback runtime.
...
- 2026-05-05 Page UI first pass: Konva canvas now has a compact right-side collapsible Pages drawer with active-page switch, new blank page creation and double-click page rename. Save/autosave/Snapshot/History serialize the current active page back into `pages[]` instead of rebuilding a single default page, and restore/load/history restore the active page plus page list. Later Page polish added lightweight thumbnails, page delete/reorder and right-click Move to page; true rendered thumbnail assets remain a follow-up.
- 2026-05-05 Board History page-title polish: Konva History entries now use the active Page title instead of repeating the Board title. Snapshot summary on both local Next storage and FastAPI storage can derive the active Page title from existing Konva v2 `pages[]`, so old entries display page context when refreshed.
- 2026-05-05 AI Chat history clean polish: Chat 节点右上角不再显示 `SUCCEEDED` 状态徽标，改为 `Clean`；点击会清空当前 Chat 的对话历史和 exported message outputs，并通过 runtimeGraph reconciliation 回收失效输出端口/边。
- 2026-05-05 Page polish first pass: Pages drawer now has lightweight geometry thumbnails, page delete and page up/down reorder controls. Right-click `Move to page` is wired from the current selection; group members and frame children move as one scope, moved shapes are removed from the source page, appended to the target page, internal runtime edges move with the selected nodes, and cross-page edges are dropped for now.

Next development focus: hand-test page create/switch/rename/delete/reorder/Move to page/save/load/history restore, then choose transparent-background/export polish, precise v1 style/binding migration, real AiRun execution or Phase 6 Yjs proof.
...
| ✅ | 3.3 | 拖拽 | 选中对象拖动丝滑 | normal drag / Alt-copy 都走 clean drag session；拖动期间统一由 preview shapes 驱动物体、controls、snap guides 和多选/组内其它对象，Konva native source 锁在原位避免双状态竞争；node/image 拖动时保持正常渲染，不切换成预览卡片；非 Select 工具只允许点选编辑、不允许直接拖动物体，避免连续绘制误移动 | `useKonvaShapeDragHandlers.ts`, `konvaDragSession.ts` | 拖动无跳变，节点/图片/shape 与缩放/旋转控件同帧移动，连续绘制时不会误拖 |
| ◐ | 3.4 | resize | 图形和 node card 可 resize | shape/image/text/sticky/frame resize 已有；bbox 边线可作为 n/e/s/w resize handle 拉长单侧，corner 仍可缩放；single 和 multi rotated resize first pass 已修；Text resize 会同步缩放 fontSize；node card resize 未做 | `KonvaSelectionOverlay.tsx`, `konvaSelectionUtils.ts`, `konvaRotatedResize.ts`, `konvaOrientedBounds.ts` | resize 后内容不坏；rectangle/sticky/frame 可拉边线延伸单侧 |
...
| ✅ | 3.11 | text edit | text/note 可输入，不抢画布快捷键 | HTML textarea overlay；text/sticky/frame/shape label 可编辑；输入中阻止画布快捷键和 Cmd/Ctrl+S 浏览器保存；standalone Text 编辑提交和 fontSize 属性修改后自动按内容撑高文本框 | `KonvaTextEditor.tsx`, `konvaShapeCommands.ts`, `konvaTextAutoFit.ts` | 输入中 Cmd+S 不误触，中文输入正常，长文字不会留在过小 text box 内 |
...
| ✅ | 3B.19 | Text tool | `T` 工具插入文字 | Text tool one-shot，HTML textarea overlay 编辑，shape label/sticky/frame 也复用 overlay；standalone Text resize 会缩放字体，编辑/fontSize 修改会 auto-fit 文本框高度 | 双击/输入/中文 IME 正常，长文字/字号变化后框不需要手动再拉 |
| ✅ | 3B.20 | Shape handles | 选中形状显示蓝色 bbox/handles | handles 尺寸随 zoom 稳定；bbox 四条边有隐形 n/e/s/w resize hit area，可单边延伸 rectangle/sticky/frame 等 box-like 对象；单个和 group/multi rotated selection handles 跟随角度；V/Select 工具下点住对象可直接拖动，不需要先单击选中；非 Draw 的连续绘制工具点对象会进入选择/编辑，已选对象可拖动，空白处继续绘制；拖动中的物体和 controls 共用 drag preview shapes，避免一方 native、一方 React preview 的滞后；locked 对象/组显示线稿锁 icon | zoom 后 handle 不巨大/过小，旋转后拖拽复制控件跟随角度，普通拖动时控件不滞后，V 下未预选对象可直接拖动，绘制工具下点对象可临时编辑，拉边线可单侧 resize |
...
当前第一批实现原则：菜单结构、可用/不可用状态和共享命令已经跑通。Group/Lock 使用轻量 shape 字段承载；Move to page 已接 page contract first pass，透明背景 toggle 仍保持 disabled。Phase 4A 已接入 selectedIds capture/export：PNG 走 Konva transparent capture，SVG 走有限 serializer contract。
...
| ◐ | 4.2 | 节点 UI | HTMLContainer 内 React 控件 | Konva lightweight card first pass 已改为简洁卡片：title/status、prompt/text box、Image Gen/Image Gen 4 模型/比例/分辨率 dropdown、Image preview；Chat 节点右上角用 `Clean` 清空对话历史，替代 `SUCCEEDED` 状态徽标；Chat 输入上方显示 connected prompt/image chips，也显示本地上传/粘贴到 Chat 的 image/PDF reference chips；Chat textarea 编辑中有 DOM Send 按钮和 Cmd/Ctrl+Enter，发送当前 textarea 内容不依赖 blur；Chat node data 只保留 capped short mock history，避免把完整长日志写进 Board 文档；dropdown 已移到 card body 最上层，避免被 output preview/warning 遮挡；Run/dropdown controls stop `dblclick` bubbling so they do not accidentally open text editing；Image Gen/Image Gen 4 output container 会随节点 resize 高度自适应拉伸；Prompt/Analysis 文本框非编辑态在节点自身 Konva group 内 clip 并显示本地 scrollbar，长文本不会溢出节点，也不会以全局 DOM 层盖住后画的图形；double-click 时打开同尺寸 HTML textarea 编辑层；port label 不再显示在节点内部，hover 才显示 text/image tooltip；`node_card` 的 rotate/flip 能力已集中到 capability 表；正式 React/HTML controls 只在编辑/运行态打开的策略未集成 | `NodeCardContent.tsx`, `KonvaNodeCardShape.tsx`, `KonvaNodeCardParts.tsx`, `KonvaNodeImagePreview.tsx`, `KonvaNodeTextEditor.tsx`, `KonvaSelectionOverlay.tsx`, `KonvaPropertiesSelectionActions.tsx`, `shapeCapabilities.ts`, `konvaShapeCapabilities.ts` | 节点视觉干净；Chat 有 Clean 而不是 succeeded 徽标；Chat 连接/上传/粘贴 prompt/image/PDF 后引用 chip 可见且窄节点会折叠；编辑中直接 Send 不丢最新输入；Run/Stop 不误开 Analysis preset editor；Gen 节点拉高后输出容器跟随放大；长 Prompt/Analysis 不撑破容器且可在节点内滚动；后画的线/shape 能按 z-order 盖过节点；模型/比例/分辨率可 first-pass 下拉选择且不被预览层挡住；Prompt/Analysis 可输入；节点不能旋转/flip |
...
| ✅ | 5.13 | Konva-only runtime closeout | active web app 只支持 Konva | 活跃 Web app 已不再保留 tldraw route 或 engine fallback；`/boards/[boardId]` 只加载 Konva v2，legacy v1/unknown docs 进入 unsupported/guard path，`/spikes/konva-canvas` 是唯一保留的 regression surface | `boardCanvasEngine.ts`, `boardDocumentGuard.ts`, `boards/[boardId]/page.tsx`, `share/[shareId]/page.tsx` | active app 不会打开 legacy tldraw Board；workspace 本地旧 v1 数据已清理；restore/list path 会阻止 legacy 文档 |
...
| ◐ | 5.16 | Page / multi-board contract | tldraw 有 pageId；未来需要多页和页面缩略图 | Konva v2 envelope 是兼容式 page contract：保留 `canvasDocument` 作为 active page mirror，同时新增 `activePageId` 和 `pages[]`；restore/load/history 使用 active page，guard/metrics 同时验证/统计 pages；右侧可收起 Pages drawer 支持切页、新建页、双击重命名、轻量几何缩略图、page delete、page up/down reorder，并在 Save/autosave/Snapshot 前把当前页写回 `pages[]`；右键 Move to page 可把当前 selection 移到另一个 page，group 成员和 frame children 跟随迁移，内部 runtime edges 随选中节点一起迁移，跨页 edge 暂时断开；真实渲染缩略图资产、page duplicate、Move selection to new page 和 page-scoped collaboration 仍未做 | `konvaBoardPageContract.ts`, `konvaBoardDocument.ts`, `useKonvaBoardPages.ts`, `konvaBoardPageActions.ts`, `KonvaCanvasPagesPanel.tsx`, `KonvaContextMenu.tsx`, `KonvaBoardSaveAudit.tsx`, `boardKonvaDocumentGuard.ts`, `board_konva_guard.py`, `boardTypes.ts`, `board_metadata.py` | 保存的新 Konva Board JSON 含多页 `pages[]`；旧无 pages 的 v2 doc 仍可 restore；切页/删除/重排后各页内容/camera 不串页；右键 Move to page 后选中对象/组/frame 内容只出现在目标页，内部 runtime edges 保留，跨页 edges 不保留；后端 tests 覆盖 pages metrics/guard |
