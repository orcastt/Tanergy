...
**Status**: Active tactical plan; Phase 4 Node/Port/Edge/Image Node UX first pass is in progress after accepted handfeel/properties/object-editing baselines.
...
Next development focus: hand-test Phase 4 node menu/card/edge/image-node upload/text-edit/fan-out behavior, then do a Phase 4 runtimeGraph cleanup before Konva `resolveNodeInputs` adapter, extensible mock Run flow, generated asset propagation, richer node editors and Phase 4A selection capture/export.
...
| ◐ | 1A.9 | node HTML | 大量 React 节点重渲染卡 | 非编辑态主体仍是轻量 Konva visual；Prompt/Analysis 长文本预览用节点内 Konva clip/scrollbar，不挂全局 DOM overlay；编辑态才挂 HTML textarea；完整 node controls 编辑/运行态挂载策略未做 | 100 node cards 可拖动，长文本不撑破节点 |
...
| ◐ | 4.2 | 节点 UI | HTMLContainer 内 React 控件 | Konva lightweight card first pass 已改为简洁卡片：title/status、prompt/text box、Image Gen/Image Gen 4 模型/比例/分辨率 dropdown、Image preview；dropdown 已移到 card body 最上层，避免被 output preview/warning 遮挡；Prompt/Analysis 文本框非编辑态在节点自身 Konva group 内 clip 并显示本地 scrollbar，长文本不会溢出节点，也不会以全局 DOM 层盖住后画的图形；double-click 时打开同尺寸 HTML textarea 编辑层；port label 不再显示在节点内部，hover 才显示 text/image tooltip；`node_card` 的 rotate/flip 能力已集中到 capability 表；正式 React/HTML controls 只在编辑/运行态打开的策略未集成 | `NodeCardContent.tsx`, `KonvaNodeCardShape.tsx`, `KonvaNodeCardParts.tsx`, `KonvaNodeTextEditor.tsx`, `KonvaSelectionOverlay.tsx`, `KonvaPropertiesSelectionActions.tsx`, `shapeCapabilities.ts`, `konvaShapeCapabilities.ts` | 节点视觉干净；长 Prompt/Analysis 不撑破容器且可在节点内滚动；后画的线/shape 能按 z-order 盖过节点；模型/比例/分辨率可 first-pass 下拉选择且不被预览层挡住；Prompt/Analysis 可输入；节点不能旋转/flip |
| ◐ | 4.3 | node data | props.data 保存 prompt/model/resolution 等 | 新建节点写入 registry default prompt/model/resolution/image refs；Prompt/Analysis textarea 编辑会写回 `props.data.prompt` / `props.data.analysisPrompt`；Image Node upload/upstream mirror 会写 asset refs；Board save/load 迁移和完整 node editor 表单尚未接 | `types/nodeRuntime.ts`, `konvaNodeCardFactory.ts`, `konvaImageNodeConversion.ts`, `useKonvaImageNodeUpload.tsx`, `useKonvaNodeCreationMenu.ts` | 保存恢复不丢参数 |
| ◐ | 4.4 | runtime summary | status/cost/error/resultAssetIds | `runtimeSummary` 已在 node card 可视化展示；Analysis/Image Gen/Image Gen 4 右上角有 Run/Stop first pass，点击会切换 running/idle；真实 AiRun status/cost/resultAssetIds 尚未接 Konva renderer | `NodeCardShape.tsx`, `KonvaNodeCardShape.tsx`, `useKonvaNodeCreationMenu.ts` | Run 变 Stop，后续接真实 upstream dataflow |
| ✅ | 4.5 | ports | typed text/image input/output dot | registry-derived ports render larger hit targets; output port drag shows preview and commits to compatible input port with zoom-aware hit radius；port snap 半径已放大，靠近 compatible input 时 preview endpoint 会吸到端口中心并显示 solid line + target halo；hover tooltip 用黑底白字显示 `text` / `image`；drawing tools 下 node/image body pass-through 但端口仍保持可拖动；runtime edge layer 放在 node card 下方，避免已有 edge 抢 source output port 命中 | `KonvaNodeCardShape.tsx`, `konvaNodePorts.ts`, `useKonvaNodeConnectionSession.ts`, `KonvaCanvasShape.tsx`, `KonvaCanvasStage.tsx`, `KonvaNodeEdgeLayer.tsx` | 拖线更容易吸附端口，吸附反馈明显；同一 output 连完一根后还能继续连其他 input |
| ✅ | 4.6 | edge store | runtimeEdges 独立保存 | `CanvasDocument.runtimeEdges` is separate from visual arrow/line; output 可一对多 fan-out，input 单端口只保留一个上游；EdgeLayer renders dataflow curves below node cards while selected edge still shows near-input disconnect; undo/redo includes edges and deleting nodes cleans connected edges | `types.ts`, `konvaRuntimeEdges.ts`, `KonvaNodeEdgeLayer.tsx`, `KonvaCanvasStage.tsx` | edge 不依赖视觉箭头存在；同一 input 重连会替换旧 edge；同一 output 可连多个下游 input |
| ◐ | 4.7 | input resolution | 通过 edges 找上游文本/图片 | Image Node 从上游 Image Node asset first-pass mirror；Image Gen/Image Gen 4 的 image input count 会随 image edges 增减；Run/Stop 目前只切 `runtimeSummary.status`，完整 `resolveNodeInputs` Konva query adapter、mock Run asset 生成和 resultAssetIds 下游传播仍未做 | `nodeDataFlow.ts`, `konvaRuntimeEdges.ts`, `useKonvaNodeCreationMenu.ts` | image→image preview 正常；prompt→gen、image→analysis runtime adapter 和 mock generated asset propagation 后续 |
...
| ◐ | 4.10 | edge delete/cut | 选中 edge 可断开 | 删除节点时 connected runtime edges cleanup 已接；edge hit/select first pass 已接，选中 edge 会在靠近 input 端显示 `-` 断链按钮；键盘 Delete/Cut edge 尚未做 | `KonvaNodeEdgeLayer.tsx`, `konvaRuntimeEdges.ts` | 点击 edge 后可点击 `-` 断开，input count 同步 |
...
| ◐ | 4A.7 | Image Node output → downstream node | Image Node 通过 `image_out` port 给 Analysis/Image Gen | runtime edge 可连接到下游 image input，Image Node → Image Node preview mirror first pass 已做；Analysis/Image Gen 的 Run input adapter 未做 | `getNodeOutput`, `resolveNodeInputs`, `konvaRuntimeEdges.ts` | Image Node 连 Image Node 可显示；连 Analysis/Gen 后 Run 可用待补 |
...
| ◐ | 4A.10 | Paste/Drop image → Canvas/Image Node | 当前 tldraw/asset layer 可接图片资产 | paste image → Canvas image 已支持并走 Asset API；drop image → Image Node 已支持；paste 直接进 Image Node 的定向行为未做 | `konvaImageClipboard.ts`, `useKonvaImageNodeUpload.tsx`, asset upload client | 粘贴/拖入图片后走 R2 asset，不进 Board document Base64 |
...
| ☐ | 4A.26 | Image operation menu | 选中图片后，上方图片操作里出现算法操作 | selection toolbar 增加 Image Ops group：Remove BG、Object Cutout；只在单张 image / Image Node asset selection 时启用 | `KonvaSelectionToolbar.tsx`, `useKonvaImageNodeActions.ts` | 选图片时看到操作，选普通 shape 时隐藏/禁用 |
| ☐ | 4A.27 | Remove Background / rembg | 一键抠背景，生成透明 PNG | Server-side `rembg` first pass；输入 asset id/original URL，输出透明 PNG 上传 R2，origin=`background_removal`；新 image shape 放到源图右下方 offset 约 24px 并选中 | FastAPI image ops service, `konvaImageClipboard.ts`, asset upload API | 点击 Remove BG 后生成新透明图片，不覆盖原图，不写 Base64 |
| ☐ | 4A.28 | Object Cutout / SAM | 点对象或框选对象后抠出目标 | Server-side `facebookresearch/segment-anything` first pass；用户在选中 image 内点选/框选对象，服务端返回 mask/透明 PNG；结果放到源图右下方并选中 | FastAPI image ops service, future image-point interaction | 点人/物体后抠出该对象，背景透明，支持撤销 |
| ☐ | 4A.29 | Cutout placement and history | 抠图结果像 copy/paste 一样出现在附近 | 新图继承源图显示尺寸/比例，默认 `x + 24, y + 24`；创建前写 history checkpoint；undo 删除新 shape 但不删除 R2 asset | engine history, image shape commands | 结果位置稳定，undo/redo 正常 |
| ☐ | 4A.30 | Cutout storage guard | 算法结果不能污染 Board document | Board doc 只保存 asset refs、尺寸、origin/source metadata；临时 input/output 文件在服务端处理，禁止 data/blob/base64/provider raw payload 写入 node props/history | Board guard, Asset API, FastAPI temp storage | 保存后的 Board JSON 无 data URL/base64 |
| ☐ | 4A.31 | Cutout license guard | 避免再次踩 SDK/model 授权坑 | 当前候选：`rembg` MIT；`facebookresearch/segment-anything` model/code Apache-2.0。实现前锁定具体 package/model checkpoint 版本，并在 docs 记录 license/source URL | dependency review | CI/release notes 能看到算法依赖和 license |
