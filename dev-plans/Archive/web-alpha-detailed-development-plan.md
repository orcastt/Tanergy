...
1. **现在开发到哪里**：项目已从旧桌面/公众号路线切换到全新 Web AI 图像画布；S1 / S1.5 / Asset LOD Slice A-D 已落地，Slice D 跨平台质量门 pass with notes。
2. **我下一步做什么**：进入 Slice E Real Asset Pipeline；临时 diagnostics 已默认关闭，Cloudflare Tunnel 只保留为本地测试支架；之后再做真实模型、生图、保存、登录、AI Chat 收口。
...
| S1.5 技术裁决门 | ✅ 五类节点、Node Runtime、动态端口、runtime edges、Inspector、mock data flow、Merge Capture 本地闭环已通过 |
| Asset LOD Slice A-D | ✅ Image Node LOD、Node LOD、local thumbnail resolver、ordinary canvas image LOD、Windows pass with notes |

### 2.2 当前未开始

| 项 | 状态 |
|----|------|
| 临时测试支架清理 | ✅ `CanvasRuntimeDiagnostics` 默认关闭，仅 `NEXT_PUBLIC_CANVAS_RUNTIME_DIAGNOSTICS=1` 启用；Cloudflare Tunnel allowlist 仅通过 `NEXT_ALLOWED_DEV_ORIGINS` 临时注入 |
| Slice E Real Asset Pipeline | ⏳ 后端上传、对象存储、多尺寸缩略图、Asset metadata、权限 URL |
| Model Registry | ⏳ 未实现 |
| 后端 AI proxy | ⏳ 新 Web 路线未实现 |
| Dashboard / 保存 / 登录 | ⏳ 未实现 |
| AI Chat 自动搭线 | ⏳ P0 保留，尚未产品化 |
...
| 图片粘贴限制 | 当前限制 PNG/JPEG/WebP，单图 30MB，长边自适应降采样，后续仍需测 Pinterest 多图粘贴 |
| 质量检查 | `npm -C apps/web run lint`、`npm -C apps/web run typecheck`、`npm -C apps/web run build`、`git diff --check` 已通过 |
| 状态 | S1 / S1.5 / Asset LOD Slice A-D 已通过；跨平台门 pass with notes，下一步 Slice E |
...
| 断连交互 | ✅ 点击 runtime edge 后选中线，并在 target 端附近显示 `−` 删除按钮 |
...
| 图片密集压力 | ✅ 已完成多轮 Mac / Windows 手测；Windows 遗留卡顿归档为 non-blocking follow-up |
...
当前下一步：**Slice E Real Asset Pipeline**。S1 / S1.5 / Asset LOD Slice A-D 已通过；Slice D 跨平台门按 `pass with notes` 收口。Windows 密集画布卡顿记录为 non-blocking performance follow-up，不阻塞继续开发。

执行顺序：

1. 设计 Slice E Asset model：original URL、256/512/1024 thumbnails、mime、byte size、dimensions、origin、owner/workspace。
2. 实现后端 upload endpoint 与对象存储路径策略。
3. 让 Image Node、ordinary canvas image、Merge Capture、Screenshot、Convert to Image Node 都走 server-backed Asset。
4. 保存 Board 前拒绝或迁移 `data:` / `blob:` 图片引用。
5. Asset Pipeline 稳定后，再进入 Model Registry / AI Proxy / AI Run log。

你现在只需要准备：

- 对象存储偏好（Cloudflare R2 / S3 / MinIO dev）。
- Alpha 阶段是否需要登录后才能上传图片。
- 真实 AI 图片的目标尺寸、预算和默认模型。
