...
- `api-staging.tanergy.cc` 已在重建后的 Hetzner 主机恢复，当前数据库事实源已经切到 Supabase Pro staging。
- Neon 因 data transfer quota 暂停后只保留为历史事故记录；不要再围绕 Neon 或 Hetzner 本机 Postgres 扩展 staging。
- Hetzner 临时 `staging-postgres` 容器和 volume 已删除。
- Cloudflare R2 `tanergy-assets` 已清空到 0 objects；接下来要重新跑 clean asset smoke。
- 后端协同持久化默认是 `final_snapshot`：画布过程更新只走 WebSocket room，Postgres 只保存 compact/final snapshot。
...
- 真实 Clerk session/admin smoke 和第一轮 signed-in board/browser 已转绿；剩余 staging 闸门是 R2 clean asset smoke、第二轮 board/browser 边界、Google/email 和一条 live AI smoke。
...
- 不要让 `apps/web/.env.local`、`apps/web/.vercel/.env.production.local` 这类嵌套本地 env/pull 副本参与 staging 构建；Web 运行时真相只认 Vercel project env。
...
改 Supabase 凭据          -> 改 API env + 重发 API
...
当前方向已经收口：域名接 Cloudflare，前端用 Vercel，后端用 Hetzner VPS，数据库用 Supabase Pro Postgres，图片继续用 Cloudflare R2，登录用 Clerk + Google OAuth。这个组合把数据库事实源固定下来，同时避免把图片对象迁到 Supabase Storage 或 Hetzner Object Storage。
...
3. 创建 Organization / Project，当前 staging 使用 Pro plan。
4. Region 选靠近后端服务器的位置。
   - 如果 Hetzner 选 Hillsboro，美西优先选 Oregon / US West 一类区域。
5. 创建数据库。
6. 复制 database connection string，至少保存 direct/session pooler 两类连接。
...
- Supabase 这类托管 Postgres 运行时优先使用 pooled URL，避免把迁移连接方式直接带进线上运行。
- 不导入 Neon 或 Hetzner local Postgres 的旧数据；当前 staging 从空库跑 Alembic 到 head。
...
不要把旧的 key 状态当成事实。每次迁移或轮换后，只用本地 `.env`、Vercel env、Hetzner server-local `api.env` 和服务商 dashboard 作为实际来源；文档只记录变量名和用途，不记录真实值，也不记录旧值。
...
- Clerk / Supabase / R2 任一服务轮换后，都要同步本地私密记录、Vercel env 和 Hetzner `~/apps/shared/staging/api.env`，不能让旧 key 留在嵌套 env 或历史文档里继续影响部署。
...
DATABASE_URL=<Supabase direct or session pooler connection string>
DATABASE_POOL_URL=<Supabase session pooler runtime connection string>
TANGENT_BOARD_REALTIME_PERSIST_MODE=final_snapshot
...
```text
Running upgrade ... <current alembic head>
```

如果这是一个一次性 disposable smoke 数据库，也可以跑 S1A 完整迁移 smoke：
...
注意：这个 smoke 会删除 `tangent_%` tables 和 `alembic_version`。不要在生产库、Supabase 正式 staging 库或任何有重要数据的库上跑。
...
如果 Supabase/Postgres 环境没有 `gen_random_uuid()`，先启用：
