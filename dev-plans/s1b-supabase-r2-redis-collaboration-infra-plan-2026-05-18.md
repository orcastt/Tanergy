...
**日期**：2026-05-18
**状态**：Decision accepted / Supabase Pro 干净重建待执行
...
协同临时层: Redis, staging 先和 API 同机 Docker 跑
...
- Redis 可以先不用注册新 SaaS，staging 先跑在 Hetzner API 同机。

## 需要新注册或开通什么

必须：

1. Supabase 账号 / Organization。
2. Supabase Pro plan。
3. Supabase staging project，区域选美国西部或最接近 Hetzner US West 的美国区域。
4. Supabase database password、direct/session pooler 连接串，写入私密 secret store，不写入文档。
5. R2 bucket 清理权限，用来删除当前 staging 旧对象；若以后有 production，则 staging/production 必须分 bucket 或分 prefix。

现在不必须：

1. Supabase Auth：暂时不需要，Clerk 继续用。
2. Supabase Storage：暂时不需要，R2 继续用。
3. Managed Redis：staging 可以先 Docker Redis 同机跑，后面多实例或生产稳定性需要时再换 Upstash / Redis Cloud / Supabase 生态外的托管 Redis。
...
| 区域 | 当前状态 | 目标状态 | 要改什么 |
| --- | --- | --- | --- |
| Database | 临时 Hetzner local Postgres 或旧 Neon | 全新 Supabase Pro Postgres, US West 附近 | 不迁旧数据；跑 Alembic 到 head，重新 bootstrap admin/workspace/test board，更新 API env，保留 `DATABASE_URL` direct + `DATABASE_POOL_URL` pooler |
| Realtime document persistence | Yjs/update chain debounce 后写 Postgres | WebSocket/Redis 广播过程，Postgres 只保存最终快照 / compacted state | 拆分 broadcast 和 persistence，增加 Redis backplane，降低写库频率 |
...
4. Yjs/update chain 不再以 0.25s 高频写 Postgres 作为主策略。
5. Postgres 写入改为 compacted/final snapshot：
   - operation-end；
   - room idle；
   - room empty；
   - fixed low-frequency autosave；
   - important structural event。
6. Redis 里的 lock / presence / draft 都必须有 TTL，断线后自动释放。

需要新增或调整的后端测试：

1. awareness-state 不触发 Postgres write。
2. draft preview 不触发 Postgres write。
3. 高频 yjs-update 不按每条写库，只更新 room state / Redis state。
4. operation-end 或 compact publish 后才写 final snapshot。
5. room empty 时 finalize flush。
6. Redis unavailable 时，单实例 memory fallback 可用，但要打日志。
7. guest/viewer 仍不能 document write。
...
1. 在 Hetzner API host 上备份当前 server-local `api.env`。
2. 停止把 staging API 指向 Hetzner 本机 Postgres；不再创建新的 server-local staging DB。
3. 更新 `DATABASE_URL` / `DATABASE_POOL_URL` 到全新 Supabase staging project。
4. R2 env 保持服务商不变，但在切换前清空当前 staging 旧对象或 staging prefix。
5. Docker Compose 加 Redis service：
...
7. Caddy / reverse proxy 需要继续支持 WebSocket upgrade 到 FastAPI。
8. API redeploy 后跑 health、migration、board、asset、realtime smoke。
...
1. `deploy/staging/deployment-secrets.local.md` 只记录需要的 key 名称和状态，不写值。
2. `deploy/staging/README.md` 从 Neon/current local Postgres 改成 Supabase Postgres truth。
3. `project_state/project_state_slice_S1B_staging_infra.md` 记录 Hetzner 本机 Postgres 为历史 fallback，不再作为可选 staging 路线。
4. `ARCH/ARCH_slice_S1B_staging_infra.md` 在执行完成后更新架构 truth：managed Supabase Postgres + R2 clean bucket。
...
1. Hetzner local Postgres 不再作为事实源，也不再作为后续 fallback 路线。
2. 可以删除 `staging-postgres` 容器和 volume；如果想保留事故追溯，最多短期保留只读 volume 名称记录。
3. 文档里把 staging database truth 改为 Supabase Pro。
4. `project_state` 记录 Neon quota -> Hetzner fallback -> Supabase clean rebuild 的最终状态。
5. 后续所有 staging migration 只跑到 Supabase staging project。
...
1. 记录当前 API env 版本，不把 secret 写进文档。
2. 停 API，避免继续写 Hetzner local Postgres。
3. 记录当前 Alembic head。
4. 记录 R2 bucket 和 S3 endpoint 名称，只记录名称，不记录 key。
5. 确认当前 staging 数据允许丢弃。

### Phase 1：创建 Supabase staging project

1. 创建 Supabase Organization / Pro。
2. 创建 staging project。
3. 区域选择美国西部或最接近 Hetzner US West 的美国区域。
4. 保存两类连接串到私密 secret store：
   - direct connection URL：Alembic / admin 用；
   - pooler URL：API runtime 用。
5. 创建 API 专用 DB role；不要让 runtime 用 owner 权限。
6. 不导入旧 Neon / Hetzner local Postgres 数据。

### Phase 2：创建 schema 和最小测试数据

执行原则：

1. 对 Supabase staging 跑 Alembic `upgrade head`。
2. 重新登录/创建测试用户。
3. 重新 bootstrap admin role。
4. 重新创建 workspace / board / history smoke 数据。
5. 不迁旧 asset metadata。
...
- `DATABASE_URL` 保留给 Alembic/admin。
- R2 env 不改，除非你选择新建 staging bucket；当前默认只清旧对象。

### Phase 3.5：清理 R2

1. 如果 R2 bucket 只用于 staging，清空 bucket。
2. 如果 R2 bucket 可能混有 production，只清 staging prefix。
...
1. 购买/启用 Supabase Pro。
2. 创建 Supabase staging project，区域靠近 Hetzner US West。
3. 不导入旧数据；在 Supabase 空库跑 Alembic 到 head。
4. API env 切到 Supabase pooled runtime URL。
5. R2 服务商不动，但清空当前 staging 旧对象或 staging prefix。
6. 重新创建测试用户、admin role、workspace、board 和一张测试图片。
7. staging smoke 全部跑通。
8. 再做 WebSocket/Redis 协同广播改造，数据库只落最终 snapshot。
