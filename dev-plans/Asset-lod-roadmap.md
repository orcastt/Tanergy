...
2026-05-01 Board entry implementation note:

Codex added a minimal product entry for Board persistence. The original implementation used `/boards` as a dashboard shell that listed Board summaries from the same local or FastAPI persistence contract, created a new board id and opened existing boards by id; `/boards/:boardId` renders the existing canvas with seed data disabled and saves/loads through that board id. This has since been refined so the user-facing list lives at `/workspaces`, while `/boards` redirects there and `/boards/:boardId` remains the canvas detail route. FastAPI and the Next local bridge support Board list responses that return summaries only, preserving the save/load rule that full documents are only returned by explicit load. Tests cover local-dev and Postgres board listing plus workspace isolation.

2026-05-01 Dashboard CRUD continuation note:

Codex extended the Board contract with rename/delete. FastAPI now exposes `PATCH /api/v1/boards/{board_id}` and `DELETE /api/v1/boards/{board_id}` for both local-dev and Postgres drivers; the Next local bridge exposes matching `local-rename` / `local-delete` routes. The current `/workspaces` Board gallery/list has client-side search, sort, inline rename and delete confirmation while still keeping list responses summary-only. `/boards/:boardId` also has Board-mode autosave, save indicator, dirty warning, load/save error state, manual Refresh preview and History preview thumbnails. Tests cover local-dev and fake Postgres rename/delete plus workspace isolation. Refresh preview / Workspace card / History thumbnail browser smoke has passed. Remaining gaps after later local work: auth-required mode and real staging credentials.

2026-05-01 Workspace Board metadata first-pass note:

Codex added lightweight Board summary metadata to the local/FastAPI persistence contract. Board summaries now include `shapeCount`, `assetCount` and reserved `thumbnailUrl`; FastAPI local-dev writes these fields, and the Postgres `tangent_boards` schema now stores `shape_count`, `asset_count` and `thumbnail_url` while preserving the rule that list/save responses do not include full `document`. `/workspaces` now shows deterministic thumbnail placeholders, object counts, loading skeleton, empty CTA and error retry in the `reference/Design.md` / `reference/Design_reference.md` product shell style. Later local work added save-time captured thumbnails, manual Refresh preview and History preview thumbnails; remaining Board entry polish is server-side pagination and long-form browser regression.

2026-05-01 Database scaffold / recent-open continuation note:

Codex added `dev-plans/p0-database-schema-roadmap-2026-05-01.md` plus an Alembic scaffold under `services/api/`. The first migration creates the P0 core schema for users, workspaces, memberships, email/session auth, boards, assets, model options, AI runs and API call logs. The staging Docker image now includes migration files and the staging runbook runs `alembic upgrade head`; local-dev adapter auto-create remains available for smoke work. Board summaries now include `lastOpenedAt`. Loading a Board through Next local bridge, FastAPI local-dev or FastAPI Postgres stamps recent-open metadata without returning documents from list/save responses; `/workspaces` can sort by Recently opened / Recently saved and renders an initial Board page with Load more. Refresh preview / Workspace card / History thumbnail browser smoke has passed. Remaining Board entry polish after S0I first pass: server-side pagination and real staging credentials.

2026-05-01 Board History continuation note:

Codex added Board History as a local production-readiness layer that works with autosave instead of competing with it. `/boards/:boardId` now exposes Snapshot and History controls; autosave writes `autosave` history entries after successful Board saves, Cmd/Ctrl+S saves the current Board and writes a `keyboard` entry, and the Snapshot button writes a `manual` entry. Next local bridge and FastAPI both support history create/list/load through the existing snapshot-named routes; FastAPI local-dev and Postgres drivers store guarded history documents, while list responses remain summary-only. The Alembic core migration now includes `tangent_board_snapshots` with `retention_tier` / `expires_at` fields for future subscription tiers. P0 retention defaults to latest 100 history entries per Board via `TANGENT_FREE_BOARD_SNAPSHOT_LIMIT`; Pro/Enterprise long-history policy and object-storage history bodies remain future work.

2026-05-01 local product shell coordination note:

Checkpoint `47d6d0f checkpoint: board management and arch progress map` is the previous committed baseline for the current local phase; the next checkpoint captures the context-index split. Product Shell, App Shell 5-tab navigation, Landing page / Collection / Account / Settings / Team / Subscription semantic shells, Board save UX, Board History, Workspace Board metadata, captured Board thumbnail, manual Refresh preview, Smart Drawing, Auth scaffold and AI contract scaffold now all have first-pass local checkpoints, but they are not product-complete. Refresh preview / Workspace card / History thumbnail browser smoke has passed. The next local work is not "full Auth" yet; continue with Board save/History browser regression, Smart Drawing browser tuning, or external-resource staging wiring. Real email, OAuth, team permissions, subscription billing, R2/Postgres credentials, public domains and AI provider calls stay behind the external-resource gate.

2026-05-01 Product Shell implementation note:

Codex added `reference/Design.md` as the active Product Shell design reference and switched the route-shell work away from old `reference/design-system.md` / `reference/theme.ts`. The Web app now has `components/app-shell/AppShell.tsx`, mock session data, `/login`, `/signup`, `/forgot-password`, `/verify-email`, `/home`, `/workspaces`, `/collections`, `/dashboard` redirect, `/settings`, `/account`, `/team` and `/billing`; `/boards` list entry redirects back to `/workspaces`, while `/boards/:boardId` remains full-screen for tldraw. Auth pages perform only local form validation and do not create real sessions or send email.

2026-05-01 Stitch reference consolidation note:

User moved the old active reference files to legacy and refreshed `reference/stitch_canvas_reference/`. Codex added `reference/Design_reference.md` as the extraction layer for every Stitch `code.html`, updated `reference/Design.md` as the canonical Quiet Editorial implementation spec, and completed the first page-development pass by moving `/login`, `/signup`, `/forgot-password` and `/verify-email` to full-screen split-screen auth surfaces. These pages still use mock local validation only; real Auth/email/OAuth remains behind the external-resource gate.

2026-05-01 Workspaces Board gallery/list note:

Codex corrected the `/workspaces` interpretation after user feedback: `workspaces_vibrant` maps to a Board gallery/list inside the active workspace, not a mixed Landing page / Collection / Settings / Account / Team / Subscription page. `/workspaces` now reads the same Board persistence list as the old `/boards` dashboard, supports Gallery/List view, search, sort, New board, Open, Rename, Copy board and Delete. App Shell, `/`, `/dashboard` and `/boards` list entry now route back to `/workspaces`; `/boards/:boardId` remains the canvas detail route. Product pages now share the reference top nav + left workspace sidebar; the top nav labels are Landing page / Workspace / Collection / Team / Subscription, and Board cards reserve footer space for collaborators, a Panel entry and a three-bar actions menu. Landing page, Collection, Settings, Account, Team and Subscription stay separate surfaces. This is still a mock workspace shell; real workspace creation, asset library CRUD, team membership and subscription billing remain behind the Auth/Postgres/productization gate.
...
2026-05-01 AI contract first-pass note:

Codex added a mock, server-owned AI contract without real provider calls. Next local bridge now exposes `GET /api/ai/models`, `POST /api/ai/runs` and `GET /api/ai/runs/{runId}`; FastAPI exposes matching `/api/v1/ai/models` and `/api/v1/ai/runs` routes. The Web AI client switches between local bridge and FastAPI with the same `NEXT_PUBLIC_API_BASE_URL` rule used by Asset/Board persistence. Image Gen / Image Gen 4 model selectors read model options through `useAiModels()` with a shared mock registry fallback, and node Run creates a mock AiRun before writing returned `runId`, `modelId`, `outputAssetIds` and `textOutput` into runtime summary. Tests cover model registry, mock run round-trip and auth-required failure. This is still not real AI: no provider key, no cost charging, no Postgres AiRun table, no R2 result assets and no budget kill switch yet.

2026-05-01 AI extension contract note:

Future AI nodes and AI Chat bots must follow `ARCH.md` 4.4.1 rather than adding one-off frontend calls. A new node has to declare its node spec, ports, params schema, model capability, run type, result shape, preview UI, failure states and persistence constraints. Node Registry, Model Registry, AiRun request/response, local/FastAPI routes, tests and Board guard must be updated together. AI Chat / Planner may only emit graph specs for Node Runtime to apply; it must not bypass Asset / AiRun persistence or write Provider payloads into the Board document.

2026-05-01 Board autosave hardening note:

Codex split `CanvasBoardSaveAudit.tsx` by moving timer and before-unload lifecycle logic into `useBoardSaveLifecycle.ts`, bringing the save controller below the 250-line warning. Board-mode autosave now tracks a last-saved document signature, so long browser sessions that receive dirty events without serialized Board changes return to `Saved` without making duplicate save API calls. Edits that happen while a save is in flight are still detected by comparing the saved signature with the current serialized document and scheduling the next autosave when needed. Real browser long-form hand smoke remains the next validation step.

2026-05-01 Board UX hand-test bugfix note:

User hand testing found three Board product bugs. First, Board rename worked locally but entering the canvas could overwrite the title with the URL-derived fallback; `/boards/:boardId` now reads the stored `board.title` after load, Workspace Board titles single-click open and double-click rename, the Open / Rename actions remain available, and the canvas title can also be double-clicked to rename. Second, browser Back now gets the same unsaved-change confirmation as refresh/close while the Board is dirty, blocked, saving or save-failed; autosave debounce was also tightened from 1600ms to 1200ms while keeping document-signature dedupe. Third, runtime image asset migration now writes only sanitized JSON-safe `tangentAsset` metadata instead of spreading tldraw runtime `asset.meta`, fixing `asset(type = image).meta` validation failures after pasting many images. `BoardDashboardRow.tsx` was split out to keep the historical dashboard source below the 300-line limit.
...
10. ▶️ Local polish if external resources are not ready — Smart Drawing tuning, long-session regression, i18n/status cleanup
...
- Product Shell, Board save UX, Workspace Board metadata, Board History, Board Management metadata, captured thumbnail, Canvas Settings, Smart Drawing, Auth scaffold and AI contract scaffold can proceed locally before server/domain/email/provider resources are ready; they must remain mock/skeleton until real Auth, real AI and staging exist. Refresh preview / Workspace card / History thumbnail browser smoke has passed; current first passes are checkpointed or locally implemented, not complete product claims, so the next local emphasis is Smart Drawing tuning / regression polish or external-resource staging wiring.
