...
**Status**: Active local backlog. L1 Product Shell skeleton plus App Shell 5-tab nav and Landing page / Collection / Account / Settings / Team / Subscription semantic cleanup, L2 Board save UX, L3 Workspace Board metadata polish, L4 Auth scaffold boundary, L5 AI contract scaffold, L6 database migration scaffold, L7 Board History, L8 Admin S0 planning, L9 Smart Drawing planning, expanded Board Management Panel metadata and per-board Canvas Settings have first-pass local checkpoints or documented boundaries, but they are not product-complete yet. Route/responsive browser smoke and Board autosave / rename / Back warning / multi-image paste regression passed on 2026-05-01. Board History browser smoke for autosave / Snapshot / Cmd+S / Restore-dirty-autosave passed on 2026-05-02. Workspace `lastOpenedAt` metadata, client-side Load more pagination, expanded Board Management Panel metadata, per-board Canvas Settings and History API contract are now implemented locally. Remaining local candidates before external resources are captured Board thumbnails, Smart Drawing, long-session regression and i18n/status polish.
...
| Board Canvas Settings | Per-board dots/grid/solid background, background/pattern color, spacing and snap strength | Lets each board keep its own visual surface and interaction feel without external resources | Settings persist in lightweight `canvasSettings`, restore with Board/History, and dots remain subtle |
...
| Smart Drawing planning | 手绘线条/形状本地拟合：直线、椭圆、矩形、三角形、低置信度保留原样、undo | 用户想要更像白板产品的“画歪也能整理干净”体验；纯本地实现，不等外部资源 | 先记录 ARCH / PRD / Harness；实现前需拆 `CanvasSpikeToolbar.tsx`，避免继续堆大文件 |
...
3. **Workspace Board polish**: recent opened metadata, client-side Load more pagination and expanded Board Management Panel metadata are implemented; remaining work is real captured thumbnail, real share/member permissions and server-side pagination after real DB scale.
...
8. **Per-board Canvas Settings**: dots/grid/solid background and snap strength are implemented; remaining work is browser visual regression across zoom/device sizes.
...
10. **Smart Drawing planning**: architecture is documented; remaining work is a dedicated canvas-tool slice, now a good local candidate after Board Panel metadata.
11. **Captured thumbnail**: still pending; this is the most product-visible remaining Workspace polish before real share/member permission.
...
| L3 | Workspace Board metadata polish | first-pass checkpoint + recent-open + management metadata pass | Board gallery/list shows thumbnail placeholder, shape/asset counts, loading skeleton, empty CTA and error retry; opening a Board stamps `lastOpenedAt`, Workspace can sort by Recently opened / Recently saved, gallery/list renders an initial page with Load more, and Board Panel persists `description` / `cardColor` / `isPinned` / `isStarred` / `visibility` / `shareId` / editable `thumbnailUrl` outside the Board document. Panel also shows owner/created/modified/location/object details and member nickname/role scaffold. Captured thumbnails, real share permissions, real member persistence and server-side pagination remain later |
...
| L9 | Smart Drawing planning | documented boundary | Smart Drawing 是本地几何拟合工具，不调用 AI provider；第一版目标是直线、椭圆、矩形、三角形、低置信度保留原样和 undo；实现前先拆 toolbar / draw recognizer 模块 |
| L10 | Captured Board thumbnail | pending local candidate | 保存或手动刷新时生成轻量 Board preview，并把缩略图作为 Asset/metadata 供 Workspace card 使用；失败时保留 deterministic placeholder，不把缩略图 binary 写进 Board document |
| L11 | Smart Drawing implementation | pending local candidate | 在 draw 工具落笔后本地识别直线、椭圆、矩形、三角形等，输出普通 tldraw shape；低置信度保留原 stroke；支持 undo 与设置开关 |
...
- Do not bolt Smart Drawing directly into `CanvasSpikeToolbar.tsx` without splitting. It should be a dedicated recognizer/controller hook that outputs normal tldraw shapes and preserves undo.
