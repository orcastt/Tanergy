...
**Status**: Active local backlog. L1 Product Shell skeleton plus App Shell 5-tab nav and Landing page / Collection / Account / Settings / Team / Subscription semantic cleanup, L2 Board save UX, L3 Workspace Board metadata polish, L4 Auth scaffold boundary, L5 AI contract scaffold, L6 database migration scaffold, L7 Board History, L8 Admin S0 planning, L9 Smart Drawing first pass, L10 captured Board thumbnail first pass, expanded Board Management Panel metadata and per-board Canvas Settings have first-pass local checkpoints or documented boundaries, but they are not product-complete yet. Route/responsive browser smoke and Board autosave / rename / Back warning / multi-image paste regression passed on 2026-05-01. Board History browser smoke for autosave / Snapshot / Cmd+S / Restore-dirty-autosave passed on 2026-05-02. Workspace `lastOpenedAt` metadata, client-side Load more pagination, expanded Board Management Panel metadata, per-board Canvas Settings, History API contract, save-time captured thumbnail, manual Refresh preview, History preview thumbnails and Smart Drawing first pass are now implemented locally. Refresh preview / Workspace card / History thumbnail browser smoke has passed after fixing Next local `local-snapshot` `thumbnailUrl` forwarding. Smart Drawing has passed front-end gates and recognizer smoke; browser tuning is still pending. Remaining local candidates before external resources are long-session regression, Smart Drawing browser tuning and i18n/status polish.
...
3. **Workspace Board polish**: recent opened metadata, client-side Load more pagination, settings-like Board Management Panel metadata, owner/admin editable guard, thumbnail remove-to-default, save-time captured thumbnail, manual Refresh preview and History preview thumbnails are implemented; Refresh preview / Workspace card / History thumbnail browser smoke has passed; remaining work is real share/member permissions and server-side pagination after real DB scale.
...
| L10 | Captured Board thumbnail | browser smoke passed | 保存时若 Board 没有自定义 thumbnail，会生成轻量 WebP Board preview，上传为 Asset，并把缩略图 URL 作为 Board metadata 供 Workspace card 使用；Refresh preview 可强制重捕获；History summary 可携带 `thumbnailUrl` 并在 History 面板显示预览；失败时保留 deterministic placeholder，不把缩略图 binary 写进 Board / History document。Refresh preview / Workspace card / History thumbnail 浏览器 smoke 已通过 |
