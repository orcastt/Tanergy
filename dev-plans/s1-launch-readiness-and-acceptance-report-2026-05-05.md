...
- S1D 当前稳定 checkpoint 还包含：Board cursor pagination、Board copy、snapshot restore、guest-aware board-member roles，以及 Board Panel 里的首轮成员管理流。
...
| S1D Board CRUD | Permission matrix、cursor pagination、copy/restore/member scaffold contract tests | 真实 Auth request context | User A 不能读/改 User B 的 Board；copy/restore 走 server-side 权限；member scaffold 不越权 |
...
1. 进入 S2 real AiRun provider adapter，把当前 mock runtime 数据流切到真实 server-side provider contracts。
2. 再补 admin search / pagination / richer audit filters，避免把 `/admin` 卡在首轮 bounded lists。
3. 视情况继续补 share editor / temporary-viewer expiry / invite accept 等更完整 shared-collab 前置能力。
