...
**Updated**: 2026-05-15
**Status**: S1X Konva Page polish、S1D public share + owner-only copy/delete + known-foreign Asset guard hardening、S1C Clerk/Auth first pass、S3 admin/entitlement first pass，以及 staging Konva-only 部署恢复、Cloudflare strict proxy 边界与真实 session/admin smoke 已接上的活跃交接报告；在 `p0-alpha-stabilization-and-acceptance-2026-05-06.md` 收口后，本文件继续作为更细粒度的验收清单。
...
- 剩余发布闸门变成 signed-in board/browser、Google/email，以及一条真实 live AI image smoke。
...
```text
1. 补完 staging 剩余浏览器验收
   signed-in board/browser: private-board CRUD, paste/upload placeholder -> persistence, reload, history, thumbnail
   |
   v
...
- signed-in browser 验收至少覆盖 private board create/open/save/delete、图片粘贴占位态、后台上传持久化、reload 后恢复、history 和 thumbnail。
...
1. 先完成 signed-in board/browser staging 验收，把 private board CRUD、图片粘贴上传持久化、reload、history 和 thumbnail 跑透。
