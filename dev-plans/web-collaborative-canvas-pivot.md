...
- 可以放 Prompt / Generate / Edit 三种 AI 节点卡片。
...
- 状态：已按首轮手测反馈收口工具层、card 自适应和图片粘贴限制；等待复测，通过后先进入 Step 1.5，而不是直接进入正式 Step 2。
...
| 过度依赖视窗剔除 | Step 1.5 用生产构建验证 50-100 复杂节点和图片密集画布 |
...
| 外部多图粘贴导致卡顿 | 限制 MIME、体积和长边；Step 1.5 继续验证压缩、缩略图和懒加载 |
...
1. Step 1 画布小 UI 暂停深抠，只保留关键复测。
2. 如果没有阻塞级坐标、误触或卡顿问题，进入 Step 1.5。
3. 做 Step 1.5：复杂节点、轻量节点 payload、端口、Inspector、自动布局、Merge Capture 技术裁决。
4. Step 1.5 通过后做 Prompt / Image Gen / Image Gen 4 / Analysis / Image 五类节点链路。
5. 做图片模型注册表与模型选择器。
6. 接后端 AI 生图，一次生成 4 张。
7. 做 Image Editor 导出为新 Image Node。
8. 做 Send to Canvas + Merge Capture。
9. 做右侧 AI Chat 自动搭线。
10. 最后做 Dashboard / 保存 / 登录收口；邀请链接进入 P0.5 协作。
