...
**当前主线**: S1.5 已稳定，Asset LOD Slice A-C 已落地，Slice D 普通 canvas image LOD spike 进行中；Slice E 真实 Asset Pipeline 在多人协作前完成
...
- Asset LOD Roadmap 已成为当前主线：Slice A Image Node moving degrade、Slice B Node LOD、Slice C local asset preview resolver 已落地；Slice D ordinary canvas image LOD spike 正在进行；Slice E real Asset Pipeline 是多人协作前置项。
...
Image Node → canvas image、canvas image → Image Node、Screenshot / Merge Capture → Image Node 的本地闭环已落地。当前需要继续验证的是：Slice D 自定义 ordinary canvas image 渲染后，Convert to Image Node、To Canvas、Screenshot、Merge Capture 和导出 fidelity 是否仍然正确。
...
| 多图多节点下图片渲染卡顿 | Slice A-C 已完成本地降噪和 thumbnail resolver；Slice D 正在把普通 canvas image 纳入同一 LOD 策略 |
| Mac 手测不能代表上线表现 | Slice D 后必须补 Windows Chrome / Edge、1080p / 2K / 4K、browser zoom 90% / 100% / 125% 和低端 GPU 的性能矩阵 |
...
1. 完成 Slice D ordinary canvas image LOD spike，确认普通画布图片也能在低缩放 / 移动时复用 `assetPreviewResolver`，同时不破坏 Convert to Image Node、To Canvas、Screenshot、Merge Capture 和导出 fidelity。
2. 跑跨平台性能验收：Mac Chrome / Safari，Windows Chrome / Edge，1080p / 2K / 4K，browser zoom 90% / 100% / 125%，重点看 25%-50% 缩放区间和 30/100 张图片规模。
3. 进入 Slice E Real Asset Pipeline：后端 upload endpoint、object storage、多尺寸缩略图、asset metadata、权限 URL，以及保存前拒绝或迁移 `data:` / `blob:` 图片引用。
4. 在真实 Asset Pipeline 稳定后，再接真实 Model Registry / AI Proxy / AI Run log，把当前 mock Image Gen / Analysis 链路换成真实后端结果。
5. 做 Dashboard / 保存 / 登录收口，保证 Board refresh 后节点、连线、viewport 和 asset 引用可恢复。
6. 解决 Link Preview 的后端 unfurl + image proxy / Asset 化。
7. 最后再进入 P0.5 多人协作；协作只同步轻量文档状态，Presence、本地 LOD、AI Run 和 Asset 写入分别走自己的边界。
