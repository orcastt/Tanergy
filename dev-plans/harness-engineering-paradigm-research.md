...
- **Settlement guard (Round 4 Codex finding, 2026-05-28)**: validator runs with `cost_credits=0` will be rejected by the existing `positive_credits()` check at `services/api/tangent_api/credit_ledger_support.py:176-179` (raises 400 for `<=0`). v2 implementation MUST bypass this guard on the validator path. Two viable options: (a) skip `settle_usage_charge_to_account` entirely when `run_type='validator'` and write the validator_cost ledger row via a new `record_validator_cost()` helper that does NOT call `positive_credits()`; (b) relax `positive_credits()` to accept `0` when the caller passes `settlement_kind='validator_cost'`. Decision deferred to v2 implementation — captured in #47.
