...
def list_admin_ai_route_metrics(
    *,
    ...
    api_call_rows, run_rows, model_rows = _load_metric_rows(_route_metric_sample_limit())
...
def _load_metric_rows(sample_limit: int) -> tuple[list[tuple[object, ...]], list[tuple[object, ...]], list[tuple[object, ...]]]:
    with connect_to_postgres() as connection:
        with connection.cursor() as cursor:
            ...
            api_call_rows = cursor.fetchall()[:sample_limit]
            run_ids = sorted({str(row[3]) for row in api_call_rows if row[3] not in (None, "")})
            run_rows = _load_run_rows(cursor, run_ids)
            cursor.execute(
                """
...
def _load_run_rows(cursor: object, run_ids: list[str]) -> list[tuple[object, ...]]:
    if not run_ids:
        return []
    cursor.execute(
        """
    ...
    return [row for row in cursor.fetchall() if str(row[0]) in run_id_set]


def _route_metric_sample_limit() -> int:
    raw = os.getenv("TANGENT_ADMIN_AI_ROUTE_METRIC_ROW_LIMIT", "5000")
    try:
        value = int(raw)
    except ValueError:
        value = 5000
