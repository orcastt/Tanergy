...
LEGACY_WORKSPACE_ROLES = frozenset(LEGACY_WORKSPACE_ROLE_ALIASES.keys())
ALL_WORKSPACE_ROLES = PRODUCT_WORKSPACE_ROLES | LEGACY_WORKSPACE_ROLES
...
def preserve_workspace_role(value: str) -> str:
    normalized = value.strip().lower()
    if normalized in ALL_WORKSPACE_ROLES:
        return normalized
    raise ValueError("Invalid workspace role.")
