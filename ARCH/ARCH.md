...
S1A Schema/DB foundation      80%  [################----]
  Stable: core user/workspace/board/asset/admin/AI/credit join points and migrations.
  Gate: Team-wallet schema delta plus staging migration/query smoke.
...
S1D Board/share/invites       68%  [##############------]
  Stable: Board CRUD, owner-only copy/delete, share expiry, public view, member first pass.
  Gate: workspace invite acceptance, Group/Team role and billing-visibility separation, explicit Asset sharing.

S2 AI runtime/provider routes 55%  [###########---------]
  Stable: model/route/pricing tables, quote/preflight, persisted AiRun shell, admin facts.
  Gate: fold local GeekAI path into provider-route adapter, Team-wallet payer resolver, real live smoke, text persistence.

S3 Admin/billing/team         46%  [#########-----------]
  Stable: server-gated admin, AI route/pricing panels, billing/team/usage first pass.
  Gate: Team-wallet/Collaborate-wallet pivot, real payment webhooks, renewals/cancelation, deeper finance and route health views.
...
| S3 Admin/Billing/Analytics | `ARCH_slice_S3_admin_billing_analytics.md` | Admin roles, audit, Team wallets, personal Collaborate wallets, credits, subscriptions, workspace dashboards, AI charge facts, analytics, moderation facts | Active pivot: first-pass `/admin`, billing/team/usage, seat, ledger and AI runtime/control-plane scaffolds exist; next architecture gate is Team wallet + personal Collaborate wallet payer resolution before real billing/payment/provider settlement |
...
S1A Schema/DB foundation      80%  [################----]
  ...
  闸门：Team-wallet schema delta，以及 staging migration/query smoke。
...
S1D Board/share/invites       68%  [##############------]
  ...
  闸门：workspace invite acceptance、Group/Team role 和 billing-visibility separation、explicit Asset sharing。

S2 AI runtime/provider routes 55%  [###########---------]
  已稳定：model/route/pricing tables、quote/preflight、persisted AiRun shell、admin facts。
  闸门：把 local GeekAI path 收口进 provider-route adapter、Team-wallet payer resolver、real live smoke、text persistence。

S3 Admin/billing/team         46%  [#########-----------]
  已稳定：server-gated admin、AI route/pricing panels、billing/team/usage first pass。
  闸门：Team-wallet/Collaborate-wallet 调整、real payment webhooks、renewals/cancelation、更深 finance 和 route health views。
...
| S3 Admin/Billing/Analytics | `ARCH_slice_S3_admin_billing_analytics.md` | Admin roles、audit、Team wallets、personal Collaborate wallets、credits、subscriptions、workspace dashboards、AI charge facts、analytics、moderation facts | 活跃调整：第一阶段 `/admin`、billing/team/usage、seat、ledger 和 AI runtime/control-plane 脚手架已存在；下一道架构闸门是真实 billing/payment/provider settlement 前，先完成 Team wallet + personal Collaborate wallet payer resolution |
