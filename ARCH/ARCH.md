...
| S1B Staging Infra | `ARCH_slice_S1B_staging_infra.md` | Vercel, FastAPI host, Postgres, R2, domain, monitoring, email provider, staging smoke | Web/API/Supabase Pro/R2, Konva-only redeploy, real session/admin smoke, release-style staging env handling, env templates, ops readiness smoke, API observability and final-snapshot realtime persistence passed; R2 clean asset, Google/email/live-AI smoke and external ops-provider proof pending |
...
## Network Security Hardening Architecture

The current security model is defense in depth: frontend checks improve UX, but server-side identity, resource membership, quotas, file validation and persistence guards are the authority. New security work is tracked in `dev-plans/p0-collaboration-security-hardening-2026-05-19.md`, with release acceptance in `docs/security-release-acceptance.md`.

Security boundaries now in the active architecture:

- Browser and Next boundary: global security headers/CSP/HSTS in production, same-origin guards on local write routes, admin proxy path/header/body allowlists, local bridge production fail-closed behavior, public share `noindex`, unsafe URL sanitization and Sentry event scrubbing.
- FastAPI HTTP boundary: production-like origin allowlists, CORS/CSRF checks, body-size limits, security response headers, request observability, structured security events and no secret/provider-key exposure to the browser.
- Auth and resource boundary: every Board, Workspace, Admin, Billing, AI and collaboration action resolves server-side membership/role from the authenticated request; spoofable frontend role/plan headers are not authority.
- Distributed anti-abuse boundary: Redis-backed counters protect shared deployments, with scoped limits for IP, user, workspace, share token, route and realtime room; Postgres or memory fallback remains local/dev-only unless explicitly allowed.
- Business cost boundary: high-cost AI, image, remove-background, upload, export, checkout, invite and share paths use short-window limits, daily quotas and idempotency keys where duplicate charges or duplicate writes are dangerous.
- Realtime boundary: WebSocket handshakes require allowed Origin in production-like environments, message size/rate is bounded, viewer writes are rejected, edit permission is revalidated per message, and revoked access closes the connection.
- Public share boundary: tokens are strong random values and stored hashed where applicable; share links support expiration, password protection, revoke/regenerate, minimal public payloads, anti-crawl headers and public rate limits.
- Upload/import boundary: uploads and remote imports validate MIME plus magic bytes, reject SVG/PDF executable-document lanes for now, block `data:`/`blob:`/Base64 persistence, and apply SSRF redirect/DNS/private-IP checks before proxying remote images.
- Rendering/persistence boundary: Board guards reject unsafe asset refs and long/generated/provider raw payloads before storage; React/Konva render text as text, not HTML, and static guards prevent new dangerous DOM sinks.

Production-like security config must be environment-owned and never documented with secret values. Relevant non-secret switches and allowlists include `TANGENT_REQUIRE_API_AUTH`, `TANGENT_ALLOWED_ORIGINS`, `TANGENT_ALLOWED_WEBSOCKET_ORIGINS`, `TANGENT_REQUIRE_WEBSOCKET_ORIGIN`, `TANGENT_SECURITY_REDIS_ENABLED`, Clerk issuer/audience/authorized-party settings, object-storage driver/bucket settings, Sentry/observability DSNs and alert/status-page metadata.

Release and smoke artifacts:

| Artifact | Purpose |
| --- | --- |
| `npm run security:gate` | Local combined security release gate. |
| `apps/web/scripts/security-static-guard.mjs` | Static scan for dangerous DOM sinks, hardcoded secret patterns, browser token storage and spoofable role/plan headers. |
| `apps/web/scripts/next-security-guard-smoke.mjs` | Next route CSRF/admin-proxy/local-bridge security smoke. |
| `apps/web/scripts/public-share-client-smoke.mjs` | Public share client request-shape, password-header and revoke/create behavior smoke. |
| `services/api/scripts/security_release_gate.py` | Backend release gate wrapper, optionally including staging auth/Redis checks. |
| `services/api/scripts/security_deploy_config_smoke.py` | Production-like env/config guard for auth, Redis, origins, WebSocket Origin, Clerk, object storage and observability readiness. |
| `services/api/scripts/security_redis_smoke.py` | Real Redis counter/TTL connectivity smoke for distributed anti-abuse. |
| `services/api/scripts/security_object_storage_smoke.py` | Real S3/R2 temporary write/read/delete plus public-read isolation smoke. |
| `services/api/scripts/security_staging_auth_smoke.py` | Real bearer-token smoke for backend auth, role spoofing rejection and origin rejection. |
| `services/api/scripts/security_api_performance_smoke.py` | Low-cost API p95/max latency smoke for release regressions. |
| `services/api/scripts/s4_realtime_multiplayer_load.py` | 15-client WebSocket collaboration load harness. |
...
| `dev-plans/s2-ai-provider-route-billing-control-plane-2026-05-07.md` | Keep the current GeekAI-first deployment and future providers behind the same server-owned AiRun route switching, credit settlement and admin observability |
...
**更新日期**：2026-05-20
**状态**：规范架构总览和切片索引，当前已对齐正在并行推进的 P0 alpha 工作流、S3 Team-wallet 调整、S3/S4 Team-Group foundation baseline，以及最新 S1B/S2 cleanup：Konva-only staging 部署与真实 Clerk session/admin smoke 已转绿，staging 数据库事实源已切到 fresh Supabase Pro Postgres，旧 Hetzner `staging-postgres` fallback 与 volume 已删除，R2 staging 对象已清理，board realtime 过程流量留在 WebSocket room memory，只有 compacted/final snapshots 写入 Postgres；Web/API env 模板、公开 TLS/header/CORS smoke、Next/FastAPI Sentry 接入、API 慢响应/RSS 观测和事故手册已存在；后端 `AiRun` 已支持 durable `text_output`，活跃 AI provider route defaults 已切回 GeekAI text / analysis / image lanes。第二轮 browser Board 验收大部分已绿，`Manage board -> Copy board` Free-plan modal 路径已完成本地 wiring，下一道闸门是 R2 clean asset smoke、该 modal 的 staging spot check、Google/email、一条 live provider smoke，以及外部 uptime/status/backup/PITR/Sentry/APM 证明，然后才继续更深的 provider / collaboration 工作。

本文件取代原来的重复 `ARCH/00-current-map.md` 和根目录长 `ARCH.md`。根目录 `ARCH.md` 现在只做指针用途。

## 平行开发泳道

原来的单主干图已经不够用了，因为 Auth/deploy、Board 权限、Team/Group billing、AI provider routing、admin tooling 和 frontend node UI 正在并行推进。下面的百分比是截至 2026-05-20 的粗颗粒架构就绪度，不是工时估算；它表示每条线是否已有可用第一阶段、服务端权威边界，以及还剩哪个 production gate。
...
| S1B Staging Infra | `ARCH_slice_S1B_staging_infra.md` | Vercel、FastAPI host、Postgres、R2、domain、monitoring、email provider、staging smoke | Web/API/Supabase Pro/R2、Konva redeploy、real session/admin smoke、env 模板、ops readiness smoke、Sentry 可选接入和 final-snapshot realtime persistence 已通过；R2 clean asset、Auth/email/OAuth/live-AI smoke 与外部 ops-provider 证明待完成 |
...
| S2 AI Runtime | `ARCH_slice_S2_ai_runtime.md` | Node Registry、Model Registry、AiRun、provider routing、AI Chat planner | Mock/runtime dataflow 和本地 GeekAI-first canvas path 现在已经证明用户侧 image/analysis/chat flow；DB-backed 模型档位 / 定价 / 线路、quote/preflight、持久化 lifecycle、按尝试分行的 `ai_api_calls`、timeout-safe failover，以及抽离出的 settlement orchestration 已存在；production gate 是把 GeekAI 和未来 providers 收口到服务端 provider-route control plane，并完成 live smoke 和 durable text output |
...
当前安全模型是多层防护：前端校验只改善体验，真正的权限、资源归属、配额、文件校验和持久化保护都由服务端裁决。开发台账在 `dev-plans/p0-collaboration-security-hardening-2026-05-19.md`，发布验收清单在 `docs/security-release-acceptance.md`。

当前已进入活跃架构的安全边界：

- Browser / Next 边界：全站安全响应头、生产 HSTS/CSP、本地写接口同源 guard、admin proxy 的 path/header/body allowlist、本地 bridge 生产 fail-closed、公开 share `noindex`、不安全 URL sanitizer，以及 Sentry event scrubber。
- FastAPI HTTP 边界：production-like origin allowlist、CORS/CSRF、body size limit、安全响应头、请求观测、结构化 security events，并且不把 secret / provider key 暴露给浏览器。
- Auth / Resource 边界：Board、Workspace、Admin、Billing、AI、Collaboration 操作都从认证请求解析服务端 membership / role；可伪造的前端 role/plan header 不作为权限事实。
- 分布式防刷边界：共享部署使用 Redis counter，按 IP、user、workspace、share token、route、realtime room 做限速；Postgres / memory fallback 仅用于 local/dev 或明确允许的降级场景。
- 业务成本边界：AI、生图、remove background、upload、export、checkout、invite、share 等高成本路径使用短窗口限速、每日配额，以及对危险重复写入 / 重复扣费的 idempotency key。
- Realtime 边界：production-like WebSocket 握手要求 Origin allowlist，限制消息大小和速率，拒绝 viewer 写入，每条消息重新校验 edit 权限，权限撤销时关闭连接。
- Public share 边界：强随机 token，必要时 hash 存储；支持过期、密码、撤销 / 重建、最小公开 payload、防爬响应头和公开访问限流。
- Upload / Import 边界：同时校验 MIME 和 magic bytes，当前拒绝 SVG/PDF 可执行文档路径，禁止 `data:` / `blob:` / Base64 落库，远程图片代理前做 redirect、DNS 和 private-IP SSRF 检查。
- 渲染 / 持久化边界：Board guard 在落库前拒绝不安全 asset refs、超长生成文本和 provider raw payload；React/Konva 以文本方式渲染用户内容，静态 guard 防止新增危险 DOM sink。

生产类安全配置必须由环境管理，tracked docs 不记录 secret 值。相关非密钥开关和 allowlist 包括 `TANGENT_REQUIRE_API_AUTH`、`TANGENT_ALLOWED_ORIGINS`、`TANGENT_ALLOWED_WEBSOCKET_ORIGINS`、`TANGENT_REQUIRE_WEBSOCKET_ORIGIN`、`TANGENT_SECURITY_REDIS_ENABLED`、Clerk issuer/audience/authorized-party、对象存储 driver/bucket、Sentry/observability DSN 以及 alert/status-page 元数据。

发布和 smoke 脚本：

| 脚本 | 用途 |
| --- | --- |
| `npm run security:gate` | 本地统一安全发布闸门。 |
| `apps/web/scripts/security-static-guard.mjs` | 扫危险 DOM sink、硬编码 secret 特征、浏览器 token storage 和可伪造 role/plan header。 |
| `apps/web/scripts/next-security-guard-smoke.mjs` | Next 写入口 CSRF、admin proxy、本地 bridge 安全 smoke。 |
| `apps/web/scripts/public-share-client-smoke.mjs` | 公开分享 request shape、密码 header、create/revoke 行为 smoke。 |
| `services/api/scripts/security_release_gate.py` | 后端 release gate wrapper，可追加 staging auth / Redis 检查。 |
| `services/api/scripts/security_deploy_config_smoke.py` | 检查 auth、Redis、origin、WebSocket Origin、Clerk、对象存储和观测配置是否符合 production-like 要求。 |
| `services/api/scripts/security_redis_smoke.py` | 真实 Redis counter / TTL 连接 smoke。 |
| `services/api/scripts/security_object_storage_smoke.py` | 真实 S3/R2 临时写入 / 读取 / 删除和 public-read 隔离 smoke。 |
| `services/api/scripts/security_staging_auth_smoke.py` | 真实 bearer token、role spoof 拒绝和恶意 Origin 拒绝 smoke。 |
| `services/api/scripts/security_api_performance_smoke.py` | 低成本 API p95/max 响应时间 smoke。 |
| `services/api/scripts/s4_realtime_multiplayer_load.py` | 15 客户端 WebSocket 协同压测 harness。 |
...
| `dev-plans/s2-ai-provider-route-billing-control-plane-2026-05-07.md` | 把 GeekAI-first 当前线路和未来 providers 一起收口到服务端 AiRun route switching、credit settlement 和 admin observability |
