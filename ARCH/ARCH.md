...
S1B Deploy/staging            88%  [##################--]
  ...
  Gate: R2 clean asset smoke, re-created staging data, staging spot check for the now-wired `Manage board -> Copy board` Free-plan modal path, Google/email/OAuth verification and one live provider smoke under staging auth.
...
S1B Deploy/staging            88%  [##################--]
  ...
  闸门：R2 clean asset smoke、重新创建 staging 数据、已经完成 wiring 的 `Manage board -> Copy board` Free-plan modal staging spot check、Google/email/OAuth 验证，以及 staging auth 下的一条 live provider smoke。
