...
- The current websocket room is intentionally minimal and provider-shaped rather than full Yjs-server-native. It gates access through existing board collaboration permissions, keeps process `yjs-update` traffic in room memory by default, persists only compacted/final realtime document snapshots to local-dev/Postgres storage unless explicit `update_chain` rollback mode is enabled, replays the current room state to newcomers as `sync-state`, uses an explicit `seedRoom` handshake so clients seed only genuinely empty rooms, requests a client `sync-state-publish` compaction when the incremental chain grows past a threshold, acknowledges accepted full-state publishes back to connected clients as `sync-state-accepted`, and fans out awareness `batch/state/remove` events.
...
- Presence now also carries an optional sanitized `draftPreview` for in-progress drawing/shape creation. The browser publishes it at a low fixed cadence and remote users render it as a non-interactive canvas overlay; it is awareness/process state, not Board document state and not a Postgres write.
...
- The websocket room is still a readiness bridge, not the final production provider: while it can collapse room state back to a fresh full-state snapshot through client-assisted compaction and persist that final snapshot, it is still single-process in fan-out behavior, not yet merging updates server-side, and not yet multi-instance safe.
...
- Postgres is not the realtime broadcast layer. Collaboration process updates should stay in WebSocket room memory or a future Redis backplane; Postgres stores final snapshots, history checkpoints and business facts.
...
4. Add awareness for cursor, selection, active page, current tool, focused editing ids and draft drawing previews.
