...
- Owner/admin can update Board metadata, visibility and members.
- Owner alone can copy and delete the Board in the initial product model.
...
The current implementation intentionally stops short of the final effective-permission resolver, but it now uses board-member facts and the already available authenticated request context:
...
- Board metadata update and snapshot clear require `owner/admin` or actual `board.owner_id`.
- Board copy/delete now require the actual Board owner or a board-member `owner` grant; `Can manage` does not imply copy/delete.
...
- Board copy exists as an owner-only server-checked duplicate flow that creates a new Board record and preserves only allowed source metadata/content.
...
- Share links can accept `expiresAt`; expired or revoked share ids no longer resolve or load a shared Board.
...
- Board save and snapshot create now run a conservative cross-workspace Asset reference guard before persistence. Known Asset ids from another workspace are rejected; unknown legacy/test ids are not forced to exist yet.

This is still not the final authorization model. First-pass public share is view-only; richer invite acceptance, temporary-viewer creation UI, explicit Asset-sharing allowlists and editor-via-share flows still remain future work, but S1D no longer stops at raw user-id member CRUD.

Current code now centralizes Board authorization through `resolve_effective_board_permission(record, context, board_member_role)`, returning one of:
...
The first-pass resolver keeps existing workspace behavior compatible while making the capability boundary explicit:

- `board.owner_id` or board-member `owner` -> `owner`
- board-member `admin` or workspace `owner/admin` -> `manage`
- board-member `editor` or workspace `member` -> `edit`
- board-member `viewer/temporary_viewer` or valid public/workspace visibility -> `view`

Snapshot create/list/load/clear now use the same Board permission boundary, so explicit Board editors can create History entries while viewers cannot.
...
- `Can manage` may not copy or delete the Board; the current backend and workspace UI now enforce this for copy/delete.
...
- Verify known assets referenced by Board belong to the same workspace before save/snapshot persistence. Later S1D/S2 work should replace the conservative known-foreign check with an explicit Asset-sharing allowlist.
...
- Board copy/delete fail for `Can manage`, `Can edit` and `Can view` actors.
...
- Share link resolve only succeeds for an active, unexpired stored share id.
- Share token Board load only succeeds for an active, unexpired stored share id and returns a full Board document without requiring workspace auth headers.
- Board save and snapshot create reject known Asset ids that belong to another workspace.
...
- Owner / Admin 可以更新 Board 元数据、可见性和成员。
- 初始产品模型中，只有 Owner 可以 copy 和 delete Board。
...
- Public / shared Board 仍然必须经过服务端 share link 校验，包括 revoke 和 expiry 检查。
...
- 当前产品方向里，Board copy/delete 是 owner-only。
...
- Board 元数据更新和 snapshot clear 需要 `owner/admin` 或真实的 `board.owner_id`。
- Board copy/delete 现在需要真实 Board owner 或 board-member `owner` grant；`Can manage` 不代表可以 copy/delete。
- Snapshot create 需要 Board 写权限。
- Board 列表在已认证的 Postgres API 路径里已经是游标分页，因此大 workspace 不需要依赖页码扫描。
- Board copy 已经是 owner-only、服务端检查过的 duplicate 流程，会创建新的 Board 记录，并只保留允许复制的元数据和内容。
...
- Share links 可以接收 `expiresAt`；过期或被 revoke 的 share id 不再能 resolve 或 load shared Board。
...
- Board save 和 snapshot create 现在都会在写入 persistence 之前运行保守的 cross-workspace Asset reference guard。已知属于另一个 workspace 的 Asset id 会被拒绝；未知的 legacy / test id 目前还不会被强制要求必须存在。

这还不是最终授权模型。当前 public share 仍然是 view-only；更丰富的 invite acceptance、temporary-viewer 创建 UI、明确的 Asset-sharing allowlists，以及 editor-via-share 流程仍然留到后续。

当前代码现在通过 `resolve_effective_board_permission(record, context, board_member_role)` 集中处理 Board 授权，并返回以下之一：
...
第一阶段 resolver 会保持现有 workspace 行为兼容，同时把 capability 边界明确化：

- `board.owner_id` 或 board-member `owner` -> `owner`
- board-member `admin` 或 workspace `owner/admin` -> `manage`
- board-member `editor` 或 workspace `member` -> `edit`
- board-member `viewer/temporary_viewer` 或有效 public/workspace visibility -> `view`

Snapshot create/list/load/clear 现在也使用同一个 Board permission boundary，因此显式 Board editor 可以创建 History entries，而 viewer 不能。
...
- `Can manage` 不能 copy 或 delete Board；当前后端和 workspace UI 已经对 copy/delete 执行这一点。
...
- 在 save / snapshot persistence 前校验 Board 引用的已知 asset 是否属于同一 workspace。后续 S1D/S2 工作应把当前保守的 known-foreign 检查替换成明确的 Asset-sharing allowlist。
...
- `Can manage`、`Can edit` 和 `Can view` actor 执行 Board copy/delete 必须失败。
...
- Share link resolve 只能对活跃、未过期的存储 share id 成功。
- Share token Board load 只能对活跃、未过期的 share id 成功，并且在不要求 workspace auth headers 的前提下返回完整 Board 文档。
- Board save 和 snapshot create 必须拒绝已知属于另一个 workspace 的 Asset id。
