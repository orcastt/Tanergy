...
- `/boards/[boardId]` is now a dual-engine shell. `boardCanvasEngine.ts` detects persisted document engine from the loaded Board document: tldraw v1 documents keep using `CanvasSpike`, Konva v2 documents use `KonvaCanvasSpike`, and new/missing Boards default to Konva unless configured otherwise before a document exists. Unknown saved documents render an unsupported state instead of opening a blank default engine, and the route does not attempt v1-to-v2 conversion yet; both choices avoid accidental overwrite of existing Boards.
