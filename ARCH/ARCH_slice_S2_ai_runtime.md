...
- GeekAI text calls now use `/chat/completions` with `stream: true`: the local Next chat proxy forwards SSE to the browser, Chat / Prompt Optimizer / Analysis node UX can append deltas instead of waiting for a blocking response, and the FastAPI OpenAI-compatible AiRun adapter requests upstream streaming then folds SSE chunks back into the existing durable terminal `text_output` contract.
...
- 2026-05-20 staging route validation confirms the frontend/backend coordination point after the redeploy: Web release is live on `staging.tanergy.cc`, API release `b35adc0` is healthy on `api-staging.tanergy.cc`, Alembic is at `20260520_0033`, and the staging DB reports enabled/healthy GeekAI routes for QwQ text, Qwen VL analysis, GPT Image 2, Nano Banana 2 and Doubao Seedream 5.0 Lite. The previous staging attempt failed before container restart because QwQ pricing was referenced before insertion; the current migration inserts the model with no default pricing, inserts pricing, then backfills `default_pricing_rule_id`.
- 2026-05-20 Web text streaming validation: Vercel staging env was missing the new server-only GeekAI variables and initially returned `503 Missing GEEKAI text API key` from the Next `/api/ai/chat/completions` proxy. After syncing `GEEKAI_TEXT_*`, `GEEKAI_BALANCE_IMAGE_*`, `GEEKAI_OFFICIAL_IMAGE_*` and `GEEKAI_VIDEO_*` to the staging Vercel Production env and redeploying Web to `dpl_CwARDUa1WkLxDbnZLjrZkHppATMg`, the same endpoint returned `200` with `Content-Type: text/event-stream` and live SSE chunks for `qwq-plus-latest`. QwQ sends `reasoning_content` chunks before final `delta.content`; clients must not persist reasoning text, and should treat reasoning chunks as streaming activity rather than final node output.
...
- 2026-05-20 post-stage image safety fix: provider image outputs now infer persisted Asset MIME from magic bytes rather than trusting wrapper/header MIME. This specifically protects Nano Banana 2 / GeekAI chat-completion image payloads that arrive behind a mismatched `data:image/png` wrapper, while SVG/PDF/non-image payloads still fail the existing raster-only guard.
...
| --- | --- | --- |
| Text default | Text route switch was committed but Alembic `0033` failed on the QwQ pricing FK, so staging DB did not complete the new default. | Alembic `0033` is applied; `qwq-plus-latest` is enabled/healthy and default with `price_qwq_plus_latest_v1`. |
| Image default | Image defaults were in transition from Jiekou/proxy routes to GeekAI routes. | `gpt-image-2` is enabled/healthy, default, provider `geekai`, provider model `gpt-image-2`; `nano-banana-2` maps to `gemini-3.1-flash-image-preview`. |
| Frontend/backend contract | Frontend model controls had been unified locally, but staging DB route facts were not yet verified after deploy. | Staging DB route facts match the frontend model contracts, including Nano Banana 2 extended ratios and 240s image timeout route boundary. |
| Rollback posture | Jiekou code still exists and can remain as historical fallback. | Active/default routes are GeekAI-first; Jiekou is not the active catalog/default path. |
| Text SSE | Web runtime initially lacked GeekAI env, so the chat proxy failed before provider call. | Vercel env is synced; direct Chat SSE smoke returns live `data:` chunks. The remaining browser UX nuance is QwQ reasoning chunks arriving before visible content chunks. |
...
- GeekAI image adapters must infer persisted output MIME from image magic bytes, not from provider headers or data-URL wrappers. This keeps Nano Banana 2 chat-completion outputs compatible when the wrapper says PNG but the bytes are JPEG/WebP, while non-image/SVG/PDF payloads still fail before Asset persistence.
