        ...
        <tbody>
          {rows.length ? rows.map((workspace) => (
            <tr key={workspace.id}>
              ...
              <td>
                <MemberStack
                  bottomActions={workspace.role === 'admin' ? workspaceMemberActions(workspace, onAction) : []}
                  manageMembers={workspace.role === 'admin'}
              ...
              <td><BoardStack boards={workspace.boards} onAction={onAction} total={workspace.boardCount} totalLabel={`${workspace.boardCount}/unlimited`} workspaceId={workspace.id} /></td>
...
function TeamCreditCell({ workspace }: { workspace: AdminOperatorWorkspacePlan }) {
  return (
    <div className="admin-plan-credit-cell">
      ...
      <MetaLine>{formatNumber(workspace.usageByUser)}</MetaLine>
    </div>
  )
}
