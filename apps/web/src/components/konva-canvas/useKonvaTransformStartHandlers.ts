...
export function useKonvaTransformStartHandlers({
  ...
  const handleResizeStart = useCallback((shapeIds: string[], handle: KonvaResizeHandle, event: KonvaEventObject<PointerEvent>) => {
    ...
    if (hasRemoteShapeLock(shapeIds, remoteLockedShapeOwnerById)) return
  ...
  }, [documentRef, onHistoryCheckpoint, onInteractionShapeIdsChange, remoteLockedShapeOwnerById, sessionRef])
  ...
  }, [cameraRef, documentRef, onHistoryCheckpoint, onInteractionShapeIdsChange, remoteLockedShapeOwnerById, sessionRef, stageRef])
