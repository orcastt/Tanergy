...
> 每次开始开发前先判断模式：小 UI polish 读 `Project_state/current-slice.md` → `Project_state/00-current-progress.md` → `ARCH/README.md` → 相关 `ARCH/Slice-*.md` → active plan；架构/API/Auth/AI/Admin/Billing/Deploy/协作变化读 `project_state.md` → `PRD.md` → `ARCH.md` → `HARNESS.md` → `ARCH/05-data-model-and-api.md` → `dev-plans/README.md` → 当前 slice plan。
> 每次 commit 前更新此文件。
...
**下一步**: 当前本地稳定状态已提交到 `847ed3c checkpoint: canvas polish and context indexes` 并推送到 `feature/local-polish-fast-slices`。当前本地 Product Shell、Board save UX、Board History、Workspace Board metadata、captured Board thumbnail、manual Refresh preview、History preview thumbnails、Smart Drawing、Auth scaffold、AI contract scaffold、DB migration scaffold 和 Admin S0 planning 都是 first-pass checkpoint 或文档边界，不是完整产品完成态：`/login`、`/signup`、`/forgot-password`、`/verify-email` 已按 `reference/Design_reference.md` 进入 split-screen Auth visual pass；`/workspaces` 已纠正为 active workspace 内的 Board gallery/list 页面，读取同一 Board persistence list，支持 Gallery/List 切换、搜索、Recently opened / Recently saved / title / object 排序、新建 Board、打开、重命名、复制、删除、Board Panel metadata 和 Load more；Settings / Account / Team / Subscription 不再混入 `/workspaces` 主体。App Shell 已改成参考图式顶部导航 + 左侧 workspace sidebar，顶部 5 个标签为 Landing page、Workspace、Collection、Team、Subscription，分别对应 `/home`、`/workspaces`、`/collections`、`/team`、`/billing`；Landing page 不放入侧栏；Account 和 Settings 保留在侧栏语义入口；Upgrade Plan 指向 `/billing`。`/home` 是本地 Landing page / orientation shell，`/collections` 是未来 Asset / reference library placeholder；`/account` 现在是 mock profile / email status / workspace ownership / session guard / danger zone 的个人账户中心；`/settings` 聚焦 canvas defaults、persistence mode、AI model availability 和 server-secret boundary；`/team` 与 `/billing` 独立 placeholder 明确不接真实邀请、角色、Stripe 或额度。Workspace Board card 已预留底部协作成员、Manage 入口和三横线菜单；Board Management Panel 已从右侧 drawer 改成 settings-like 居中管理面板，顶部放 Copy link / Invite / Open / Save，左侧放 created / modified / opened / location / objects，右侧只放 Members，owner/admin 可改 title / description / card color / thumbnail URL 或上传/Remove 缩略图、star/pin/visibility，editor/viewer 先显示灰态权限；Workspace gallery/list card 右上角显示红色 pin 和 visibility 图标，share/copy link 有 copied toast，public/private 菜单交替显示并确认，copy board 会继承原卡片颜色。`/boards/:boardId` 在 Board 模式会监听画布 document / runtime edges / canvasSettings 变化并 debounce 保存，显示 Ready / Unsaved changes / Saving / Saved / Load failed / Save failed 等状态，刷新或关闭、以及浏览器 Back 遇到未保存状态时都会提示；打开 Board 会写入 `lastOpenedAt`，autosave 会保留该 metadata；Canvas Settings 现在按单个 Board 保存 `canvasSettings`，支持 subtle dots / grid / solid background、background color、pattern color、spacing 和 snap strength；autosave 已从 1600ms 调到 1200ms，并保留 signature 去重，避免无语义 dirty 事件重复保存。Board 保存时若缺少自定义 thumbnail，会自动捕获当前页面 WebP preview 并写入 Asset-backed `thumbnailUrl` metadata；Refresh preview 可手动强制重捕获，并把该用户保存写入 History；这些缩略图只保存 URL metadata，不写入 Board / History document。Board History first pass 已接入并通过真实浏览器 smoke：autosave 成功保存当前 Board 后写入 `autosave` history entry，Snapshot 按钮写入 `manual` entry，Cmd/Ctrl+S 执行当前 Board save 并写入 `keyboard` entry，Restore 后先进入 `Unsaved changes` dirty 状态，再由 autosave 保存恢复后的 Board document；History 支持 all/autosave/user saves filter、作者/头像显示、preview thumbnail 和 autosave/user save 视觉区分，History document 同样走 Board guard，免费层默认每个 Board 分别保留 autosave 100 条和 user saves 100 条。Refresh preview / Workspace card / History thumbnail 浏览器 smoke 已通过。Board title 现在会从保存记录回填；Workspace gallery 标题单击打开 Board、双击重命名，画布标题也支持双击重命名，右侧 Open / Rename 操作仍保留；Workspace 标题双击已改成 click-count guard，避免双击先触发单击打开。S0E first pass 已完成：Next `/api/ai/models` / `/api/ai/runs` 和 FastAPI `/api/v1/ai/models` / `/api/v1/ai/runs` 返回 mock Model Registry / AiRun；Image Gen / Image Gen 4 模型下拉通过 AI model client 读取 contract；节点 Run 现在创建 mock AiRun，再把返回的 mock asset ids 写回 runtime summary。未来新增 AI 节点 / AI Chat bot 的扩展规则已收敛到 `ARCH.md` 4.4.1：必须先扩展 Node Registry / Model Registry / AiRun 合同，节点 UI 不直接调用 Provider。Admin S0 口径已收敛到 `ARCH.md` 4.11：现在只做 `admin_roles`、审计、用户备注、Board members、credits/billing、AI API calls、analytics/moderation 事实源和 `/admin` access boundary 规划；完整 Mixpanel 级后台、生产可用 impersonation、收入大屏和审核工作台后置。真实浏览器复测已覆盖 App Shell 路由语义、`/home` / `/workspaces` / `/collections` / `/account` / `/settings` / `/team` / `/billing` 响应式，以及 Board autosave / rename / Back warning / multi-image paste 保存稳定性；下一刀若外部资源未就绪，继续 Smart Drawing browser tuning 或长时回归，若资源就绪则转真实 staging / R2 / Postgres / domain smoke。真实邮箱验证、OAuth、团队权限、AI provider key、R2/Postgres/domain/staging server 仍等外部资源接入。当前本地开发默认 `dev-user` / `dev-workspace`，Asset metadata 已带 `createdBy` / `workspaceId`，Board record 已带 `ownerId` / `workspaceId` / `lastOpenedAt` / `description` / `cardColor` / `createdAt` / `isPinned` / `isStarred` / `shareId` / `thumbnailUrl` / `visibility`，Board document 已带轻量 `canvasSettings`，Board history record 已带 `workspaceId` / `createdBy` / `retentionTier` / `thumbnailUrl`，Postgres driver 已可保存 Board document / summary metadata / history entries 和 S3 Asset metadata，Board rename/delete/copy/update metadata 已接 local/FastAPI/Postgres contract。
...
**2026-05-02 文档同步快照**: 当前实际架构已经收敛为 Next.js Product Shell + tldraw Canvas Runtime + Next local bridge / FastAPI API 双通道 + Postgres/Object Storage 边界。已落地的本地 first pass 包括 `/workspaces` Board gallery/list、Board Management metadata、per-board Canvas Settings、Board save/autosave、Board History、captured Board thumbnail、manual Refresh preview、History preview thumbnails、Smart Drawing、Auth scaffold、AI contract scaffold、Alembic migration scaffold 和 Admin S0 schema/access planning。Refresh preview / Workspace card / History thumbnail 已通过真实浏览器 smoke，并修复 Next local `local-snapshot` 漏传 `thumbnailUrl`。尚未完成或不能假装完成的部分是：真实 Auth/email/session、真实 share/member permission、server-side pagination、Smart Drawing 浏览器 smoke / 阈值调参、真实 staging Postgres/R2/domain smoke、真实 AI provider/AiRun/cost logs、完整 Admin Analytics、计费和多人协作。

**2026-05-02 上下文加速快照**: 为减少每次小改动的上下文开销，新增并扩展 `ARCH/` 和 `Project_state/` 短上下文层。根目录 `ARCH.md`、`PRD.md`、`project_state.md` 和 `HARNESS.md` 仍是 canonical；`ARCH/00-current-map.md` 现在镜像原 `ARCH.md` 的架构图、分层规则、11.5 并行路线和切片进度，`ARCH/05-data-model-and-api.md` 镜像数据模型/API 总览，其他 `ARCH/*.md` 按 Canvas、Board/Asset、AI、Product/Auth/Admin/Deploy 拆分必要细节；`Project_state/00-current-progress.md` 镜像当前阶段、完成项、未完成项和下一步 fork，`Project_state/current-slice.md` 记录当前关键文件、下一步和质量闸门。之后小 UI polish 可只更新短上下文和 active plan；数据/API/Auth/AI/Admin/Billing/Deploy/协作变化仍同步全套根文档和对应短镜像。

**当前下一刀**: S0J Smart Drawing first pass 已通过前端质量闸门和 recognizer smoke：Canvas Settings 新增 Smart Drawing 开关，画布监听当前用户完成的 draw stroke，高置信度时用本地几何识别替换为普通 `line` / `geo` shape，低置信度保留原始 draw，不调用 AI provider，不保存原始大采样 payload。之后如果外部资源仍未就绪，建议本地继续做 Smart Drawing browser tuning、long-session regression 或 i18n/status polish；如果资源准备好了，则按 `ARCH.md` 11.5-11.7 进入 staging Postgres/R2/domain smoke。
...
| 架构短索引 | `ARCH/README.md` | 快速进入当前架构图和按切片拆分的短文档 |
| 架构总图短镜像 | `ARCH/00-current-map.md` | 当前架构总图、并行开发路线、进度百分比和切片流程 |
| 数据/API 短镜像 | `ARCH/05-data-model-and-api.md` | 数据模型、表关系、API contract 和 migration/permission 规则 |
| 当前短状态 | `Project_state/current-slice.md` | 小 UI polish / 快速接手的当前上下文、下一步和测试命令 |
| 当前进度短镜像 | `Project_state/00-current-progress.md` | 当前阶段、完成项、未完成项、进度百分比和下一步 fork |
| 开发 Harness | `HARNESS.md` | 跨功能开发索引、代码规范、验收标准、接班规则 |
| Dev Plans 索引 | `dev-plans/README.md` | 当前活跃计划与归档计划入口 |
| P0 Harness 路线 | `dev-plans/Archive/p0-development-harness-roadmap-2026-04-30.md` | 已归档；当前切片顺序、Done 标准和交接规则以 `ARCH.md` 11.5-11.7 与 `HARNESS.md` 为准 |
| Asset LOD 主线 | `dev-plans/Asset-lod-roadmap.md` | 当前下一阶段正式路线：Asset model、thumbnail cache、Image / Node LOD、协作前置资产管线 |
| 本地产品壳 + Slice E 收口 | `dev-plans/p0-local-product-shell-and-slice-e-roadmap-2026-05-01.md` | 当前近程队列：Product Shell 5-tab navigation、Landing page / Collection / Account / Settings / Team / Subscription semantic shells、Board save UX、Board History、Workspace Board metadata、Auth scaffold、AI contract scaffold、context split |
...
- ✅ Smart Drawing 已有 first pass：本地几何识别把用户自己的 draw stroke 拟合成普通 line/geo shape，低置信度保留原始 draw；前端 typecheck/lint/build 和 recognizer smoke 已通过；不调用 AI provider，不保存原始大采样 payload，不改变 Board / Asset / AiRun 边界。
...
- ✅ 节点复杂度由 Node Runtime / Node Registry / 节点卡片自包含控制管理，tldraw 只做画布底座和渲染承载；P0 不显示独立左侧 Node Inspector。
...
- ✅ Codex 七十四次架构补充 2026-05-02：按用户提出的 Smart Drawing 需求，补充 `ARCH.md` / `PRD.md` / `HARNESS.md` / local plan：Smart Drawing 作为后续本地画布工具切片，第一版做直线、椭圆、矩形、三角形拟合、低置信度保留原样和 undo；拟合结果必须是普通 tldraw shape，保存/History/Merge 走现有路径。
- ✅ Codex 七十五次画布交互修复 2026-05-02：移除连续绘制 mouseup 后反复出现的浮动 Selection Toolbar，把 Capture to Image Node / Convert to Image Node 动作并入固定左侧属性侧栏；侧栏在普通拖拽/连续绘制时保持展开，只在手型平移或 camera moving 时隐藏。为保持源码上限，先拆出 `CanvasStylePanelGroups.tsx` 和 `CanvasStylePanelSelectionActions.tsx`，`CanvasSpikeStylePanel.tsx` 降到 233 行。
- ✅ Codex 七十六次本地产品化 2026-05-02：实现 Workspace Board Management Panel first pass。Board card 的 Panel 入口现在打开右侧管理抽屉，可改 title、description 和 preset card color，并保留 Open / Copy / Delete 操作；card color 与 description 作为 Board summary metadata 持久化，不进入 Board document。Next local bridge 新增 `local-update`，FastAPI `PATCH /api/v1/boards/{board_id}` 扩展为 metadata update，local-dev / Postgres Board store 和 Alembic migration `20260502_0002_board_metadata.py` 均支持 `description` / `card_color`。
- ✅ Codex 七十七次画布设置产品化 2026-05-02：Canvas Settings 从全局 grid 偏好升级为 per-board lightweight `canvasSettings`。设置面板支持 Dots / Grid / Solid background、background color、pattern color、spacing、Snap Alignment、Snap Strength、Zoom Sensitivity、Edge Color 和 AI Chat Style；Dots 背景改成 Google Stitch 风格的细小低对比度点阵。Board 模式下设置变化会标记 dirty 并进入 autosave / History；restore Board / History 时会恢复对应 `canvasSettings`。
- ✅ Codex 七十八次 Board 管理扩展 2026-05-02：扩展 Workspace Board Management。Board summary metadata 新增 `createdAt`、`isPinned`、`isStarred`、`shareId`、`thumbnailUrl` 和 `visibility`；Next local bridge、FastAPI local-dev/Postgres adapter、Alembic migration `20260502_0003_board_management_metadata.py` 和 persistence tests 已覆盖这些字段。Board Panel 现在支持缩略图 URL / 上传、Board access、Star/Pin、owner / created / last modified / location / object details，以及团队成员 nickname / email / role / access 的管理 scaffold；Board card 三横线菜单支持 Share link、Open in new tab、Star、Pin、Rename、Copy、Details、Make private/public 和 Delete，并保留外部点击关闭菜单。
- ✅ Codex 七十九次 S0I 推进 2026-05-02：实现 captured Board thumbnail first pass。`/boards/:boardId` Board 模式保存时若当前 Board 没有 `thumbnailUrl`，会用 tldraw 导出当前页面为轻量 WebP preview，上传到现有 Asset API，再把返回的 512/256/original URL 写入 Board summary metadata；Board document / History document 不写入 data URL、blob URL 或缩略图 binary。自定义缩略图已存在时不会被 autosave 覆盖。当前剩余是浏览器视觉 smoke、手动刷新缩略图入口、History entry preview 和 staging R2/Postgres smoke。
- ✅ Codex 八十次 S0J 推进 2026-05-02：实现 Smart Drawing first pass。新增 `smartDrawingRecognizer` 解码 tldraw draw segments，识别近似直线、开放曲线、椭圆、矩形和三角形；新增 `useSmartDrawing` 在当前用户完成 draw stroke 后高置信度替换为普通 `line` / `geo` shape，低置信度保留原始 draw，并保持 undo / autosave / History 走现有 tldraw shape 路径。Canvas Settings 新增 Smart Drawing 开关，默认开启。`npm -C apps/web run typecheck`、`lint`、`build`、`git diff --check` 和 recognizer smoke 已通过。当前剩余是浏览器手测、阈值调参和后续 diamond/arrow 等更丰富形状。
- ✅ Codex 八十一次 Board Panel UX 2026-05-02：按用户反馈把 Workspace Board Panel 从右侧 drawer 改成 settings-like 居中管理面板。顶部固定 Copy link / Invite / Open / Close，card footer 入口从 `Panel` 改成 `Manage`；title、description、card color、thumbnail、star/pin、visibility 进入 owner/admin editable guard，editor/viewer 先显示灰态只读 contract；thumbnail section 拆出独立组件，支持 upload 和一键 Remove 回默认 preview，不再要求手动删除 URL。文档同步到 ARCH / PRD / HARNESS / AGENTS / dev-plans。
- ✅ Codex 八十二次 Board UX polish 2026-05-02：继续按手测反馈收紧 Workspace Board 和 Canvas UI。Board Panel 改成图 5 式三栏 layout：左侧 Board Info，中间 Board Identity，右侧 Appearance & Access + Collaboration；Save 固定在顶部，details 移到左侧信息栏，右侧 Members 降低字号；share/copy link 会显示 copied toast；gallery/list card 右上角显示更小更精致的 red pin 与 private/public visibility 图标，pin 不再挤压 Board title；public/private 菜单交替显示并带确认；copy board 保留原 card color；History 面板新增 all/autosave/user saves filter、作者/头像行和 autosave/user save 视觉区分；local-dev/Postgres History retention 测试已改成 autosave 与 user saves 分桶保留；画布主 toolbar 回到上方横向布局并新增 Canvas Settings 齿轮入口；左侧绘图属性栏固定为展开抽屉，空白点击或 Esc 只切换内容不卸载面板，左下角 Navigator 缩放数字可按当前视角 reset 到 100%。
- ✅ Codex 八十三次 Canvas Settings reference polish 2026-05-02：Canvas Settings 按最新参考图改成更紧凑的管理面板：顶部 `Canvas Settings` + `Done` / close，左侧 Canvas / Interaction / Display 分区，右侧四组设置卡片。Background Settings 现在同时暴露 Dots/Grid/Solid、Background Color、Pattern Color 和 Spacing；Editor Behavior / View Controls / Interface 保留 Snap、Smart Drawing、Zoom、Edge Color、Language 和 AI Chat Style。面板宽度、卡片标题、行高和移动端布局已收紧，避免设置页过宽或裁切。
- ✅ Codex 八十四次 Canvas background polish 2026-05-02：修正 Dots/Grid 不应悬浮在绘图元素上方的问题，移除独立 surface overlay，改回使用 tldraw Grid 背景层承载 pattern；同时降低 dots/grid opacity 和 dot radius，让点阵更接近低对比度背景纸纹，不压过线条、图片或节点。
- ✅ Codex 八十五次 Node Inspector removal 2026-05-02：按用户反馈移除独立左侧 Node Inspector。`CanvasNodeInspector` 与 `node-inspector.css` 已从产品路径删除；节点选择时左侧固定绘图属性栏保持普通空状态，不再切换到节点参数面板。节点关键参数、状态、运行摘要和高频操作保留在节点卡片内；未来参数膨胀优先考虑节点内折叠区或专用设置弹窗。
- ✅ Codex 八十六次上下文拆分 2026-05-02：按用户要求把大上下文拆成短索引层，新增 `ARCH/README.md`、`ARCH/00-current-map.md`、`ARCH/01-canvas-runtime.md`、`ARCH/02-board-asset-persistence.md`、`ARCH/03-ai-node-contract.md`、`ARCH/04-product-shell-auth-admin-deploy.md`、`ARCH/Slice-S0-local-polish.md`、`Project_state/README.md`、`Project_state/current-slice.md` 和 `Project_state/handoff-2026-05-02-fast-slices.md`。随后扩展 `ARCH/00-current-map.md`、新增 `ARCH/05-data-model-and-api.md` 与 `Project_state/00-current-progress.md`，把原 `ARCH.md` 的架构图、分层原则、数据/API 总览、并行路线和根 `project_state.md` 的当前阶段同步为短镜像，避免短文档与根文档割裂。`AGENTS.md` / `ARCH.md` / `PRD.md` / `HARNESS.md` / `dev-plans/README.md` 已记录 Fast UI polish 与 Architecture slice 两种工作模式，后续小 UI 改动优先读短上下文，架构/API/权限/AI/部署等变化再同步 canonical 根文档。
- ✅ Codex 八十七次 Board thumbnail polish 2026-05-02：实现手动 Refresh preview 与 History preview thumbnails。`/boards/:boardId` 保存状态条新增 Refresh preview，会强制保存当前 Board、重新捕获轻量 WebP preview 并写入 Board summary metadata，同时把该用户保存写进 History；Snapshot 手动保存会尝试携带当前捕获缩略图。Next local bridge、FastAPI local-dev 和 Postgres history contract 的 create request 均支持可选 `thumbnailUrl`，History list 仍只返回 summary，load 才返回 document；缩略图只保存 Asset URL metadata，不写入 Board / History document。同轮真实浏览器 smoke 覆盖新 Board -> Sample shapes -> Refresh preview -> History preview thumbnails -> Workspace card thumbnail；修复 Next local `local-snapshot` route 漏传 `thumbnailUrl`。
...
2. **Step 1.5 复杂节点与架构裁决**
   ...
   - 当前状态：已通过。五类节点、节点卡片自包含控制、动态端口、类型连线颜色、端口校验、mock graph、60 节点入口已实现；端口数据线已切换为 Node Runtime SVG overlay；Prompt / Image / Analysis / Image Gen mock 数据流已接入；fan-out 与 input auto-replacement 已修；canvas image 与 Image Node 可双向转换；Merge Capture / Screenshot 可生成带预览的 Image Node；多图多节点暴露的图片渲染瓶颈已转入 Asset LOD / Slice E 处理。
...
| 复杂节点越做越大 | Node Runtime + Node Registry + 节点卡片自包含控制；P0 不显示独立 Node Inspector，未来参数膨胀优先用节点内折叠区或专用设置弹窗 |
| Spike 源码文件变大 | 已拆分 `globals.css`、`useArrowPortSnapping.ts`、`CanvasSpikeStylePanel.tsx` 等；当前 `CanvasSpikeToolbar.tsx`、`node-card-content.css`、`CanvasBoardSaveAudit.tsx`、`canvas-action-icons.css`、`smartDrawingRecognizer.ts`、`useEditorRevision.ts`、`assetPreviewResolver.ts`、`boards.css`、`postgres_board_store.py` 超过 250 行，后续触碰这些文件必须优先拆分 |
...
| 属性面板常驻打扰画布操作 | 左侧绘图属性栏固定为侧拉抽屉，只对普通绘图对象显示样式控件；选中节点保持空状态，不显示 Node Inspector |
| card 缩小后文字溢出 | 内容层 line clamp、长 URL 断行、小尺寸折叠；节点参数保持卡片内克制展示 |
...
Slice D 跨平台质量门已 `pass with notes`，不再继续在 Cloudflare Tunnel + `next dev` 临时环境里追求完美。当前 Slice E 已完成本地 Asset / Board bridge、FastAPI local-dev、真实 `s3-compatible` Asset adapter、Postgres persistence adapter、Alembic migration scaffold、Web-to-FastAPI switch、staging API package、`/workspaces` Board gallery/list CRUD、Board History first pass、captured Board thumbnail first pass、Smart Drawing first pass、Auth scaffold first pass 和 AI contract first pass。下一步分成“本地可继续做”和“外部资源就绪后做”两段推进。
...
| L3 | Workspace Board metadata polish | 已完成第一版 + recent-open + management metadata pass + captured thumbnail / manual refresh / History preview browser smoke | 已完成 | Refresh preview / Workspace card / History thumbnail 浏览器 smoke 通过 | thumbnail placeholder/field/upload/remove default、save-time captured thumbnail、Refresh preview、History preview thumbnails、shape/asset counts、`lastOpenedAt`、Recently opened sorting、client-side Load more、settings-like Board Panel `description` / `cardColor` / star / pin / visibility / share / details / member scaffold、owner/admin editable guard、空/错/加载状态；share 权限 / member persistence / server-side pagination 后补 |
...
| L7 | Board History | 已完成第一版 + preview thumbnails | 已完成 | 后端测试与浏览器 thumbnail smoke 已通过 | Autosave、Snapshot 按钮、Cmd/Ctrl+S 进入同一 History restore 时间线，支持 all/autosave/user saves filter、作者显示和 preview thumbnail，local/FastAPI/Postgres history contract，free tier autosave 100 + user saves 100 分桶 retention；Pro/Enterprise 长期历史后补 |
| L8 | Smart Drawing | first pass | 已完成 | 前端闸门和 recognizer smoke 通过，待浏览器 smoke | 当前用户 draw stroke 完成后本地拟合直线、开放曲线、椭圆、矩形、三角形，输出普通 line/geo shape；低置信度保留原 draw；不接 AI provider |
...
当前推荐下一刀：若外部资源就绪，则转入真实 staging server / managed Postgres / R2 / staging Web origin smoke；若外部资源仍未就绪，则继续 Smart Drawing browser tuning、长时回归或 i18n/status polish。
