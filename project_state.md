...
**下一步**: 已提交 checkpoint `eb9ddd6 checkpoint: dashboard metadata and auth scaffold`。当前本地 Product Shell、Board save UX、Workspace Board metadata、Auth scaffold 和 AI contract scaffold 都是 first-pass checkpoint，不是完整产品完成态：`/login`、`/signup`、`/forgot-password`、`/verify-email` 已按 `reference/Design_reference.md` 进入 split-screen Auth visual pass；`/workspaces` 已纠正为 active workspace 内的 Board gallery/list 页面，读取同一 Board persistence list，支持 Gallery/List 切换、搜索、排序、新建 Board、打开、重命名、复制、删除；Settings / Account / Team / Subscription 不再混入 `/workspaces` 主体。App Shell 已改成参考图式顶部导航 + 左侧 workspace sidebar，顶部 5 个标签为 Home、Workspace、Collection、Team、Subscription，分别对应 `/home`、`/workspaces`、`/collections`、`/team`、`/billing`；Account 和 Settings 保留在侧栏语义入口；Upgrade Plan 指向 `/billing`。`/home` 是本地 app home / orientation shell，`/collections` 是未来 Asset / reference library placeholder；`/account` 现在是 mock profile / email status / workspace ownership / session guard / danger zone 的个人账户中心；`/settings` 聚焦 canvas defaults、persistence mode、AI model availability 和 server-secret boundary；`/team` 与 `/billing` 独立 placeholder 明确不接真实邀请、角色、Stripe 或额度。Workspace Board card 已预留底部协作成员、Panel 入口和三横线菜单，菜单含 Rename / Copy board / Delete。`/boards/:boardId` 在 Board 模式会监听画布 document / runtime edges 变化并 debounce 保存，显示 Ready / Unsaved changes / Saving / Saved / Load failed / Save failed 等状态，刷新或关闭、以及浏览器 Back 遇到未保存状态时都会提示；autosave 已从 1600ms 调到 1200ms，并保留 signature 去重，避免无语义 dirty 事件重复保存。Board title 现在会从保存记录回填；Workspace gallery 标题单击打开 Board、双击重命名，画布标题也支持双击重命名，右侧 Open / Rename 操作仍保留；Workspace 标题双击已改成 click-count guard，避免双击先触发单击打开。S0E first pass 已完成：Next `/api/ai/models` / `/api/ai/runs` 和 FastAPI `/api/v1/ai/models` / `/api/v1/ai/runs` 返回 mock Model Registry / AiRun；Image Gen / Image Gen 4 模型下拉通过 AI model client 读取 contract；节点 Run 现在创建 mock AiRun，再把返回的 mock asset ids 写回 runtime summary。未来新增 AI 节点 / AI Chat bot 的扩展规则已收敛到 `ARCH.md` 4.4.1：必须先扩展 Node Registry / Model Registry / AiRun 合同，节点 UI 不直接调用 Provider。真实浏览器复测已覆盖 App Shell 路由语义、`/home` / `/workspaces` / `/collections` / `/account` / `/settings` / `/team` / `/billing` 响应式，以及 Board autosave / rename / Back warning / multi-image paste 保存稳定性；下一刀若外部资源未就绪，继续 Workspace recent-open metadata / pagination，若资源就绪则转真实 staging / R2 / Postgres / domain smoke。真实邮箱验证、OAuth、团队权限、AI provider key、R2/Postgres/domain/staging server 仍等外部资源接入。当前本地开发默认 `dev-user` / `dev-workspace`，Asset metadata 已带 `createdBy` / `workspaceId`，Board record 已带 `ownerId` / `workspaceId`，Postgres driver 已可保存 Board document / summary metadata 和 S3 Asset metadata，Board rename/delete/copy 已接 local/FastAPI/Postgres contract。
...
**0-to-1 总路线**: 详见 `ARCH.md` 11.5-11.7；Sprint 级任务拆分见 `ARCH.md` 11.5.1。当前已完成本地 Product Shell、Board save UX、Workspace Board metadata、Auth scaffold 和 AI contract scaffold 第一版；外部资源就绪后再进入“真实 staging 基础设施 + Auth/Board 产品化 + AI 生图”。多人协作放到 P0.5，必须等 Asset / Board / Auth / AI Run 边界稳定。
...
| 本地产品壳 + Slice E 收口 | `dev-plans/p0-local-product-shell-and-slice-e-roadmap-2026-05-01.md` | checkpoint `5ffed96` 后的近程队列：Product Shell 5-tab navigation、Home / Collection / Account / Settings / Team / Subscription semantic shells、Board save UX、Workspace Board metadata、Auth scaffold、AI contract scaffold |
...
- ✅ Codex 六十八次 App Shell 语义收口 2026-05-01：按用户要求清理 App Shell、Account、Settings、Team、Billing 页面语义。随后按截图反馈把顶部导航改成 5 个标签：Home、Workspace、Collection、Team、Subscription；左侧 sidebar 中 Home 指向 `/home`，Workspace 指向 `/workspaces`，Collections 指向 `/collections`，Team 指向 `/team`，Subscription 指向 `/billing`，Settings 保留为侧栏入口，Account 放到侧栏 footer。新增 `/home` 和 `/collections` placeholder；`/team` 和 `/billing` 独立页面明确不假装真实邀请、角色、Stripe、订阅或额度已接通。`/account` 改成个人账户中心，展示 mock profile、email status、workspace ownership、session guard 和 disabled danger zone；`/settings` 改成 app settings，聚焦 canvas defaults、persistence mode、AI model availability 和 server-secret boundary，并链接到 Team/Subscription。新增 `product-management.css` / `product-management-table.css`，保持单文件低于 250 行。
- ✅ Codex 六十九次浏览器回归 2026-05-01：用本机 Chrome headless 做真实浏览器 smoke，`/home`、`/workspaces`、`/collections`、`/account`、`/settings`、`/team`、`/billing` 均通过桌面/移动端检查：顶部 Home / Workspace / Collection / Team / Subscription 标签、侧栏 active state、移动端菜单 active、页面宽度和横向溢出均正常。Board 长时回归通过：新 Board 插入形状后 autosave 可保存，Workspace 标题重命名后重新进入画布会回填新标题，画布标题双击重命名会同步回 Workspace，未保存状态点击浏览器 Back 会弹确认且取消后留在当前 Board，多图 PNG paste 后保存为 Asset-backed Board document，未出现 `Expected json serializable value` / tldraw asset meta validation error，持久化 document 未含 `data:` / `blob:`。同轮修复 Workspace 标题双击：标题现在用 click-count guard 区分单击打开和双击重命名，避免双击先触发单击进入 Board。
...
Slice D 跨平台质量门已 `pass with notes`，不再继续在 Cloudflare Tunnel + `next dev` 临时环境里追求完美。当前 Slice E 已完成本地 Asset / Board bridge、FastAPI local-dev、真实 `s3-compatible` Asset adapter、Postgres persistence adapter、Web-to-FastAPI switch、staging API package、`/workspaces` Board gallery/list CRUD、Auth scaffold first pass 和 AI contract first pass。下一步分成“本地可继续做”和“外部资源就绪后做”两段推进。

当前本地可做队列：

| 顺序 | 阶段 | 状态 | 预计开发 | 预计测试 | 当前要做 |
|------|------|------|----------|----------|----------|
| L1 | Product Shell | 已完成第一版 + Auth/Workspaces visual pass + 5-tab navigation / management semantics pass | 已完成 | 浏览器 smoke 已通过 | App Shell 顶部 `Home / Workspace / Collection / Team / Subscription`；`/login`、`/signup`、`/forgot-password`、`/verify-email` split-screen Auth surfaces；`/home`、`/workspaces`、`/collections`、`/account`、`/settings`、`/team`、`/billing` 语义分离；mock user/workspace |
...
| L3 | Workspace Board metadata polish | 已完成第一版 | 已完成 | 基础浏览器 smoke 已通过 | thumbnail placeholder/field、shape/asset counts、空/错/加载状态；recent opened 和 richer pagination 后补 |
...
| 4 | Board CRUD 产品化 | search/rename/delete 已接 contract | 2-4 天 | 1-2 天 | `/workspaces` thumbnail、recent/opened metadata、空/错/加载 polish、save indicator、刷新恢复 |
...
当前推荐下一刀：若外部资源就绪，则转入真实 staging server / managed Postgres / R2 / staging Web origin smoke；若外部资源仍未就绪，则继续 Workspace recent-open metadata / richer pagination 和 Board management Panel 入口的轻量产品化。
