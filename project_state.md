...
> 每次开始开发前先判断模式：小 UI polish 读 `Project_state/current-slice.md` → `ARCH/README.md` → 相关 `ARCH/Slice-*.md` → active plan；架构/API/Auth/AI/Admin/Billing/Deploy/协作变化读 `project_state.md` → `PRD.md` → `ARCH.md` → `HARNESS.md` → `dev-plans/README.md` → 当前 slice plan。
> 每次 commit 前更新此文件。
...
**2026-05-02 文档同步快照**: 当前实际架构已经收敛为 Next.js Product Shell + tldraw Canvas Runtime + Next local bridge / FastAPI API 双通道 + Postgres/Object Storage 边界。已落地的本地 first pass 包括 `/workspaces` Board gallery/list、Board Management metadata、per-board Canvas Settings、Board save/autosave、Board History、captured Board thumbnail、Smart Drawing、Auth scaffold、AI contract scaffold、Alembic migration scaffold 和 Admin S0 schema/access planning。尚未完成或不能假装完成的部分是：真实 Auth/email/session、真实 share/member permission、手动刷新/History captured thumbnails、server-side pagination、Smart Drawing 浏览器 smoke / 阈值调参、真实 staging Postgres/R2/domain smoke、真实 AI provider/AiRun/cost logs、完整 Admin Analytics、计费和多人协作。

**2026-05-02 上下文加速快照**: 为减少每次小改动的上下文开销，新增 `ARCH/` 和 `Project_state/` 短上下文层。根目录 `ARCH.md`、`PRD.md`、`project_state.md` 和 `HARNESS.md` 仍是 canonical；`ARCH/README.md`、`ARCH/00-current-map.md`、`ARCH/Slice-S0-local-polish.md` 等文件用于快速定位当前架构切片；`Project_state/current-slice.md` 记录当前关键文件、下一步和质量闸门。之后小 UI polish 可只更新短上下文和 active plan；数据/API/Auth/AI/Admin/Billing/Deploy/协作变化仍同步全套根文档。

**当前下一刀**: S0J Smart Drawing first pass 已通过前端质量闸门和 recognizer smoke：Canvas Settings 新增 Smart Drawing 开关，画布监听当前用户完成的 draw stroke，高置信度时用本地几何识别替换为普通 `line` / `geo` shape，低置信度保留原始 draw，不调用 AI provider，不保存原始大采样 payload。之后如果外部资源仍未就绪，建议本地继续做 Smart Drawing browser tuning、thumbnail 手动刷新/History preview、long-session regression 或 i18n/status polish；如果资源准备好了，则按 `ARCH.md` 11.5-11.7 进入 staging Postgres/R2/domain smoke。
...
| L3 | Workspace Board metadata polish | 已完成第一版 + recent-open + management metadata pass + captured thumbnail first pass | 已完成 | 前端闸门通过，待浏览器复测 | thumbnail placeholder/field/upload/remove default、save-time captured thumbnail、shape/asset counts、`lastOpenedAt`、Recently opened sorting、client-side Load more、settings-like Board Panel `description` / `cardColor` / star / pin / visibility / share / details / member scaffold、owner/admin editable guard、空/错/加载状态；manual refresh / History thumbnails / share 权限 / member persistence / server-side pagination 后补 |
...
| L7 | Board History | 已完成第一版 | 已完成 | 后端测试已通过，待浏览器复测 | Autosave、Snapshot 按钮、Cmd/Ctrl+S 进入同一 History restore 时间线，支持 all/autosave/user saves filter 和作者显示，local/FastAPI/Postgres history contract，free tier autosave 100 + user saves 100 分桶 retention；Pro/Enterprise 长期历史后补 |
...
当前推荐下一刀：若外部资源就绪，则转入真实 staging server / managed Postgres / R2 / staging Web origin smoke；若外部资源仍未就绪，则继续 captured thumbnail 手动刷新/History preview、Smart Drawing browser tuning、长时回归或 i18n/status polish。
