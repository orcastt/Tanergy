...
**2026-05-02 文档同步快照**: 当前实际架构已经收敛为 Next.js Product Shell + tldraw Canvas Runtime + Next local bridge / FastAPI API 双通道 + Postgres/Object Storage 边界。已落地的本地 first pass 包括 `/workspaces` Board gallery/list、Board Management metadata、per-board Canvas Settings、Board save/autosave、Board History、Auth scaffold、AI contract scaffold、Alembic migration scaffold 和 Admin S0 schema/access planning。尚未完成或不能假装完成的部分是：真实 Auth/email/session、真实 share/member permission、captured thumbnail、server-side pagination、Smart Drawing 实现、真实 staging Postgres/R2/domain smoke、真实 AI provider/AiRun/cost logs、完整 Admin Analytics、计费和多人协作。

**当前下一刀**: 先把本轮文档和 dev-plan 索引更新到真实项目状态，并把已被 ARCH/HARNESS 吸收的 harness roadmap 归档。之后如果外部资源仍未就绪，建议本地继续做 `captured Board thumbnail` 或 `Smart Drawing`；如果资源准备好了，则按 `ARCH.md` 11.5-11.7 进入 staging Postgres/R2/domain smoke。
...
- ✅ Smart Drawing 可以加入画布工具链，但先作为 P0.5 canvas polish：本地几何识别，把用户自己的 draw stroke 拟合成普通 line/geo/draw shape；不调用 AI provider，不保存原始大采样 payload，不改变 Board / Asset / AiRun 边界。
...
| Spike 源码文件变大 | 已拆分 `globals.css`、`useArrowPortSnapping.ts`、`CanvasSpikeStylePanel.tsx` 等；当前 `CanvasSpikeToolbar.tsx`、`node-card-content.css`、`CanvasBoardSaveAudit.tsx`、`canvas-action-icons.css`、`useEditorRevision.ts`、`assetPreviewResolver.ts`、`boards.css`、`postgres_board_store.py` 超过 250 行，后续触碰这些文件必须优先拆分 |
...
| L3 | Workspace Board metadata polish | 已完成第一版 + recent-open + management metadata pass | 已完成 | 后端/前端闸门通过，待最终浏览器复测 | thumbnail placeholder/field/upload、shape/asset counts、`lastOpenedAt`、Recently opened sorting、client-side Load more、Board Panel `description` / `cardColor` / star / pin / visibility / share / details / member scaffold、空/错/加载状态；真实 captured thumbnail / share 权限 / member persistence / server-side pagination 后补 |
...
当前推荐下一刀：若外部资源就绪，则转入真实 staging server / managed Postgres / R2 / staging Web origin smoke；若外部资源仍未就绪，则继续真实 captured thumbnail 或 Smart Drawing 实现。
