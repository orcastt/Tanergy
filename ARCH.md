...
**版本**: v0.6
**日期**: 2026-05-01
**状态**: Web 重启方向正式架构；S1.5 与 Asset LOD Slice A-D 已通过，当前主线是 Slice E Real Asset Pipeline + P0 产品壳收口。FastAPI local-dev、真实 `s3-compatible` Asset adapter、Postgres Board / Asset metadata persistence、Web-to-FastAPI switch、staging API package、`/workspaces` Board gallery/list entry shell、Product Shell route skeleton、App Shell / Home / Account / Settings / Collection / Team / Subscription semantic cleanup、Board save UX 第一版、Workspace Board metadata first pass、Auth scaffold first pass 和 AI contract scaffold first pass 已落地；`/workspaces` 已支持 gallery/list、create/open/search/sort/rename/delete、thumbnail placeholder、对象计数和基础空/错/加载状态；Auth split-screen visual pass、`/workspaces` Board gallery/list visual pass 和 management route semantic pass 已按 `reference/Design_reference.md` 完成，顶部导航为 Home / Workspace / Collection / Team / Subscription，`/`、`/dashboard` 和 `/boards` 列表入口都回到 `/workspaces`。下一步先做路由/响应式浏览器 smoke、Board save 长时回归或真实 staging wiring；真实 AI provider 等外部资源后接入。
**对应 PRD**: `PRD.md`
...
| `app/home` | P0 App Home / orientation shell，指向当前 Workspace、Collection 和 Account | 替代 `/workspaces` 的 Board gallery/list 主产品入口，或伪造真实 recents/onboarding |
...
当前状态：Next local bridge 和 FastAPI 已支持 Board validate/list/save/load/rename/delete；`/workspaces` Board gallery/list 和 `/boards/:boardId` entry 已落地，Workspace gallery 已支持 Gallery/List、search、sort、inline rename、delete confirm、thumbnail placeholder、shape/asset count、loading/empty/error first pass。真实 captured thumbnail、recent/opened metadata、richer pagination 和 Alpha 级错误/空状态仍待产品化。
...
- `apps/web/src/app/api/boards/_lib/boardStorageAdapter.ts` 是 Next local Board persistence seam；当前支持 `TANGENT_BOARD_STORAGE_DRIVER=local-dev`，`local-list` 只返回 summary，`local-save` 只返回 summary，`local-load` 才返回 document，`local-rename` / `local-delete` 按 request context 校验 workspace；不支持的 driver 必须明确失败。正式 staging/production Workspace Board gallery 应优先通过 `NEXT_PUBLIC_API_BASE_URL` 走 FastAPI。
- `apps/web/src/components/workspaces/WorkspaceBoardGallery.tsx` 是当前 `/workspaces` Board gallery/list shell；它现在做 summary list、gallery/list、create/open/search/sort/rename/delete/refresh，权限仍必须由后端 request context / future Auth 执行。旧 `BoardDashboard` 组件暂时保留为历史实现，不作为主入口。
...
| Workspace Board polish | thumbnail placeholder/field、shape/asset counts、pagination/list limit、空/错/加载状态 | 完整团队 workspace dashboard 或 Admin analytics | first pass 已完成；多 Board 管理可用，list response 仍只返回 summary，captured thumbnail / recent-open 以后补 |
...
| S0A | 本地 Product Shell | 已完成 | App shell navigation；`/login`、`/signup`、`/forgot-password`、`/verify-email` split-screen auth surfaces；`/workspaces` Board gallery/list；`/dashboard` redirect、`/home`、`/collections`、`/settings`、`/account`、`/team`、`/billing` route skeleton；mock user/workspace 状态 | 路由可访问，Workspace 展示 Board gallery/list，Home/Collection/Account/Settings/Team/Subscription 语义分离，Auth 表单本地校验可用，不接真实邮件/secret |
...
| S0C | Workspace Board metadata polish | 已完成第一版 | thumbnail placeholder/field、shape/asset counts、list limit、空/错/加载状态 | 多 Board 管理可用，list response 仍只返回 summary；captured thumbnail / recent-open metadata / richer pagination 后续补 |
