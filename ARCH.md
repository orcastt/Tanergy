...
**版本**: v0.7
**日期**: 2026-05-02
**状态**: Web 重启方向正式架构；S1.5 与 Asset LOD Slice A-D 已通过，当前主线是 Slice E Real Asset Pipeline + P0 产品壳收口。FastAPI local-dev、真实 `s3-compatible` Asset adapter、Postgres Board / Asset metadata persistence、Web-to-FastAPI switch、staging API package、`/workspaces` Board gallery/list entry shell、Product Shell route skeleton、App Shell / Landing page / Account / Settings / Collection / Team / Subscription semantic cleanup、Board save UX 第一版、Board History first pass、Workspace Board metadata first pass、expanded Board Management metadata first pass、per-board Canvas Settings、Auth scaffold first pass、AI contract scaffold first pass、P0 database migration scaffold 和 Admin S0 schema/access boundary 已落地。`/workspaces` 已支持 gallery/list、create/open/search/sort/rename/delete、thumbnail placeholder/URL/upload、对象计数、`lastOpenedAt` 最近打开排序、Load more 分页、Board Panel title/description/card color/star/pin/visibility/share/details/member scaffold 和基础空/错/加载状态；`/boards/:boardId` 已支持统一 Board History：autosave 成功后写入 `autosave` history entry，Snapshot 按钮写入 `manual` entry，Cmd/Ctrl+S 执行当前 Board save 并写入 `keyboard` entry，History 按时间倒序展示并支持 restore；history list 只返回 summary，load 才返回 document，免费层默认保留最近 100 条。Auth split-screen visual pass、`/workspaces` Board gallery/list visual pass 和 management route semantic pass 已按 `reference/Design_reference.md` 完成，顶部导航为 Landing page / Workspace / Collection / Team / Subscription，Landing page 不放入侧栏，`/`、`/dashboard` 和 `/boards` 列表入口都回到 `/workspaces`。下一步若外部资源未就绪，优先继续真实 captured thumbnail、Smart Drawing、本地长时回归和 i18n/状态 polish；若资源就绪，转真实 staging wiring；真实 Auth、Email、R2/Postgres、AI provider 和 Admin 生产入口等外部资源后接入。
**对应 PRD**: `PRD.md`
...
- Web `/workspaces` 和 `/boards/:boardId` 已是当前 Board gallery / Board canvas entry shell；`/workspaces` 已支持 gallery/list、create/open/search/sort/rename/delete、summary metadata、thumbnail placeholder/URL/upload、star/pin/private/public/share actions、Board Management Panel、member scaffold 和基础状态。`/`、`/dashboard`、`/boards` 列表入口都进入 `/workspaces`；`/spikes/canvas` 仍保留为技术验证入口。
...
Browser
  ...
  │    Inspector / fixed style panel / Canvas Settings
...
| `components/canvas` | tldraw canvas、工具栏、style panel、settings panel、save/history orchestration | 调 Provider API；保存图片二进制；实现真实 billing |
| `components/inspector` | 选中节点的详细参数面板 | 保存 Provider Key 或直接扣费 |
| `components/nodes` | 节点卡片 UI 和端口展示 | 直接访问数据库或 Provider |
...
| `services/api/tangent_api/storage/postgres_board_snapshot_store.py` | 256 | 继续加 retention tier 前拆 SQL helpers |
...
| `apps/web/src/components/workspaces/WorkspaceBoardGallery.tsx` | 245 | 已拆 header / results / toolbar；再加 Workspace 行为前拆 actions hook |
| `apps/web/src/components/workspaces/BoardManagementPanel.tsx` | 245 | 再加 tab/history/member 编辑前拆 thumbnail/details/access subcomponents |
...
当前状态：Next local bridge 和 FastAPI 已支持 Board validate/list/save/load/rename/delete/history create/list/load；`/workspaces` Board gallery/list 和 `/boards/:boardId` entry 已落地，Workspace gallery 已支持 Gallery/List、search、sort、inline rename、delete confirm、thumbnail placeholder、shape/asset count、`lastOpenedAt` 最近打开、Recently opened / Recently saved 排序、client-side Load more、loading/empty/error first pass。Board Management metadata 包含 `description`、preset `cardColor`、`createdAt`、`isPinned`、`isStarred`、`shareId`、editable `thumbnailUrl` 和 `visibility`，只进入 Board summary metadata，不进入 Board document；Panel 展示 owner / created / last modified / location / objects，并包含团队成员 nickname / email / role / access 的 mock 管理 scaffold。`/boards/:boardId` 的统一 Board History 已能保存和恢复 guarded Board document，autosave、Snapshot 和 Cmd/Ctrl+S 都进入同一时间线，免费层默认保留最近 100 条。真实 captured thumbnail、server-side pagination、真实 share link 权限、真实 Board members、企业级 history retention 和 Alpha 级错误/空状态仍待产品化。
...
当前状态：计划中。目标是在现有 tldraw draw 工具上加一层本地几何识别，让用户随手画出的线条、曲线和基础形状可以自动拟合成更干净的 whiteboard shape。该功能不属于 AI provider，不产生服务端成本，不改变 Asset / Board persistence 边界。
...
- 提供工具栏 toggle 或 draw tool option：Smart Drawing on/off。
...
- 矩形 / 三角形 / 菱形：用 Ramer-Douglas-Peucker 简化多段线，按角点数量、角度和闭合误差分类。
- 曲线：第一版可保留为 smoothed draw；后续再考虑转成 bezier-like custom shape。
- 所有阈值放在本地 canvas settings，后续可按手测调优。
...
- P0 retention 只实现 `free` tier，默认每个 Board 保留最近 100 条，可用 `TANGENT_FREE_BOARD_SNAPSHOT_LIMIT` 调整。
...
- 下一步产品化目标：补 captured thumbnail、Smart Drawing 实现、长时浏览器回归和真实 Auth 后的权限错误态。
...
- Board History、per-board Canvas Settings 和 Workspace Board Management Panel 均有本地 first pass：History 统一 autosave / manual Snapshot / Cmd/Ctrl+S，Canvas Settings 保存轻量 `canvasSettings`，Board Panel 管理 title / description / color / thumbnail / star / pin / visibility / details / member scaffold。
...
```text
     ...
     |                                    |                                    |
+----v-------------+                +-----v-------------+                +-----v----------+
| Local UX [75%]   |                | Data/Backend [35%]|                | AI/Node [30%] |
...
| Board History [85%]  |          | Alembic scaffold [80%] |          | AiRun / logs [20%]     |
| Canvas Settings [85%]|          | Auth boundary [35%]    |          | AI Chat planner [10%]  |
| Board Mgmt [75%]     |          | staging Postgres [0%]  |          | image models [25%]     |
| Captured thumb [0%]  |          | R2 / domains [0%]      |          +--------+---------------+
| Smart Drawing [0%]   |          +-------+----------------+                   |
+----+-----------------+                  |                                    |
     |                                    |                                    |
     +-----------------------+------------+------------+-----------------------+
...
```text
S0I Captured Board Thumbnail [0%]
  canvas capture
      -> upload as Asset / thumbnail metadata
      -> board.thumbnailUrl summary
      -> Workspace card renders real preview
      -> browser smoke: save / refresh / fallback placeholder

S0J Smart Drawing [0%]
  draw stroke
      -> local recognizer
      -> confidence check
      -> convert to normal line/geo OR keep original stroke
      -> undo + autosave + History restore smoke
...
| Board History | Autosave、Snapshot 按钮、Cmd/Ctrl+S 进入同一 History 时间线；History summary list、Restore guarded document、free tier 最近 100 条 retention | Pro/Enterprise 长期历史、diff preview、object-storage history body、多人协作冲突历史 | 第一版已完成；本地/FastAPI/Postgres history create/list/load contract 可测；list 不返回 document；restore 后当前 Board 被标记 dirty 并可 autosave |
| Per-board Canvas Settings | Dots/Grid/Solid background、background/pattern color、spacing、snap strength、zoom sensitivity 等轻量设置 | 复杂主题系统、团队模板、服务端权限规则 | 第一版已完成；设置存入 Board document `canvasSettings`，History restore 可恢复，不保存图片或大 payload |
| Workspace Board polish | thumbnail placeholder/field/upload、shape/asset counts、recent-open metadata、Panel description/card color/star/pin/visibility/share/details、card menu actions、成员管理 scaffold、pagination/list limit、空/错/加载状态 | 完整团队 workspace dashboard、真实 share permission、真实 Board members 或 Admin analytics | first pass 已完成；多 Board 管理可用，list response 仍只返回 summary，真实 captured thumbnail / server-side pagination 以后补 |
...
| Smart Drawing | 本地几何识别，把用户画出的 stroke 拟合成 line/geo/draw shape；低置信度保留原样，可 undo | AI provider 参与、保存原始大采样点、改变 Board/Asset/AiRun 边界 | 计划已记录；实现时先拆 toolbar / recognizer / settings，输出普通 tldraw shape |
...
| S0C | Workspace Board metadata polish | 已完成第一版 + recent-open + management metadata pass | thumbnail placeholder/field/upload、shape/asset counts、`lastOpenedAt`、Recently opened sorting、client-side Load more、Panel `description` / `cardColor` / star / pin / visibility / share id / created details、成员管理 scaffold、空/错/加载状态 | 多 Board 管理可用，list response 仍只返回 summary；真实 captured thumbnail、真实 share permission、真实 member persistence / server-side pagination 后续补 |
...
| S0F | Board History | 已完成第一版 | Autosave、Snapshot 按钮、Cmd/Ctrl+S 进入同一 History list/restore；Next/FastAPI/Postgres history contract、free tier latest-100 retention | history document 过 Board guard；list 只返回 summary，load 才返回 document；长期 retention / plan tiers 后续补 |
| S0G | Per-board Canvas Settings | 已完成第一版 | Dots/Grid/Solid background、background/pattern color、spacing、snap strength、zoom sensitivity、edge/chat style 轻量设置 | 设置保存到 `canvasSettings` 并随 Board/History restore；不写入重型 UI 或图片 payload |
| S0H | Workspace Board Management expansion | 已完成第一版 | Board Panel title/description/card color/thumbnail URL/upload、star/pin、visibility、share id、details、member scaffold；card menu share/open/star/pin/rename/copy/details/private/public/delete | Metadata 持久化在 Board summary；真实 share permission、真实 member persistence、captured thumbnail 后续补 |
| S0I | Captured Board Thumbnail | 待做，本地优先 | 保存或手动刷新时生成轻量 Board preview，Workspace card 展示真实缩略图 | 缩略图走 Asset/metadata，不进入 Board document；失败有 placeholder fallback |
| S0J | Smart Drawing | 待做，本地优先 | draw stroke 几何拟合为 line/geo/draw shape，支持开关、置信度阈值、undo | 输出普通 tldraw shape；不调用 AI provider；不保存原始大采样 payload |
...
- `dev-plans/p0-local-product-shell-and-slice-e-roadmap-2026-05-01.md`：checkpoint `5760541` 之后的本地 Product Shell + Slice E 收口队列。
