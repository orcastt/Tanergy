...
**版本**: v0.6
**日期**: 2026-05-01
**状态**: Web 重启方向正式架构；S1.5 与 Asset LOD Slice A-D 已通过，当前主线是 Slice E Real Asset Pipeline + P0 产品壳收口。FastAPI local-dev、真实 `s3-compatible` Asset adapter、Postgres Board / Asset metadata persistence、Web-to-FastAPI switch、staging API package、`/boards` entry shell、Product Shell route skeleton 和 Board save UX 第一版已落地；`/boards` 已支持 list/create/open/search/rename/delete。下一步先在本地补 Dashboard metadata、Auth scaffold 和 AI contract scaffold，同时准备真实 staging 资源。
**对应 PRD**: `PRD.md`
...
- Web `/boards` 和 `/boards/:boardId` 已是当前 Dashboard / Board entry shell；Dashboard 已支持 list/create/open/search/rename/delete。`/spikes/canvas` 仍保留为技术验证入口。
...
| `apps/web/src/components/canvas/CanvasBoardSaveAudit.tsx` | 291 | 拆 board save actions / status display / autosave hooks |
...
当前状态：尚未实现正式登录；现阶段只有 `apiRequestContext` / FastAPI request context 的 dev fallback 和显式 `x-tangent-user-id` / `x-tangent-workspace-id` 开发头。`/login`、`/signup`、`/forgot-password`、`/verify-email` 的产品壳、表单校验、mock session/workspace 和 App Shell 已落地，但不接真实邮件、OAuth、密码存储或团队权限。生产前必须替换为真实 session/JWT。
...
当前状态：Next local bridge 和 FastAPI 已支持 Board validate/list/save/load/rename/delete；`/boards` Dashboard shell 和 `/boards/:boardId` entry 已落地，Dashboard 已支持 search / inline rename / delete confirm。缩略图、recent/opened metadata、pagination 和完整错误/空状态仍待产品化。
...
当前状态：五类节点、runtime edge store、fan-out、input replacement、mock data flow、Image Node 双向转换、Merge Capture 到 Image Node 已在 canvas spike 中通过；真实 AI Run 和 Model Registry 尚未接入。
...
当前状态：仅有架构合同和数据模型，尚未实现真实 provider 调用、AiRun table、成本记录或限流。下一步应在 Auth / Board 边界稳定后落地最小 Prompt → Image Gen / Image Gen 4 → Asset → Image Node。
...
当前状态：仅为架构合同；正式模型列表、能力 schema、启用状态和后端二次校验尚未实现。
...
当前实现字段：FastAPI / Next bridge 的 Board record 当前包含 `id`、`workspaceId`、`ownerId`、`title`、`document`、`byteSize`、`savedAt`；list/save response 只返回 summary，不返回完整 `document`。下面字段是进入正式数据库后的目标形状。
...
当前状态：计划中，尚未建表。真实 AI 生图接入前必须先落 AiRun / ApiCallLog，避免 provider 调用无法追踪成本、状态和失败原因。
...
当前状态：计划中。下列字段是服务端 Model Registry contract；具体模型 id、价格和能力必须以接入时 provider 官方/代理返回为准，不能在前端硬编码为最终真相。
...
当前状态：尚未实现 route。下面是目标 API contract，真实模型 id 和参数 schema 必须由服务端从 provider/配置中返回。
...
当前状态：尚未实现 route。必须等 Model Registry、Auth context、Asset persistence 和成本/限流策略就绪后接入。
...
| Dashboard polish | thumbnail placeholder/field、recent saved/opened metadata、pagination/list limit、空/错/加载状态 | 完整团队 workspace dashboard 或 Admin analytics | 多 Board 管理可用，list response 仍只返回 summary |
| Auth scaffold | session/workspace 类型、mock current-user boundary、route guard 形状、Auth-required dev smoke | 真实 provider、cookie 安全策略最终落地、邮箱域名信誉 | dev fallback 不阻塞开发；开启 auth-required 检查时缺 context 会明确失败 |
...
| AI contract scaffold | mock Model Registry、`AiRun` schema draft、server-only AI proxy stub、mock provider response | 真实 provider key、真实扣费、预算熔断最终值 | 前端模型选择可走服务端合同；没有真实 key 也能测试 UI |
...
| S0C | Dashboard metadata polish | 1-2 天 | thumbnail placeholder/field、recent saved/opened metadata、pagination/list limit、空/错/加载状态 | 多 Board 管理可用，list response 仍只返回 summary |
| S0D | Auth scaffold boundary | 2-3 天 | current-user/session/workspace 类型和 mock boundary；route guard 形状；auth-required dev smoke | dev fallback 不阻塞开发，开启 auth-required 检查时缺 context 会明确失败 |
| S0E | AI contract scaffold | 2-3 天 | mock Model Registry、`AiRun` schema draft、server-only AI proxy stub、mock provider response | 模型选择不再依赖组件硬编码最终真相；没有真实 key 也能跑 UI |
