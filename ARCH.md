...
**版本**: v0.6
**日期**: 2026-05-01
**状态**: Web 重启方向正式架构；S1.5 与 Asset LOD Slice A-D 已通过，当前主线是 Slice E Real Asset Pipeline。FastAPI local-dev、真实 `s3-compatible` Asset adapter、Postgres Board / Asset metadata persistence、Web-to-FastAPI switch、staging API package 和 `/boards` entry shell 已落地；下一步接真实 staging 资源、Auth / Board CRUD 和 AI Run。
**对应 PRD**: `PRD.md`
...
- Web `/boards` 和 `/boards/:boardId` 已是当前 Dashboard / Board entry shell；`/spikes/canvas` 仍保留为技术验证入口。
...
| `apps/web/src/app/styles/boards.css` | 255 | 拆 dashboard table / action form 样式 |
...
当前状态：尚未实现正式登录；现阶段只有 `apiRequestContext` / FastAPI request context 的 dev fallback 和显式 `x-tangent-user-id` / `x-tangent-workspace-id` 开发头。生产前必须替换为真实 session/JWT。
...
当前状态：`/spikes/canvas` 是成熟技术验证入口，`/boards/:boardId` 已复用同一 canvas runtime 作为产品 entry shell。后续要把 dev controls 渐进替换成产品化 save indicator / autosave / Board status。
...
- 当前阶段：`CanvasBoardSaveAudit` 提供手动 Save audit / Save board / Load board 验证；`/boards/:boardId` 已按 board id 保存/加载同一 document。
- 产品化目标：用户操作后 debounce 保存 Board document，并显示 save indicator。
...
- `/boards` Dashboard shell 和 `/boards/:boardId` entry 已开始接入同一 persistence contract。
...
1. 先做阶段 1-2，让部署通路真实可用。
2. 再做阶段 3-4，让每个 Board 归属真实用户和 workspace。
3. 然后做阶段 5-6，接入真实 AI 能力。
4. 阶段 7-8 是 Alpha 前的硬门，不要跳过。
5. 阶段 9 多人协作必须等 Asset / Board / Auth / AI Run 边界稳定后再做。
...
- S1-S4 必须先完成，否则后续 Auth / AI / Alpha 没有真实验收环境。
