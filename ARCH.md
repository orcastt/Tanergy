...
**版本**: v0.6
**日期**: 2026-05-01
**状态**: Web 重启方向正式架构；S1.5 与 Asset LOD Slice A-D 已通过，当前主线是 Slice E Real Asset Pipeline + P0 产品壳收口。FastAPI local-dev、真实 `s3-compatible` Asset adapter、Postgres Board / Asset metadata persistence、Web-to-FastAPI switch、staging API package、`/workspaces` Board gallery/list entry shell、Product Shell route skeleton、App Shell / Landing page / Account / Settings / Collection / Team / Subscription semantic cleanup、Board save UX 第一版、Board History first pass、Workspace Board metadata first pass、Auth scaffold first pass、AI contract scaffold first pass 和 P0 database migration scaffold 已落地；`/workspaces` 已支持 gallery/list、create/open/search/sort/rename/delete、thumbnail placeholder、对象计数、`lastOpenedAt` 最近打开排序、Load more 分页和基础空/错/加载状态；`/boards/:boardId` 已支持统一 Board History：autosave 成功后写入 `autosave` history entry，Snapshot 按钮写入 `manual` entry，Cmd/Ctrl+S 执行当前 Board save 并写入 `keyboard` entry，History 按时间倒序展示并支持 restore；history list 只返回 summary，load 才返回 document，免费层默认保留最近 100 条。Auth split-screen visual pass、`/workspaces` Board gallery/list visual pass 和 management route semantic pass 已按 `reference/Design_reference.md` 完成，顶部导航为 Landing page / Workspace / Collection / Team / Subscription，Landing page 不放入侧栏，`/`、`/dashboard` 和 `/boards` 列表入口都回到 `/workspaces`。下一步若外部资源未就绪，继续 captured thumbnail / Board management Panel；若资源就绪，转真实 staging wiring；真实 AI provider 等外部资源后接入。
**对应 PRD**: `PRD.md`
...
- Web `/workspaces` 和 `/boards/:boardId` 已是当前 Board gallery / Board canvas entry shell；`/workspaces` 已支持 gallery/list、create/open/search/sort/rename/delete、summary metadata、thumbnail placeholder 和基础状态。`/`、`/dashboard`、`/boards` 列表入口都进入 `/workspaces`；`/spikes/canvas` 仍保留为技术验证入口。
...
├── reference/
│   ├── Design.md
│   ├── Design_reference.md        # extracted Stitch page reference and route mapping
│   ├── stitch_canvas_reference/   # active page reference source
│   └── stitch_tangent_ai_canvas.zip
├── deploy/
│   └── staging/
│       ├── README.md
│       ├── api.env.example
...
| `app/boards` | `/boards/:boardId` 画布入口；`/boards` 列表入口重定向到 `/workspaces` | 恢复旧 table dashboard 作为主入口；替代 Auth、完整 Board CRUD 或权限模型 |
| `app/home` | P0 Landing page / orientation shell，指向当前 Workspace、Collection 和 Account | 替代 `/workspaces` 的 Board gallery/list 主产品入口，或伪造真实 recents/onboarding |
| `app/login` / `app/signup` / `app/forgot-password` / `app/verify-email` | P0 Auth route shell；当前是 full-screen split-screen Quiet Editorial 表单、mock 状态和未来 session boundary | 直接发送真实邮件、保存密码、把 provider key 放前端 |
| `app/workspaces` | Active workspace 的 Board gallery/list，读取 Board persistence summary，支持 Gallery/List、搜索、排序、新建、打开、重命名、删除 | 混入 Settings / Account / Team / Subscription；替代真实 workspace membership |
| `app/collections` | P0 Collection placeholder，预留未来 Asset library / references 路由 | 提前实现完整素材库 CRUD，或把 Board gallery 搬进 Collection |
| `app/account` | P0 个人账户中心，展示 mock profile、email status、workspace ownership、session guard 和 disabled danger zone | 提前实现真实 profile mutation、数据导出/删除闭环或 token 签发 |
| `app/settings` | P0 App settings 页面，聚焦 canvas defaults、persistence mode、AI model availability 和 server-secret boundary | 混入团队权限、计费、Stripe、Provider secret 编辑或 Admin |
| `app/team` / `app/billing` | P0 独立 placeholder 页面，保持 Team/Subscription 语义清晰但不假装真实邀请、角色、订阅或额度已接通 | 路由回 `/workspaces`、伪造真实团队/计费状态、提前接 Stripe 或权限矩阵 |
...
| `apps/web/src/components/canvas/CanvasBoardSaveAudit.tsx` | 294 | 继续加保存/历史行为前必须先拆 save/history orchestration |
| `apps/web/src/components/boards/BoardDashboard.tsx` | 267 | 拆 row / empty / loading helpers |
| `services/api/tangent_api/storage/postgres_board_store.py` | 282 | 拆 SQL row mapping / query helpers |
| `services/api/tangent_api/schemas.py` | 276 | 拆 board / asset / AI schemas |
...
当前状态：Next local bridge 和 FastAPI 已支持 Board validate/list/save/load/rename/delete/history create/list/load；`/workspaces` Board gallery/list 和 `/boards/:boardId` entry 已落地，Workspace gallery 已支持 Gallery/List、search、sort、inline rename、delete confirm、thumbnail placeholder、shape/asset count、`lastOpenedAt` 最近打开、Recently opened / Recently saved 排序、client-side Load more、loading/empty/error first pass。`/boards/:boardId` 的统一 Board History 已能保存和恢复 guarded Board document，autosave、Snapshot 和 Cmd/Ctrl+S 都进入同一时间线，免费层默认保留最近 100 条。真实 captured thumbnail、server-side pagination、企业级 history retention 和 Alpha 级错误/空状态仍待产品化。
...
当前实现字段：FastAPI / Next bridge 的 Board record 当前包含 `id`、`workspaceId`、`ownerId`、`title`、`document`、`byteSize`、`savedAt`、`lastOpenedAt`、`shapeCount`、`assetCount`、`thumbnailUrl`；list/save response 只返回 summary，不返回完整 `document`。FastAPI Postgres driver 已把 `shape_count` / `asset_count` / `thumbnail_url` / `last_opened_at` 存入 `tangent_boards`。`services/api/migrations/` 已有第一版 P0 core schema migration；下面字段是进入正式数据库后的目标形状。
...
- 下一步产品化目标：补长时浏览器回归、Workspace Board metadata polish 和真实 Auth 后的权限错误态。
...
- `apps/web/src/components/workspaces/WorkspaceBoardGallery.tsx` 是当前 `/workspaces` Board gallery/list shell；它现在做 summary list、gallery/list、create/open/search/sort/rename/delete/refresh/recent-open/Load more，权限仍必须由后端 request context / future Auth 执行。旧 `BoardDashboard` 组件暂时保留为历史实现，不作为主入口。
...
- `/workspaces` Board gallery/list 和 `/boards/:boardId` entry 已接入同一 persistence contract，并支持 list/create/open/search/sort/rename/delete。
...
| Board History | Autosave、Snapshot 按钮、Cmd/Ctrl+S 进入同一 History 时间线；History summary list、Restore guarded document、free tier 最近 100 条 retention | Pro/Enterprise 长期历史、diff preview、object-storage history body、多人协作冲突历史 | 本地/FastAPI/Postgres history create/list/load contract 可测；list 不返回 document；restore 后当前 Board 被标记 dirty 并可 autosave |
| Workspace Board polish | thumbnail placeholder/field、shape/asset counts、recent-open metadata、pagination/list limit、空/错/加载状态 | 完整团队 workspace dashboard 或 Admin analytics | first pass 已完成；多 Board 管理可用，list response 仍只返回 summary，captured thumbnail / server-side pagination 以后补 |
...
| 4. Board CRUD 产品化 | Workspace Board gallery 从 shell 变成可用产品入口 | New / list / gallery / open / rename / delete / search / sort、空状态、加载/错误状态、autosave 或 save indicator | 2-4 天 | 1-2 天 | 用户能创建多个 Board，刷新/重开不丢图、不丢节点和连线 |
...
1. 外部资源未就绪时，先做本地 Product Shell、Board save UX、Workspace Board metadata、Auth scaffold 和 AI contract scaffold。
...
| S0C | Workspace Board metadata polish | 已完成第一版 + recent-open pass | thumbnail placeholder/field、shape/asset counts、`lastOpenedAt`、Recently opened sorting、client-side Load more、空/错/加载状态 | 多 Board 管理可用，list response 仍只返回 summary；captured thumbnail / server-side pagination 后续补 |
...
| S9 | 4. Workspace Board 产品化 | 2-3 天 | `/workspaces` 加搜索、排序、重命名、删除确认、空/错/加载状态、recent saved time、thumbnail placeholder | 用户可管理多个 Board；删除/重命名刷新后正确 |
