...
跨功能执行规范见 `HARNESS.md`。
短上下文架构索引见 `ARCH/README.md`；当前切片短状态见 `Project_state/current-slice.md`，短进度镜像见 `Project_state/00-current-progress.md`。
...
- 2026-05-02 已新增并扩展 `ARCH/` 和 `Project_state/` 短上下文层：`ARCH/00-current-map.md` 镜像本章总图与 11.5 路线，`ARCH/05-data-model-and-api.md` 镜像数据/API 总览，`Project_state/00-current-progress.md` 镜像当前状态。未来小 UI polish 优先读短索引，根文档只在架构/API/数据/权限/部署/AI 等边界变化时同步。
...
- 下一步产品化目标：补 Smart Drawing 浏览器 smoke / 阈值调参、长时浏览器回归和真实 Auth 后的权限错误态。
...
S0I Captured Board Thumbnail [65%]
  canvas capture
      ...
      -> manual Refresh preview + History thumbnails [browser smoke passed]
...
| Workspace Board polish | thumbnail placeholder/field/upload/captured preview、manual Refresh preview、History preview thumbnails、shape/asset counts、recent-open metadata、settings-like Panel description/card color/star/pin/visibility/share/details、top Copy link / Invite、thumbnail remove-to-default、owner/admin editable guard、card menu actions、成员管理 scaffold、pagination/list limit、空/错/加载状态 | 完整团队 workspace dashboard、真实 share permission、真实 Board members 或 Admin analytics | first pass 已完成；Refresh preview / Workspace card / History thumbnail 浏览器 smoke 已通过；list response 仍只返回 summary，server-side pagination 后补 |
...
| S0C | Workspace Board metadata polish | 已完成第一版 + recent-open + management metadata + captured thumbnail pass | thumbnail placeholder/field/upload/captured preview、manual Refresh preview、History preview thumbnails、shape/asset counts、`lastOpenedAt`、Recently opened sorting、client-side Load more、settings-like Panel `description` / `cardColor` / star / pin / visibility / share id / created details、thumbnail remove-to-default、成员管理 scaffold、空/错/加载状态 | 多 Board 管理可用，list response 仍只返回 summary；真实 share permission、真实 member persistence / server-side pagination 后续补 |
...
| S0H | Workspace Board Management expansion | 已完成第一版 | Board Panel 已改成 settings-like 居中管理面板；顶部 Copy link / Invite / Open / Save，左侧放 created/modified/opened/location/object 信息，右侧只放 Members；title/description/card color/thumbnail URL/upload/remove、star/pin、visibility、share id、details、member scaffold；owner/admin 可编辑，editor/viewer 灰态；card menu share/open/star/pin/rename/copy/manage/private/public/delete，public/private 交替显示并确认；gallery/list card 右上角显示红色 pin 和 visibility 图标，share/copy link 有 copied toast | Metadata 持久化在 Board summary；真实 share permission、真实 member persistence 后续补 |
| S0I | Captured Board Thumbnail | 浏览器 smoke 通过 | 保存时若 Board 没有自定义 thumbnail，则生成轻量 WebP Board preview；Refresh preview 可手动强制更新；History 条目可显示 `thumbnailUrl` preview；缩略图上传为 Asset，Workspace card / History list 展示真实缩略图 | 缩略图走 Asset/metadata，不进入 Board / History document；失败有 placeholder fallback；后续补长时回归和 staging R2 smoke |
...
- `ARCH/00-current-map.md`：当前架构总图、并行路线和进度百分比短镜像。
- `ARCH/05-data-model-and-api.md`：数据模型和 API contract 短镜像。
- `project_state.md`：当前状态和下一步。
- `Project_state/README.md`：短状态索引；小切片优先读 `Project_state/current-slice.md` 和 `Project_state/00-current-progress.md`。
...
```text
如果是小 UI polish，先读 AGENTS.md、Project_state/current-slice.md、Project_state/00-current-progress.md、ARCH/README.md 和相关 ARCH slice；如果涉及数据/API/Auth/AI/Deploy/Admin/Billing/协作，先读 project_state.md、PRD.md、ARCH.md、HARNESS.md、ARCH/05-data-model-and-api.md 和 dev-plans/README.md。不要读 legacy，不要读 .env。
```
