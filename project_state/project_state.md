...
2026-05-20 fullstack acceptance result: the repo-level local security gate is green. `security_release_gate.py --env-file deploy/staging/api.env.example` passed Web security guards, high-severity npm audit, Web typecheck/lint/build, 28 Playwright public security E2E checks, API compileall, API performance smoke, backend tests, deploy config smoke and `git diff --check`; the latest direct backend run is `373 passed`. After the `b35adc0` API staging redeploy and the follow-up Vercel deployment `dpl_CwARDUa1WkLxDbnZLjrZkHppATMg`, public `ops_readiness_smoke.py` is also green for Web/API TLS, Web headers, static cache, API `/health` headers and CORS. Detailed report: `docs/fullstack-security-acceptance-2026-05-20.md`.

2026-05-20 staging comparison result: the previous stage had restored Web/API reachability and some TLS/CORS checks, but the GeekAI API deploy first failed on Alembic `0033` QwQ pricing FK order and the public ops smoke still caught stale security headers. The current stage runs API release `b35adc0`, reports Alembic `20260520_0033 (head)`, returns `Cache-Control: no-store` on `/health`, passes the public ops smoke, and verifies enabled/healthy GeekAI control-plane routes for QwQ text, Qwen VL analysis, GPT Image 2, Nano Banana 2 and Doubao Seedream.
...
2026-05-20 Vercel env/staging text smoke: staging Web initially returned `503 Missing GEEKAI text API key` on `/api/ai/chat/completions` because Vercel Production env still only had the older `JIEKOU_*` keys. Server-only `GEEKAI_TEXT_*`, `GEEKAI_BALANCE_IMAGE_*`, `GEEKAI_OFFICIAL_IMAGE_*` and `GEEKAI_VIDEO_*` env names were synced to Vercel without printing secret values, then Web was redeployed to `dpl_CwARDUa1WkLxDbnZLjrZkHppATMg`. A direct Chat SSE smoke now returns `200 text/event-stream` and multiple `data:` chunks for `qwq-plus-latest`. QwQ emits `reasoning_content` before final `content`, so the transport is live even if visible node text waits until ordinary content chunks arrive.

2026-05-20 post-stage local regression fixes: Board title validation is strict across frontend/local/FastAPI paths; admin and workspace/invite mutations now use Bearer-first or same-origin proxy forwarding so strict Origin/CSRF checks can remain enabled; Chat, Prompt Optimizer and local Analysis stream through the chat-completions proxy; Nano Banana 2 provider outputs persist Assets from byte-detected MIME/dimensions instead of trusting wrapper MIME; active collaborator movement keeps editor identity visible during transforms; canvas pan ignores stale preview camera snapshots; and tooltip/hover state is cleared across drag/resize/move boundaries. These fixes still need redeploy plus signed-in staging browser smoke before being marked live-accepted.

API redeploy caveat: the current workstation cannot SSH to the Hetzner API host (`Permission denied (publickey)` for `deploy@5.78.122.74`), so FastAPI container redeploy from the uncommitted local backend changes is still pending. Treat API-side fixes, especially byte-detected provider MIME persistence and backend Board-title enforcement, as locally tested but not live-accepted until the deploy key/CI path is restored.
...
S1B Deploy/staging            91%  rebuilt Hetzner API host, public HTTPS API, Supabase Pro fresh schema, historical R2/board smoke, CORS, the Konva-only staging web deploy and the current public ops readiness smoke are green; the live env no longer depends on the retired dirty worktree, Web/API env templates plus API observability are in place, first signed-in browser and most second-round board checks are green, the `Manage board -> Copy board` Free-plan modal path is wired locally, and the remaining work is the staging modal spot check, R2 clean asset smoke, re-created staging admin/workspace/board data, Google/email flow verification, one live AI smoke and external uptime/backup/error-tracking proof
...
S4 Collaboration              35%  local Yjs room/document foundation, bounded websocket queued-update reconnect semantics, low-rate draft drawing presence preview, final-snapshot backend persistence and post-stage mover-identity/tooltip/camera fixes exist; Redis backplane, provider-grade multi-instance reconnect/resync, TTL locks, real two-user staging smoke and deeper conflict semantics still remain
```

## Current Execution Order

1. Restore API deploy access or CI, redeploy the FastAPI side of the post-stage fixes, then rerun signed-in Board browser spots for Board title rejection, admin edits, workspace invite generate/revoke, `Manage board -> Copy board` Free-plan modal, Chat node resize/tooltip cleanup and canvas drag smoothness.
2. Run a two-user collaboration smoke for invite accept/reopen and active mover identity during transform gestures.
3. Finish Google/email plus CORS/origin acceptance.
4. Run one real server-backed image smoke through the refreshed GeekAI S2 image lane, including Nano Banana 2 image display/MIME verification.
5. Return to S1D/S3 closeout: permission edges, payer visibility, billing language and staged payment/usage truth.
6. Finalize public policy/operator details and smoke the backend-backed `/pricing` catalog path before any live paid launch.
7. Resume S4 websocket/provider deepening only after the gates above are green.
...
2026-05-20 全栈验收结果：repo 级本地安全闸门已转绿。`security_release_gate.py --env-file deploy/staging/api.env.example` 已通过 Web security guards、高危 npm audit、Web typecheck/lint/build、28 条 Playwright public security E2E、API compileall、API performance smoke、367 条 backend tests、deploy config smoke 和 `git diff --check`。`b35adc0` staging redeploy 后，公开 `ops_readiness_smoke.py` 也已对 Web/API TLS、Web security headers、Next static cache、API `/health` headers 和 CORS 转绿。详细报告见 `docs/fullstack-security-acceptance-2026-05-20.md`。
...
与此同时，画布现在也有了一条通过 Web app 接入 GeekAI-first 的本地 fast path，用来验证用户流程：Chat 可以流式输出文本，Prompt Optimizer 可以流式优化出图提示词，Analysis 也走同一条 chat-completions 流式代理承接视觉分析 UX，Image Gen / Image Gen 4 现在已经对齐 GPT Image 2、Nano Banana 2、Doubao Seedream 5.0 Lite 的参数界面。这对产品验证有价值，但还不是生产权限边界；2026-05-20 又补上了后端 `20260520_0032_geekai_provider_routes`，把 active provider defaults 切回 GeekAI，并把长耗时生图超时保持在 240s。Nano Banana 2 / GeekAI 图片输出现在按图片 magic bytes 决定落库 MIME 和尺寸，不再信任 provider wrapper MIME。下一步 S2 cut 仍然是把这条 GeekAI-first 路径收口到 `dev-plans/s2-ai-provider-route-billing-control-plane-2026-05-07.md` 里定义的 server AiRun provider-route/billing control plane。
...
S1B Deploy/staging            91%  重建后的 Hetzner API、public HTTPS API、Supabase Pro fresh schema、历史 R2/board smoke、CORS、Konva-only staging Web deploy 和当前公开 ops readiness smoke 已转绿；Web/API env 模板、Sentry 可选接入、API 慢响应/RSS 观测和事故手册已存在；signed-in browser 首轮和大部分第二轮 Board 检查已转绿，`Manage board -> Copy board` Free-plan limit 弹窗路径已完成本地 wiring，剩余工作是 staging 弹窗 spot check、R2 clean asset smoke、重新创建 staging admin/workspace/board 数据、Google/email、一条 live AI smoke，以及外部 uptime/backup/error-tracking 证明
...
S4 Collaboration              35%  本地 Yjs room/document 基础、reconnect/resync smoke harness、低频 draft drawing presence preview、后端 final snapshot persistence，以及 post-stage 移动者身份/tooltip/camera 修复已存在；Redis backplane、TTL locks、多实例 Yjs/provider proof 和真实两用户 staging smoke 仍后置
```

## 当前主线顺序

1. 先把 post-stage 本地修复重新部署到 staging。
2. 再做小范围 signed-in Board browser spot check：Board title 特殊字符拒绝、admin edits、workspace invite generate/revoke、`Manage board -> Copy board` Free-plan limit 弹窗、Chat node resize/tooltip cleanup 和画布拖拽顺滑度。
3. 跑两用户协同 smoke：invite accept/reopen，以及用户 A 移动/缩放/旋转时用户 B 页面能持续看到 A 的 editor label/icon。
4. 再完成 Google/email 与 CORS/origin 验收。
5. 跑通一条刷新后 GeekAI 生图线的真实 live image smoke，并确认 Nano Banana 2 图片显示/MIME 不再 404 或 mismatch。
6. 回到 S1D/S3 收口：permission edges、payer visibility、billing language、credits、usage 和 staged payment truth。
7. 在任何 live paid launch 前，补齐公开政策里的正式 operator 信息，并确认外部 uptime/status/backup/PITR/Sentry/APM。
8. 只有在前面闸门转绿后，才继续 S4 collaboration。
...
下一个主要检查点应当按这个明确顺序推进：在 signed-in browser 首轮和大部分第二轮 Board 验收已转绿的基础上，先部署并 spot-check 已完成 wiring 的 `Manage board -> Copy board` Free-plan limit 弹窗路径、Board title 特殊字符拒绝、admin/workspace invite mutations 和 Nano Banana 2 MIME 修复，再完成 Google/email 与 CORS/origin 验收，然后跑一条刷新后 GeekAI 生图线的真实 live image smoke，接着回到 S1D/S3 收口，最后才继续更深的 S4 Yjs/provider proof。避免重新引入 legacy canvas compatibility 行为，也不要为了不紧急的 backlog 项目打断这条顺序。
