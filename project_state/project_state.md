...
S3 also tightened its local production shape on 2026-05-13: admin hot GET paths were split toward dedicated read models, repeated access work was reduced, runtime Postgres now prefers `DATABASE_POOL_URL` when present, and slow SQL logging now surfaces queries over the configured threshold without leaking parameters. This improves the admin/billing/team lane for the upcoming real-DB staging smoke. Public staging repair has now moved forward again too: the Vercel production alias was republished from a cleaned root package, `CLERK_SECRET_KEY` was restored for the web runtime, `/boards/[boardId]` now returns the expected Clerk-protected `200` instead of a `500`, and real signed-in smoke now returns `200` for `/api/auth/session`, `/api/v1/admin/me`, operator users, finance summary and AI route metrics. Remaining S1B/S1C work is now the `Manage board -> Copy board` Free-plan limit modal edge, Google/email flow verification and final deploy-secret cleanup rather than basic auth reachability.
...
S1B Deploy/staging            88%  rebuilt Hetzner API host, public HTTPS API, Supabase Pro fresh schema, historical R2/board smoke, CORS and the Konva-only staging web deploy are green; the live env no longer depends on the retired dirty worktree, first signed-in browser and most second-round board checks are green, and the remaining work is R2 clean asset smoke, re-created staging admin/workspace/board data, the `Manage board -> Copy board` Free-plan limit modal edge, Google/email flow verification and one live AI smoke
...
1. Close the remaining `Manage board -> Copy board` Free-plan limit modal edge and rerun a small signed-in Board browser spot check on staging.
...
| S1B Staging Infra | `project_state_slice_S1B_staging_infra.md` | In progress; rebuilt public API host plus Supabase Pro fresh schema, historical R2/board smoke and current API health passed, the dirty old worktree has been retired after env migration, and R2 clean asset smoke plus re-created staging data, auth/email and the `Manage board -> Copy board` Free-plan limit modal edge still remain |
...
S1B Deploy/staging            88%  重建后的 Hetzner API、public HTTPS API、Supabase Pro fresh schema、历史 R2/board smoke、CORS 和 Konva-only staging Web deploy 已转绿；signed-in browser 首轮和大部分第二轮 Board 检查已转绿，剩余工作是 R2 clean asset smoke、重新创建 staging admin/workspace/board 数据、`Manage board -> Copy board` Free-plan limit 弹窗边界、Google/email 和一条 live AI smoke
...
1. 先补完 `Manage board -> Copy board` Free-plan limit 弹窗边界，并做一轮小范围 signed-in Board browser spot check。
...
| S1B Staging Infra | `project_state_slice_S1B_staging_infra.md` | 进行中；FastAPI / Supabase Pro fresh schema / 历史 R2 smoke 已通过，R2 clean asset smoke、重新创建 staging 数据、auth/email 和 `Manage board -> Copy board` Free-plan limit 弹窗边界仍待完成 |
...
下一个主要检查点应当按这个明确顺序推进：在 signed-in browser 首轮和大部分第二轮 Board 验收已转绿的基础上，先补完 `Manage board -> Copy board` Free-plan limit 弹窗边界，再完成 Google/email 与 CORS/origin 验收，然后跑一条刷新后四模型生图线的真实 live image smoke，接着回到 S1D/S3 收口，最后才继续更深的 S4 Yjs/provider proof。避免重新引入 legacy canvas compatibility 行为，也不要为了不紧急的 backlog 项目打断这条顺序。
