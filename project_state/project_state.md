...
2026-05-18 全项目验收 pass 已建立新的 active ledger：`dev-plans/p0-project-wide-acceptance-audit-2026-05-18.md`。本轮核对 PRD / ARCH / project_state / active dev-plans 后，没有把新的活跃功能文档移入 Archive；已有 Archive 仍只作为历史参考。当前事实继续保持：staging DB 以 fresh Supabase Pro 为唯一事实源，旧 Neon / Hetzner-local Postgres 只保留事故记录；R2 继续做图片对象存储；协同过程更新默认只走 WebSocket room memory，Postgres 只落 compacted/final snapshots；active AI provider env 是 Jiekou-first，GeekAI env 名称只可作为私密 operator 记录里的注释历史。真实 key 不写入 tracked docs，远端 runtime truth 保持在 Vercel env、Hetzner shared `api.env`、provider dashboards 和 private operator storage。
...
S1B Deploy/staging            89%  rebuilt Hetzner API host, public HTTPS API, Supabase Pro fresh schema, historical R2/board smoke, CORS and the Konva-only staging web deploy are green; the live env no longer depends on the retired dirty worktree, first signed-in browser and most second-round board checks are green, the `Manage board -> Copy board` Free-plan modal path is wired locally, and the remaining work is the staging modal spot check, R2 clean asset smoke, re-created staging admin/workspace/board data, Google/email flow verification and one live AI smoke
...
1. Deploy and rerun a small signed-in Board browser spot check for the now-wired `Manage board -> Copy board` Free-plan limit modal path on staging.
...
5. Finalize public policy/operator details and decide whether `/pricing` should read the backend public plan catalog before any live paid launch.
...
Done locally:
  ...
  Canvas-facing Jiekou-first local fast path for chat streaming, prompt optimization, image generation/edit/reference and analysis
...
| S1B Staging Infra | `project_state_slice_S1B_staging_infra.md` | In progress; rebuilt public API host plus Supabase Pro fresh schema, historical R2/board smoke and current API health passed, the dirty old worktree has been retired after env migration, the `Manage board -> Copy board` Free-plan modal path is wired locally, and R2 clean asset smoke plus re-created staging data, auth/email and the staging modal spot check still remain |
...
7. Fold the current Jiekou-first local fast path into the server provider-route adapter layer while preserving timeout-safe per-attempt observability and no-double-charge settlement before real provider charging.
...
与此同时，画布现在也有了一条通过 Web app 接入 Jiekou-first 的本地 fast path，用来验证用户流程：Chat 可以流式输出文本，Prompt Optimizer 可以流式优化出图提示词，Analysis 可以选择 OpenAI 系或 Gemini 系视觉分析模型，Image Gen / Image Gen 4 现在已经对齐 GPT Image 2、Nano Banana 2、Doubao Seedream 5.0 Lite 和 Jimeng 4.0 的参数界面。这对产品验证有价值，但还不是生产权限边界；2026-05-14 又补上了后端 `20260514_0021_ai_image_model_refresh` 对齐、移除了活跃生图面里的 `gemini-3.1-flash-image-preview`，并把长耗时生图超时抬到 240s。下一步 S2 cut 仍然是把这条 Jiekou-first 路径收口到 `dev-plans/s2-ai-provider-route-billing-control-plane-2026-05-07.md` 里定义的 server AiRun provider-route/billing control plane。
...
S1B Deploy/staging            89%  重建后的 Hetzner API、public HTTPS API、Supabase Pro fresh schema、历史 R2/board smoke、CORS 和 Konva-only staging Web deploy 已转绿；signed-in browser 首轮和大部分第二轮 Board 检查已转绿，`Manage board -> Copy board` Free-plan limit 弹窗路径已完成本地 wiring，剩余工作是 staging 弹窗 spot check、R2 clean asset smoke、重新创建 staging admin/workspace/board 数据、Google/email 和一条 live AI smoke
...
1. 先部署并做一轮小范围 signed-in Board browser spot check，确认已经完成 wiring 的 `Manage board -> Copy board` Free-plan limit 弹窗路径。
...
5. 在任何 live paid launch 前，补齐公开政策里的正式 operator 信息，并决定 `/pricing` 是否改读后端 public plan catalog。
6. 只有在前面闸门转绿后，才继续 S4 collaboration。
...
本地已完成：
  ...
  面向画布的 Jiekou-first 本地 fast path：chat streaming、prompt optimization、image generation/edit/reference 和 analysis
...
5. 在真实 provider charging 之前，先把当前 Jiekou-first 本地 fast path 收口到服务端 provider-route adapter layer，同时保留 timeout-safe 逐次尝试可观测性和 no-double-charge settlement 边界。
...
下一个主要检查点应当按这个明确顺序推进：在 signed-in browser 首轮和大部分第二轮 Board 验收已转绿的基础上，先部署并 spot-check 已完成 wiring 的 `Manage board -> Copy board` Free-plan limit 弹窗路径，再完成 Google/email 与 CORS/origin 验收，然后跑一条刷新后四模型生图线的真实 live image smoke，接着回到 S1D/S3 收口，最后才继续更深的 S4 Yjs/provider proof。避免重新引入 legacy canvas compatibility 行为，也不要为了不紧急的 backlog 项目打断这条顺序。
