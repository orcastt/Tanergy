...
- 2026-05-16 local-bridge cleanup follow-up: provider display-name / text-mode rules now live in one `providerApiConfig` helper instead of being repeated across analysis/image local bridge files, but local image execution still keeps a provider+model-family dispatch table in `geekAiImageRun.ts`, so frontend local fallback is not yet fully “admin route only”.
