...
- `/api/v1/admin/finance/manual/*` now provides audited owner/admin/finance write operations for Stripe-unavailable operations: top up a user personal wallet, top up a Team wallet, assign/change a Collaborate/Group user plan, assign/change a Team workspace plan and cancel a subscription. The `/admin` finance panel has matching controls.
...
- `/admin` has been reorganized into Overview, Users, Teams, Groups, AI API Routes, Finance and Access tabs. Users show personal wallet and Collaborate/Group plan controls; Teams show Team wallet/plan controls and member/board detail; Groups show shared workspace detail with owner-level personal billing controls; AI API Routes separates image/text route usage before the model/route/pricing control plane.
...
- Admin directory views can navigate from registered users to owned Team/Group workspaces, inspect Team/Group member and board detail, and enter the relevant manual billing operation while Stripe remains unavailable.
