...
**Status**: Konva v2 formal Board route is accepted as the production path for active Boards, and the remaining tldraw web runtime/reference code has now been removed from `apps/web`. Page polish, runtimeGraph mock dataflow, public share view, page-limit guard, node minimum sizing and first-pass collaboration are stable: permission-aware presence/read-only wiring, passive remote-save sync, guarded Yjs room/snapshot foundations, richer room-state semantics, visible sync state, deferred remote-apply handling, native page/shapes/edge Yjs storage, first-pass page-level remote merge/apply, provider-ready transport shaping, document/awareness initial-sync gating, occupancy-aware presence, local collaboration undo/redo, low-rate draft drawing presence preview and a first-pass FastAPI websocket room provider whose default persistence mode stores final snapshots instead of every process update. Export/background polish, real AiRun and production-grade multi-instance provider/awareness remain.
...
- A first-pass FastAPI websocket room provider now sits behind the shared board-realtime transport. When a remote persistence API is configured, awareness/document hooks prefer websocket transport; otherwise they fall back to the existing `BroadcastChannel` rooms. The current server room is intentionally lightweight: it gates access through board collaboration permissions, exposes an explicit room-seed handshake so stale reconnects do not immediately overwrite server state, keeps ordinary `yjs-update` process traffic in room memory by default, persists accepted compacted/final realtime document snapshots to local-dev/Postgres storage, replays the current room snapshot/update state to newcomers, broadcasts awareness batch/state/remove events, and retains the old incremental update-chain persistence only behind the explicit `TANGENT_BOARD_REALTIME_PERSIST_MODE=update_chain` rollback switch. Websocket contract tests now cover replay, final-snapshot persistence, legacy update-chain compatibility, compaction request/replace, disconnect cleanup, room-key mismatch and visibility gating.
...
- True production collaboration transport/provider wiring still remains pending: the websocket room is single-process/in-memory for process fan-out, does not yet use Redis or a dedicated Yjs server/provider stack, and is not yet multi-instance deploy safe. Postgres is now reserved for final realtime snapshots, Board History and business facts rather than per-stroke/per-update process traffic.
- 2026-05-20 low-zoom image preview fix: canvas image shapes, Image Nodes and generated-output slots no longer choose preview URLs from fixed zoom cutoffs such as `zoom <= 0.25`. They now pick 256/512/1024/original sources from the shape or slot's estimated on-screen pixel edge, so zooming the canvas far out still keeps a large resized image visible while genuinely tiny previews stay on smaller thumbnails to protect memory. Existing loaded-image cache count and pixel budgets remain unchanged.
...
- Current Alembic/Postgres migrations.
- R2 asset storage.
...
- 2026-05-19 Remove BG reconnect pass: single persisted image selections now show the floating image actions, Remove BG calls the FastAPI `/api/v1/image-ops/remove-background` route, returns backend error details to the canvas toolbar and creates a new transparent PNG asset near the source. The staging API compose build now enables `services/api[image-ops]`; final acceptance is a staging uploaded-image smoke after deployment.
...
- 2026-05-18 canvas/collaboration polish: Board header now keeps only Back plus inline rename, removing the Board switcher/new/open affordances from the canvas top bar; Pages drawer now reads live plan page limits and opens a centered upgrade dialog when create/duplicate would exceed the current plan; page thumbnail rendering moved into a focused component; node cards now enforce registry default minimum size on load/resize so chat/input controls cannot shrink below their layout contract; drawing/shape creation publishes sanitized draft previews through presence at a low cadence so collaborators can see the process before the final Yjs update lands.
