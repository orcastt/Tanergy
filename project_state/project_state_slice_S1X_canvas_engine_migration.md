...
**Status**: Phase 4 runtimeGraph cleanup first pass in progress after accepted Phase 1A/2A/3 spike baselines.
...
- 2026-05-04 Phase 4A capture/export first pass is implemented: selection toolbar Capture renders explicit `selectedIds` bounds to transparent PNG with overlays/highlights hidden, uploads through the Asset API with `origin=merge_capture`, and creates an Image Node below the selection. Right-click Copy as PNG and Export as PNG use the same selectedIds transparent capture; Copy/Export as SVG use a limited vector serializer that renders canvas images and Image Nodes from asset URLs, while unsupported node internals stay simplified. Board/node state still stores only asset refs/metadata, not raw image bytes.
- 2026-05-04 Phase 4A capture UX debt is recorded: Capture selection currently uses the visible Konva stage in a temporary capture mode to hide overlays/non-selected shapes before `toDataURL()`, so users can see a brief flash. Data behavior is accepted for this pass, but the next polish should render capture through an offscreen stage/render tree to avoid live canvas flicker.
...
Phase 4A selection capture/export hand-test: Capture to Image Node, Copy as PNG/SVG and Export as PNG/SVG from selectedIds bounds
```

After this checkpoint, hand-test Capture selection → Image Node on shape/image/markup selections, Copy as PNG into an external app, and Export as PNG/SVG file downloads. Move to page, transparent background toggle, merge preview, offscreen capture to remove visible flicker, multi-page contracts, real AiRun execution and Board/Yjs persistence remain follow-up work.
