...
Hand-test Page UI on `/boards/[boardId]`: create a second page, draw different objects on each page, rename a page by double-clicking its row, save/reload, create/restore a Snapshot and confirm pages do not bleed into each other. Next whole blocks are true page thumbnails/delete/reorder/Move to page, transparent-background/export polish, precise legacy style/binding migration if hand-test finds gaps, real AiRun execution, and then Phase 6 Yjs collaboration.
