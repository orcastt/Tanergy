...
After this checkpoint, hand-test Capture selection → Image Node on shape/image/markup selections, Copy as PNG into an external app, and Export as PNG/SVG file downloads. Move to page, transparent background toggle, merge preview, multi-page contracts, real AiRun execution and Board/Yjs persistence remain follow-up work.
